...
out = Path('monic_scalar_v2/source').resolve()
...
raw=cp.stdout+'\n'+cp.stderr
print('MONIC_SCALAR_V2_EXIT',cp.returncode)
print(raw[-20000:])
Path('monic_scalar_v2').mkdir(exist_ok=True)
Path('monic_scalar_v2/result.json').write_text(json.dumps({'exit':cp.returncode,'tail':raw[-30000:]},indent=2))
