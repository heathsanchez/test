...
out = Path('monic_scalar_v4/source').resolve()
...
theorem inv_product_modeq (p a b : Nat) (hp : Galoistools.PrimeField p)
  ...
  have hR : NatModEq p (c * d) 1 := by
    unfold NatModEq
    simpa [Nat.mod_eq_of_lt hp1] using hright
  have hL : NatModEq p (ic * c) 1 := by
    unfold NatModEq
    simpa [Nat.mod_eq_of_lt hp1] using hleft
  ...
  have hfinal : NatModEq p (ic * 1) (1 * d) := h1'.symm.trans h2
  simpa [ic, c, d] using hfinal
'''

p = out/'Probe.lean'; p.write_text(probe)
cp=subprocess.run(['lake','lean',p.name],cwd=out,text=True,capture_output=True)
raw=cp.stdout+'\n'+cp.stderr
print('MONIC_SCALAR_V4_EXIT',cp.returncode)
print(raw[-20000:])
Path('monic_scalar_v4').mkdir(exist_ok=True)
Path('monic_scalar_v4/result.json').write_text(json.dumps({'exit':cp.returncode,'tail':raw[-30000:]},indent=2))
