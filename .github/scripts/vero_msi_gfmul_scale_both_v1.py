...
out = Path('msi_gfmul_scale_both_v1/source').resolve()
...
theorem gfMul_scale_both
  ...
  exact gfStrip_map_scale p (a*b)
    (Galoistools.convolve p f.reverse g.reverse).reverse hzero
...
raw=cp.stdout+'\n'+cp.stderr
print('MSI_GFMUL_SCALE_BOTH_V1_EXIT',cp.returncode)
print(raw[-26000:])
Path('msi_gfmul_scale_both_v1').mkdir(exist_ok=True)
Path('msi_gfmul_scale_both_v1/result.json').write_text(json.dumps({'exit':cp.returncode,'tail':raw[-34000:]},indent=2))
