...
out = Path('msi_gfstrip_separator_v4/source').resolve()
create_sandbox(bench, out, mode='codeproof', overwrite=True, seed_artifact=seed)

header = '''import Galoistools.Proof.Ring
import Galoistools.Impl.Division

namespace GaloistoolsMSIGfStripSeparatorV4
'''
footer = '\nend GaloistoolsMSIGfStripSeparatorV4\n'
...
Path('msi_gfstrip_separator_v4').mkdir(exist_ok=True)
Path('msi_gfstrip_separator_v4/census.json').write_text(json.dumps(census,indent=2))
print('MSI_GFSTRIP_SEPARATOR_V4', json.dumps([{'probe':x['probe'],'exit':x['exit']} for x in census]))
