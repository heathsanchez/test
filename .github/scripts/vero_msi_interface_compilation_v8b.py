...
ROOT=Path.cwd(); SOURCE=(ROOT/'coldcert'/'project').resolve(); OUT=(ROOT/'vero_msi_interface_compilation_v8b').resolve()
if OUT.exists(): shutil.rmtree(OUT)
OUT.mkdir(); DIV=Path('Galoistools/Proof/Division.lean')

GEN2=r'''
@[simp] theorem msi_gfMonic_nil (p : Nat) : (Galoistools.gfMonic [] p).snd = [] := by rfl
@[simp] theorem msi_gfMonic_if_nil (g : List Nat) (p : Nat) :
    (if g = [] then [] else (Galoistools.gfMonic g p).snd) = (Galoistools.gfMonic g p).snd := by
  by_cases h : g = []
  · subst g; simp
  · simp [h]
'''
STRUCT=r'''
theorem msi_refGfStrip_ne_zero_cons : ∀ xs ys : List Nat, Galoistools.refGfStrip xs ≠ 0 :: ys := by
...
@[simp] theorem msi_eval_append_zeros (p x n : Nat) (f : List Nat) (hp : 0 < p) :
  ...
  | succ n ih =>
      ...
      rw [ih (f := f ++ [0]), msi_eval_append_zero, Nat.pow_succ]
...
'''
MUL_EVAL=r'''
@[simp] theorem msi_mul_eval_hom (p x : Nat) (u v : List Nat) :
  ...
  · rw [if_neg hzero, refPolyEval_gfStrip]
    ...
    have hevalmod : Galoistools.refPolyEvalRevAux p x (Galoistools.convolve p u.reverse v.reverse) % p =
        Galoistools.refPolyEvalRevAux p x (Galoistools.convolve p u.reverse v.reverse) := by
      cases Galoistools.convolve p u.reverse v.reverse <;> simp [Galoistools.refPolyEvalRevAux]
    calc
      ...
      _ = (Galoistools.refPolyEvalRevAux p x u.reverse * Galoistools.refPolyEvalRevAux p x v.reverse) % p := hconv
'''
SHAM=r'''@[simp] theorem msi_v8b_sham (n : Nat) : n + 0 = n := by simp'''
...
def run(label,ring=False,reduced=False,padding=False,mul=False,sham=False):
  ...
  return {'exit':cp.returncode,'error_count':len(errs),'infra_errors':infra,'error_lines':errs,'tail':raw[-42000:]}

matrix={
 'structural':run('structural'),
 'sham':run('sham',sham=True),
 'ring_import_control':run('ring_import_control',ring=True),
 'eval_reduced':run('eval_reduced',ring=True,reduced=True),
 'eval_padding':run('eval_padding',ring=True,padding=True),
 'mul_eval':run('mul_eval',ring=True,mul=True),
 'joint_eval':run('joint_eval',ring=True,padding=True,mul=True),
}
for k,v in matrix.items(): print('MSI_V8B_ARM',k,json.dumps({'exit':v['exit'],'error_count':v['error_count'],'infra_errors':len(v['infra_errors'])}))
base=matrix['ring_import_control']['error_count']
payload={'schema':'msi.vero-interface-compilation.v8b','matrix':matrix,'gains_vs_import_control':{k:base-v['error_count'] for k,v in matrix.items()}}
(OUT/'result.json').write_text(json.dumps(payload,indent=2))
print('VERO_MSI_INTERFACE_COMPILATION_V8B',json.dumps(payload['gains_vs_import_control']))
