...
BLOCK_RE = re.compile(
    ...
    r'(?P<body>.*?)'
    r'(?P<tail>\n-- !benchmark @end proof def=(?P=name))',
    re.S,
)


def isolate_all_other_proves(src: str) -> str:
    """Make process exit status depend on exactly one benchmark proof.

    All nonselected benchmark `kind=prove` bodies become `sorry`.  Definitions,
    specs, implementation code, proof auxiliaries, sat/unsat targets, imports and
    the selected theorem are unchanged.  This is verifier isolation only.
    """
    seen=[]
    def repl(m):
        name=m.group('name'); seen.append(name)
        if name == TARGET:
            return m.group(0)
        return m.group('head') + '  sorry' + m.group('tail')
    out=BLOCK_RE.sub(repl,src)
    if TARGET not in seen:
        raise RuntimeError(f'sealed target not found among prove benchmarks: {TARGET}')
    return out


def patch_selected(src: str, body: str) -> str:
    def repl(m):
        if m.group('name') != TARGET: return m.group(0)
        return m.group('head') + body + m.group('tail')
    out,n = BLOCK_RE.subn(repl,src)
    if n == 0: raise RuntimeError('no prove benchmark blocks found')
    return out
...
def run_candidate(i,label,body):
    ...
    src=isolate_all_other_proves(pf.read_text())
    pf.write_text(patch_selected(src,body))
    ...
    return {'index':i,'label':label,'exit':cp.returncode,'success':cp.returncode==0,'error_count':len(errors),'error_tail':errors[-8:]}

rows=[]; winner=None
for i,(label,body) in enumerate(CANDIDATES,1):
    row=run_candidate(i,label,body); rows.append(row)
    print('SEALED_PROBE',json.dumps({k:row[k] for k in ('index','label','exit','error_count')},sort_keys=True))
    if row['success']:
        winner={'index':i,'label':label,'body':body}; break

result={
 'schema':'msi.vero-sealed-target-discovery.v1-fully-isolated',
 ...
 'harness_fix':'all nonselected benchmark kind=prove bodies replaced by sorry solely to isolate selected theorem exit status',
 'budget':BUDGET,'queries_used':len(rows),'success':winner is not None,'winner':winner,'rows':rows,
}
