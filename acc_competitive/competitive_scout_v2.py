...
def main():
    ...
    p.add_argument("--mechanism-id",default="gssub-v2-current")
    a=p.parse_args()
    ...
    defend=[]
    for cid,row in ac.items():
        if cid not in fmap: continue
        live_status=row.get("status")
        if live_status not in ("solved","unsolved"): continue
        best=row.get("currentBestLength"); k=row.get("kTeams")
        if live_status=="solved" and (not isinstance(best,int) or best<100): continue
        os=our_ac.get(cid)
        ...
        if cid in set(competitive_attempted) and status not in ("unique_hold","tied_hold"):
            continue
        ...
        # Point-centric opportunity estimate. Unsolved cells are full new
        # scoring opportunities. Solved cells with fewer holders are more
        # valuable to attack or tie, while final publication independently
        # accepts any scoring tie.
        value=1.0 if live_status=="unsolved" else (1.0/max(1,(k or 1)))
        length_score=math.log1p(best if isinstance(best,int) else max(1,fmap[cid]["total_len"]))
        lost_bonus=1.5 if status=="lost" else 0.0
        recent_bonus=0.8 if cid in event_set else 0.0
        if status=="unique_hold":
            # Possession is part of the objective. Spend a bounded fraction of
            # compute trying to deepen vulnerable owned records rather than
            # treating defense as unreachable dead code.
            vulnerability=(1.5 if cid in event_set else 0.0) + 0.25*length_score
            score=1.25*length_score + 1.25*cap + 0.75*mov + vulnerability
            defend.append({
              ...
              "mathgraph_status":status,"recent_public_move":cid in event_set,
              "vulnerability_score":vulnerability,**fmap[cid],
            })
            continue
        ...
        unsolved_bonus=2.0 if live_status=="unsolved" else 0.0
        score=2.5*value + 0.75*length_score + 1.25*cap + 0.75*mov + lost_bonus + recent_bonus + tie_break_bonus + unsolved_bonus
        acquire.append({
    ...
    report={
      ...
      "objective":"maximize and preserve scoring cells; ties are scoring outcomes and owned unique records are defendable",
    }
