...
This gives a clean epistemic boundary:
  ...
  exact Atlas join -> executable suffix
...
def build_index(core, atlas_path, out_db, total_cap, progress_every=10000):
    ...
    for k, d, nm in rows:
        ...
        batch.append((bs, fs, int(d), distance_band(int(d)), None if nm is None else int(nm)))
        n += 1
        if len(batch) >= 2000:
    ...
    report = {
        "version": "acc-future-quotient-v1",
        "status": "CANDIDATE_REPRESENTATION",
        "trusted_boundary": "search guidance only; exact Atlas joins plus pinned official verifier remain authoritative",
...
def search_target(
    ...
    base_idx, future_idx, policies = load_index(index_path)
    adb = sqlite3.connect(atlas_path)
...
def cmd_search(a):
    ...
    report = {
        ...
        "claim_boundary": "Future quotient and learned policy are search-order heuristics only. Every emitted row is reconstructed through an exact Proof Atlas state and replayed by the pinned official verifier.",
    }
