...
def connect(core,initial,orbit_idx,db,normalizers,bound_exclusive,depth_limit,total_cap,node_cap):
    ...
    while q and nodes<node_cap:
        ...
        if hit is not None:
            ...
            if predicted<bound_exclusive and (best is None or predicted<best[0]):
    ...
    if best is None:return None,{'nodes':nodes,'orbit_hits':orbit_hits,'node_cap_hit':nodes>=node_cap}
    ...
    if suffix is None:return None,{'nodes':nodes,'orbit_hits':orbit_hits,'hit':True,'suffix_fail':True,'node_cap_hit':nodes>=node_cap}
    transported=[map_move(m,invh) for m in suffix]
    norm=normalizers[SYM_INDEX[invh]]
    path=prefix+transported+norm
    return path,{
        'nodes':nodes,'orbit_hits':orbit_hits,'node_cap_hit':nodes>=node_cap,
        'prefix':len(prefix),'atlas_suffix':len(suffix),'transported_suffix':len(transported),
...
def main():
    ...
    for cid in targets:
        ...
        if path is not None:
            ...
            if not v.get('ok'):raise RuntimeError(f'orbit connector verifier fail {cid}: {v}')
        ...
        rows.append(rec);print('TRANSPORT_ORBIT_CASE',json.dumps(rec,sort_keys=True))

    (out/'results.json').write_text(json.dumps(rows,indent=2,sort_keys=True)+'\n')
    (out/'pending_submission.txt').write_text('\n'.join(lines)+('\n' if lines else ''))
    summary={
      'version':'transport-orbit-atlas-connector-v1','targets':len(rows),
    ...
    print('TRANSPORT_ORBIT_SUMMARY',json.dumps(summary,sort_keys=True));db.close()
