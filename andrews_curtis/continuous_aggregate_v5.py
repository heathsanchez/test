...
def scoring_candidate(row,candidate_len,our_best=None):
    ...
    # Do not spend a submission on a score cell we already hold at the same
    # or shorter length.
    ...
    if isinstance(best,int) and candidate_len == best:
        # ACC awards reduced score to tied shortest solutions. A tie is a
        # scoring consequence, not a failure.
...
def main():
    ...
    # One authenticated read prevents wasting scarce submissions on ties or
    # records we already own at an equal/better length.
    st,_,mine_obj=api("GET","/competitions/acc/submissions/mine")
    if not 200 <= st < 300:
    ...
    for sub in md.get("items",[]):
        for rr in sub.get("results",[]):
            ...
            qid=rr.get("challenge_id"); n=rr.get("length")
    ...
    report={
        ...
        "selected_scoring_ties":sum(x.get("selected") and x.get("reason")=="scoring_tie" for x in selection),
        "selected_strict_improvements":sum(x.get("selected") and x.get("reason")=="strict_improvement" for x in selection),
        "selected_unsolved":sum(x.get("selected") and x.get("reason")=="currently_unsolved" for x in selection),
