...
/-! A sound interpreter for certificate-program trees, independent of synthesis. -/
namespace CrossDomainResidual.ProgramRules
...
theorem domain_nonneg (d : Domain) {x : ℝ}
  ...
  | ray => exact hx
  | interval l u =>
      exact le_trans hd.1 hx.1

/-- Every well-formed program denotes a nonnegative function on its domain. -/
...
#print axioms sound
end CrossDomainResidual.ProgramRules
