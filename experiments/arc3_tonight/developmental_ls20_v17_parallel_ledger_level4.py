...
class ParallelAuthority:
    ...
    def eval(self,path):
        ...
        for j,a in enumerate(path):
            o=e.step(am[a],data={},reasoning={'mode':'parallel_ledger','depth':j+1})
            local+=1
            if o is None or sname(o)=='GAME_OVER':
...
def main():
    ...
    max_depth=160; max_expansions=4000
    ...
    result={'classification':'ARC3_PARALLEL_SELF_LEDGER_LEVEL4',
    ...
    with open('arc3_developmental_v17_parallel_ledger_level4_result.json','w') as f:
        json.dump(result,f,indent=2,sort_keys=True)
    print('RESULT',json.dumps(result,sort_keys=True))
    if q.get('ok'):
        print('VERIFIED_EXTERNAL_ARC3_LEVEL4_PARALLEL_SELF_LEDGER_BREAKTHROUGH')
    elif goal:
        print('LEVEL4_PARALLEL_LEDGER_FAILED_REPLAY')
    else:
        print('LEVEL4_PARALLEL_LEDGER_FRONTIER')
