...
def hregions(a,regions):
    ...
    resource_bit = 1 if np.any(a[61:63,:] == 11) else 0
    h.update(bytes([resource_bit]))
    return h.hexdigest()
...
def main():
    ...
    result={'classification':'ARC3_CONTINUOUS_CURIOSITY_SELF_LEDGER_LEVEL4_WITH_ONE_BIT_RESOURCE',
        ...
        'scene':list(K.scene)}
    with open('arc3_developmental_v22_one_bit_resource_ledger_result.json','w') as f:
        json.dump(result,f,indent=2,sort_keys=True)
    print('RESULT',json.dumps(result,sort_keys=True))
    if q.get('ok'):
        print('VERIFIED_EXTERNAL_ARC3_LEVEL4_ONE_BIT_RESOURCE_LEDGER_BREAKTHROUGH')
    elif collisions:
        print('CERTIFIED_NEXT_LEVEL4_RESIDUAL_AFTER_ONE_BIT_RESOURCE')
    elif goal_edge:
        print('LEVEL4_CONTINUOUS_GOAL_GRAPH_COMPRESSION_FAILED_REPLAY')
    else:
        print('LEVEL4_ONE_BIT_RESOURCE_FRONTIER')
