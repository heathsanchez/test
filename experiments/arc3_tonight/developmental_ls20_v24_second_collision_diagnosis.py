...
def hregions(a,regions):
    ...
    resource_count = int(np.sum(a[61:63,:] == 11))
    h.update(resource_count.to_bytes(2,'little',signed=False))
    return h.hexdigest()
...
def main():
    ...
    for step in range(1,max_steps+1):
        ...
        if z is None or sname(z)=='GAME_OVER':
            ...
            if old is not None and old!='TERM':
                ...
                with open('arc3_developmental_v24_second_collision_result.json','w') as f: json.dump(report,f,indent=2,sort_keys=True)
                break
            if old is None:
    ...
    result={'classification':'ARC3_CONTINUOUS_CURIOSITY_SELF_LEDGER_LEVEL4_WITH_SCALAR_RESOURCE_COUNT',
        ...
        'scene':list(K.scene)}
    with open('arc3_developmental_v24_second_collision_ledger_result.json','w') as f:
        json.dump(result,f,indent=2,sort_keys=True)
    print('RESULT',json.dumps(result,sort_keys=True))
    if q.get('ok'):
        print('VERIFIED_EXTERNAL_ARC3_LEVEL4_SCALAR_RESOURCE_COUNT_LEDGER_BREAKTHROUGH')
    elif collisions:
        print('CERTIFIED_NEXT_LEVEL4_RESIDUAL_AFTER_SCALAR_RESOURCE_COUNT')
    elif goal_edge:
        print('LEVEL4_CONTINUOUS_GOAL_GRAPH_COMPRESSION_FAILED_REPLAY')
    else:
        print('LEVEL4_SCALAR_RESOURCE_COUNT_FRONTIER')
