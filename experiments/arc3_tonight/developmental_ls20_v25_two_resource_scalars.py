...
def hregions(a,regions):
    ...
    # Counterexample-earned one-bit causal resource register.
    # V20/V21 proved two otherwise identical presents had different futures
    # exactly when the excluded bottom-strip marker value 11 was present vs absent.
    resource_count_11 = int(np.sum(a[61:63,:] == 11))
    resource_count_8 = int(np.sum(a[61:63,:] == 8))
    h.update(resource_count_11.to_bytes(2,'little',signed=False))
    h.update(resource_count_8.to_bytes(2,'little',signed=False))
    return h.hexdigest()
...
def main():
    ...
    for step in range(1,max_steps+1):
        ...
        before=cur
        pre_frame=grid(o).copy()
        pre_path=list(actual_path)
        ...
        if z is None or sname(z)=='GAME_OVER':
            ...
            if old is not None and old!='TERM':
                first=edge_first[(before,a)]
                diff=first['frame']!=pre_frame
                report={
                    'step':step,'state':before,'action':a,'old':old,'new':'TERM',
                    'first_step':first['step'],'first_frame_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(first['frame'][60:64,:],return_counts=True))},
                    'second_frame_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(pre_frame[60:64,:],return_counts=True))},
                    'first_row61_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(first['frame'][61,:],return_counts=True))},
                    'second_row61_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(pre_frame[61,:],return_counts=True))},
                    'first_row62_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(first['frame'][62,:],return_counts=True))},
                    'second_row62_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(pre_frame[62,:],return_counts=True))},
                    'diff_pixels':int(diff.sum()),'diff_rows':[int(x) for x in diff.sum(axis=1)],
                    'diff_cols':[int(x) for x in diff.sum(axis=0)],
                    'first_path_len':len(first['path']),'second_path_len':len(pre_path),
                    'first_suffix':first['path'][-40:],'second_suffix':pre_path[-40:]
                }
                print('COLLISION_DETAIL',json.dumps(report,sort_keys=True))
                with open('arc3_developmental_v25_two_resource_scalars_result.json','w') as f: json.dump(report,f,indent=2,sort_keys=True)
                break
            if old is None:
                edge_first[(before,a)]={'step':step,'frame':pre_frame,'path':pre_path}
        ...
        if old is not None and old!=ns:
            ...
            diff=first['frame']!=pre_frame
            report={
                'step':step,'state':before,'action':a,'old':old,'new':ns,
                'first_step':first['step'],
                'first_frame_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(first['frame'][60:64,:],return_counts=True))},
                'second_frame_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(pre_frame[60:64,:],return_counts=True))},
                'first_row61_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(first['frame'][61,:],return_counts=True))},
                'second_row61_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(pre_frame[61,:],return_counts=True))},
                'first_row62_counts':{str(int(v)):int(n) for v,n in zip(*np.unique(first['frame'][62,:],return_counts=True))},
    ...
    result={'classification':'ARC3_CONTINUOUS_CURIOSITY_SELF_LEDGER_LEVEL4_WITH_TWO_RESOURCE_SCALARS',
        ...
        'scene':list(K.scene)}
    with open('arc3_developmental_v25_two_resource_scalars_ledger_result.json','w') as f:
        json.dump(result,f,indent=2,sort_keys=True)
    print('RESULT',json.dumps(result,sort_keys=True))
    if q.get('ok'):
        print('VERIFIED_EXTERNAL_ARC3_LEVEL4_TWO_RESOURCE_SCALAR_BREAKTHROUGH')
    elif collisions:
        print('CERTIFIED_NEXT_LEVEL4_RESIDUAL_AFTER_TWO_RESOURCE_SCALARS')
    elif goal_edge:
        print('LEVEL4_CONTINUOUS_GOAL_GRAPH_COMPRESSION_FAILED_REPLAY')
    else:
        print('LEVEL4_TWO_RESOURCE_SCALAR_FRONTIER')
