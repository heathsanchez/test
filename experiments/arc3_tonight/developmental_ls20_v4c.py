...
def explore_level(env,obs,amap,prior,max_actions=7000):
    ...
    def restore():
        ...
        # ARC's reset is level-local once progress has been made. Respect that
        # environment semantics first; only replay prior compiled levels if a
        # reset unexpectedly returns to an earlier level.
        ro=env.reset(); resets+=1; episode=[]
        if ro is not None and ro.levels_completed==level:
            return ro
...
def main():
    ...
    result={'classification':'ARC3_LEVEL_LOCAL_RESET_REPLAY_QUALIFIED_DEVELOPMENTAL_COMPILATION','observation_channel':list(EXTRA),
            'levels_developed':len(sols),'records':records,'final_independent_replay':final,'discovery_score':discovery_score}
    with open('arc3_developmental_v4c_result.json','w') as f: json.dump(result,f,indent=2,sort_keys=True)
