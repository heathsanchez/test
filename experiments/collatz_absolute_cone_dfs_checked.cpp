...
[[noreturn]] static void range_fail(const char* what){
  std::cerr<<"SYMBOLIC_RANGE_FAIL "<<what<<"\n";
  std::exit(9);
}
static u128 add_checked(u128 a,u128 b,const char* what){
  if(a>UMAX-b)range_fail(what);
  return a+b;
}
static u128 mul_checked(u128 a,u128 b,const char* what){
  if(a && b>UMAX/a)range_fail(what);
  return a*b;
}
static i128 as_i128(u128 x,const char* what){
  if(x>I128MAX)range_fail(what);
  return i128(x);
}
static u128 eval_checked(u128 A,uint64_t q,u128 B,const char* what){
  return add_checked(mul_checked(A,u128(q),what),B,what);
}
...
static bool uniform_lt(
  ...
  const i128 coef=as_i128(Ac,"uniform_lt_Ac")-as_i128(Bc,"uniform_lt_Bc");
  const i128 cons=as_i128(Ad,"uniform_lt_Ad")-as_i128(Bd,"uniform_lt_Bd");
...
static bool uniform_cone(const State&s){
  ...
  const i128 coef=2*as_i128(s.Yc,"cone_Yc")-3*as_i128(s.Nc,"cone_Nc");
  const i128 cons=2*as_i128(s.Yd,"cone_Yd")-3*as_i128(s.Nd,"cone_Nd")-1;
...
static void dfs(const State&s,int t,int H,Stats&st){
  ...
  if(s.L==s.U){
    ++st.singleton_nodes;
    const u128 q=s.L;
    const u128 nu=eval_checked(s.Nc,uint64_t(q),s.Nd,"singleton_n");
    const u128 yu=eval_checked(s.Yc,uint64_t(q),s.Yd,"singleton_y");
  ...
  for(int e=0;e<2;++e){
    ...
    State z;
    z.L=L2;z.U=U2;
    z.Nc=mul_checked(2,s.Nc,"child_Nc");
    z.Nd=add_checked(s.Nd,mul_checked(u128(e),s.Nc,"child_Nd_mul"),"child_Nd");

    const u128 rawYc=mul_checked(2,s.Yc,"raw_Yc");
    const u128 rawYd=add_checked(s.Yd,mul_checked(u128(e),s.Yc,"raw_Yd_mul"),"raw_Yd");
    if(rawYd&1){
      z.Yc=mul_checked(3,rawYc,"odd_Yc")/2;
      z.Yd=add_checked(mul_checked(3,rawYd,"odd_Yd_mul"),1,"odd_Yd_add")/2;
    }else{
      z.Yc=rawYc/2;
      z.Yd=rawYd/2;
    }
...
int main(int argc,char**argv){
  ...
  if(K<2||K>48||H<1||H>4096||SHARDS<1||SID>=SHARDS)return 2;
