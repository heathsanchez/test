...
static void consequence_refine(const State& s, int budget,
  ...
  if (reverse_eo_closes(s,macroDepth,emax,usedMacro,expanded)) {
    ++st.macro_closed;
  ...
  // Adaptive 3-adic refinement: try one exact q mod 3 split, but retain it
  // only if at least one child is discharged. With budget=1 the trial has
  // exactly three children and no deeper split, so trial.size()<3 is exactly
  // "this ternary digit exposed a new consequence".
  if (budget>0) {
    ...
    // Logical state was not refined; account only for the search work.
...
int main(int argc,char**argv) {
  if (argc!=5) {
    std::cerr<<"usage: adaptive_reverse DEPTH ADAPTIVE_TERNARY_BUDGET MACRO_DEPTH EMAX\n";
    return 2;
  }
  const int DEPTH=std::stoi(argv[1]);
  const int RB=std::stoi(argv[2]);
  const int MD=std::stoi(argv[3]);
  const int EM=std::stoi(argv[4]);
  if (DEPTH<1 || DEPTH>22 || RB<0 || RB>4 || MD<0 || MD>8 || EM<0 || EM>9) return 2;
  ...
  std::cout<<"ADAPTIVE23_REVERSE_START depth="<<DEPTH<<" ternary_budget="<<RB
  ...
  for (int depth=1; depth<=DEPTH; ++depth) {
    ...
    std::cout<<"ADAPTIVE23_REVERSE_LEVEL depth="<<depth
             <<" parents="<<live.size()
  ...
  std::cout<<"ADAPTIVE23_REVERSE_RESULT depth="<<DEPTH
           <<" ternary_budget="<<RB
           <<" macro_depth="<<MD
           <<" emax="<<EM
           <<" survivors="<<live.size()
  ...
  std::cout<<"FINAL_GATE=ADAPTIVE23_REVERSE_D"<<DEPTH<<"_R"<<RB
           <<"_M"<<MD<<"_E"<<EM<<"\n";
  return 0;
}
