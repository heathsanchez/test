...
using DonorBuckets = std::vector<
    std::unordered_map<uint64_t,std::vector<RevCert>>>;

static DonorBuckets load_all_donors(const std::string& path,int& max_o){
  std::ifstream in(path);
  if(!in){std::cerr<<"ALL_DONOR_BANK_OPEN_FAILED\n";std::exit(20);}
  std::vector<RevCert> all;max_o=0;std::string line;
  while(std::getline(in,line)){
    if(line.empty())continue;
    std::istringstream ss(line);RevCert z{};
    if(!(ss>>z.o>>z.steps>>z.C>>z.residue)){
      std::cerr<<"ALL_DONOR_BANK_PARSE_FAILED\n";std::exit(21);
    }
    max_o=std::max(max_o,z.o);all.push_back(z);
  }
  DonorBuckets bank(max_o+1);
  uint64_t unique=0;
  for(const auto& z:all){
    auto& bucket=bank[z.o][z.residue];
    bool seen=false;
    for(const auto& u:bucket)
      if(u.steps==z.steps && u.C==z.C){seen=true;break;}
    if(!seen){bucket.push_back(z);++unique;}
  }
  std::cout<<"ALL_DONOR_BANK raw="<<all.size()
           <<" unique="<<unique
           <<" max_o="<<max_o<<"\n";
  return bank;
}
...
int main(int argc,char**argv){
  if(argc!=5){std::cerr<<"usage: all_donor_macro K COMPACT_BANK ALL_BANK MACRO_DEPTH\n";return 2;}
  const int K=std::stoi(argv[1]);
  const std::string bankpath=argv[2];
  const std::string allbankpath=argv[3];
  const int macro_depth=std::stoi(argv[4]);
  if(K<1||K>34||macro_depth<1||macro_depth>8){
    std::cerr<<"K_OR_MACRO_DEPTH_OUT_OF_RANGE\n";return 2;
  }
  ...
  int all_max_o=0;
  const auto all_donors=load_all_donors(allbankpath,all_max_o);
  ...
  for(int k=1;k<=K;++k){
    ...
    for(const State&s:cur){
      ...
      for(int e=0;e<2;++e){
        ...
        // Goal-relative donor refinement: search every exact q14
        // first-contraction donor whose cylinder contains this future state,
        // not only the prefix-free donor retained for the older goal.
        RevCert mb{};
        int md=0,msteps=0;i128 qA=0,qD=0;
        bool macro_closed=false;
        {
          const int top=std::min(c,all_max_o);
          uint64_t mod=1;
          for(int o=1;o<=top && !macro_closed;++o){
            mod*=3;
            auto it=all_donors[o].find(d%mod);
            if(it==all_donors[o].end())continue;
            for(const RevCert& cand:it->second){
              const i128 nc=(i128(1)<<cand.steps)*i128(d)-i128(cand.C);
              if(nc%i128(mod)!=0){
                std::cerr<<"ALL_DONOR_BASE_DIV_FAIL\n";return 12;
              }
              const i128 mD=nc/i128(mod);
              const i128 mA=(i128(1)<<cand.steps)*i128(p3[c-cand.o]);
              int tmd=0,tmsteps=0;i128 tA=0,tD=0;
              if(macro_blocks_close(
                    b,k,mA,mD,macro_depth,tmd,tmsteps,tA,tD)){
                mb=cand;md=tmd;msteps=tmsteps;qA=tA;qD=tD;
                macro_closed=true;
                break;
              }
            }
          ...
          if(macro_controls<20000){
            for(uint64_t a:{1ULL,3ULL}){
              ...
              if(ps<=0||u128(ps)>=n||
                ...
                std::cerr<<"ALL_DONOR_MACRO_CONTROL_FAILED k="<<k
                         <<" b="<<b<<" depth="<<md<<"\n";
                return 13;
              }
              ++macro_controls;
  ...
  std::cout<<"VERIFIED_ALL_DONOR_REVERSE_MACRO_GRAMMAR\n";
  return 0;
}
