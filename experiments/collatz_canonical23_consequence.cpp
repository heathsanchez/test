...
int main(int argc,char**argv) {
  ...
  std::cout<<"CANONICAL23_START depth="<<DEPTH<<" ternary_budget="<<RB
  ...
  for (int depth=1; depth<=DEPTH; ++depth) {
    ...
    std::cout<<"CANONICAL23_LEVEL depth="<<depth
             <<" parents="<<live.size()
  ...
  std::cout<<"CANONICAL23_RESULT depth="<<DEPTH
           <<" ternary_budget="<<RB
           <<" reverse_chain=exhaustive_minimal_e01"
           <<" survivors="<<live.size()
  ...
  std::cout<<"FINAL_GATE=CANONICAL23_D"<<DEPTH<<"_R"<<RB<<"\n";
  return 0;
}
