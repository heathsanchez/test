...
static void consequence_refine(const State& s, int budget,
  ...
  const auto allpf=canonical_partial_flags(s);
  if(allpf.prefix || allpf.suffix){
    std::cerr<<"CANONICAL_SIGN_MIXED"
             <<" j="<<s.j<<" A="<<s.A<<" k="<<s.k
             <<" r="<<s128(s.r)<<" x="<<s128(s.x)
             <<" prefix="<<allpf.prefix<<" suffix="<<allpf.suffix<<"\n";
    std::exit(71);
  }
...
int main(int argc,char**argv) {
  ...
  std::cout<<"CANONICAL_SIGN_START depth="<<DEPTH<<" ternary_budget="<<RB
  ...
  for (int depth=1; depth<=DEPTH; ++depth) {
    ...
    std::cout<<"CANONICAL_SIGN_LEVEL depth="<<depth
             <<" parents="<<live.size()
  ...
  std::cout<<"CANONICAL_SIGN_RESULT depth="<<DEPTH
           <<" ternary_budget="<<RB
           <<" reverse_chain=exhaustive_minimal_e01"
           <<" survivors="<<live.size()
  ...
  std::cout<<"VERIFIED_CANONICAL_SIGN_CONSISTENCY\n";
  std::cout<<"FINAL_GATE=CANONICAL_SIGN_D"<<DEPTH<<"_R"<<RB<<"\n";
  return 0;
}
