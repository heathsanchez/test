...
static void classify_refined(const State&s,int RB,int KCAP,int MD,int EM,
  ...
  if(RB>0 && s.k<KCAP){
    ++st.ternary_splits;
    for(int e=0;e<3;++e)classify_refined(refine3(s,e),RB-1,KCAP,MD,EM,out,st);
    return;
  }
...
int main(int argc,char**argv){
  if(argc!=6){std::cerr<<"usage: capped_reverse DEPTH RB KCAP MD EM\n";return 2;}
  const int DEPTH=std::stoi(argv[1]),RB=std::stoi(argv[2]),KCAP=std::stoi(argv[3]);
  const int MD=std::stoi(argv[4]),EM=std::stoi(argv[5]);
  if(DEPTH<1||DEPTH>21||RB<0||RB>3||KCAP<0||KCAP>6||MD<0||MD>8||EM<0||EM>9)return 2;
  ...
  std::cout<<"CAPPED_REVERSE_START depth="<<DEPTH<<" rb="<<RB
           <<" kcap="<<KCAP<<" kcap="<<KCAP<<" macro_depth="<<MD<<" emax="<<EM<<"\n";
  ...
  for(int depth=1;depth<=DEPTH;++depth){
    ...
    for(const State&p:live){
      ...
      for(int a=1;a<tail;++a){
        ++children;
        classify_refined(extend_state(p,a),RB,KCAP,MD,EM,next,st);
      }
    }
    ...
    std::cout<<"CAPPED_REVERSE_LEVEL depth="<<depth
  ...
  std::cout<<"CAPPED_REVERSE_RESULT depth="<<DEPTH<<" rb="<<RB
  ...
  std::cout<<"FINAL_GATE=CAPPED_REVERSE_D"<<DEPTH<<"_R"<<RB
           <<"_K"<<KCAP<<"_M"<<MD<<"_E"<<EM<<"\n";
}
