...
struct Aff{ i128 A; i128 D; int extra; uint32_t bits; };
...
static bool deep_continue_closes(
    ...
    const std::vector<std::unordered_map<uint64_t,RevCert>>& bank,
    int max_o,const std::vector<uint64_t>& p3,
    RevCert& base,int& extra_steps,uint32_t& outbits,i128& outA,i128& outD){
  i128 A=0,D=0;
  ...
  for(int h=1;h<=H;++h){
    ...
    for(const Aff&s:cur){
      ...
      // Reverse E is always exact.
      {
        ...
        const uint32_t bits2=s.bits;
        if(lower_family(a2,d2,k,b)){
          extra_steps=h;outbits=bits2;outA=a2;outD=d2;return true;
        }
        if(can_ever_subcritical(a2))
          next.push_back({a2,d2,h,bits2});
      }
      ...
      if(s.A%3==0){
        ...
        if(z%3==0){
          ...
          const i128 d2=z/3;
          if(d2%2!=0){
            const uint32_t bits2=s.bits | (uint32_t(1)<<(h-1));
            if(lower_family(a2,d2,k,b)){
              extra_steps=h;outbits=bits2;outA=a2;outD=d2;return true;
            }
            if(can_ever_subcritical(a2))
...
int main(int argc,char**argv){
  ...
  std::map<std::string,uint64_t> program_shape_counts;
  std::map<std::string,uint64_t> exact_constructor_counts;
  ...
  for(int k=1;k<=K;++k){
    ...
    for(const State&s:cur){
      ...
      for(int e=0;e<2;++e){
        ...
        RevCert base{};int extra=0;uint32_t pathbits=0;i128 qA=0,qD=0;
        if(deep_continue_closes(b,c,d,k,H,bank,max_o,p3,base,extra,pathbits,qA,qD)){
          ++deep_cont;++total_deep_cont;++extra_hist[extra];

          auto word_string=[&](){
            std::string w;
            for(int t=0;t<extra;++t)
              w.push_back((pathbits&(uint32_t(1)<<t))?'O':'E');
            return w;
          };
          const std::string w=word_string();
          const std::string shape=
              std::to_string(base.o)+":"+
              std::to_string(base.steps)+":"+w;
          const std::string exact=
              shape+":"+
              std::to_string(base.C)+":"+
              std::to_string(base.residue);
          ++program_shape_counts[shape];
          ++exact_constructor_counts[exact];
  ...
  std::vector<std::pair<std::string,uint64_t>> shapes(
      program_shape_counts.begin(),program_shape_counts.end());
  std::sort(shapes.begin(),shapes.end(),
      [](const auto&a,const auto&b){
        if(a.second!=b.second)return a.second>b.second;
        return a.first<b.first;
      });
  std::cout<<"DEEP_PROGRAM_PROFILE"
           <<" unique_program_shapes="<<program_shape_counts.size()
           <<" unique_exact_constructors="<<exact_constructor_counts.size()
           <<" total_deep_closures="<<total_deep_cont<<"\n";
  const size_t lim=std::min<size_t>(100,shapes.size());
  for(size_t i=0;i<lim;++i)
    std::cout<<"DEEP_PROGRAM_SHAPE"
             <<" rank="<<(i+1)
