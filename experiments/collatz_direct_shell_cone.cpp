...
static bool maximal_cone(u128 y,u128 n,int& rused,u128& p){
  ...
  p=y;
  for(int j=0;j<r;++j){
    // Since p+1 is divisible by 3 at every valid inverse odd step,
    // divide first to avoid overflow: (2p-1)/3 = 2((p+1)/3)-1.
    if((p+1)%3!=0){std::cerr<<"CONE_DIVISIBILITY_BUG\n";std::exit(4);}
    const u128 q=(p+1)/3;
    p=2*q-1;
  }
  rused=r;
...
int main(int argc,char**argv){
  ...
  std::cout<<"DIRECT_SHELL_CONE_RESULT"
           <<" K="<<K<<" H="<<H
           <<" checked="<<checked
           <<" direct="<<direct
           <<" cone="<<cone
  ...
  std::cout<<"VERIFIED_DIRECT_SHELL_DETERMINISTIC_CONE\n";
  return 0;
}
