...
int main(int argc,char**argv){
  ...
  // Shell-specific theorem: for n=2^K+b we know the concrete future
  // m=T^K(n).  We no longer need a preselected q14 donor.  Search the exact
  // reverse tree of m directly.
  //
  // Any reverse word decomposes into an optional initial O^r run followed by
  // blocks E O^s.  The initial run is
  //
  //   O^r(m) = 2^r * ((m+1)/3^r) - 1,
  //
  // valid exactly while 3^r divides m+1.  shell_macro_close then searches the
  // subsequent E O^s blocks.  A witness p<n with T^t(p)=m proves n by strong
  // induction because T^K(n)=m.
  const u128 shellM=u128(1)<<K;
  uint64_t shell_direct=0,shell_reverse=0,shell_hard=0;
  ...
  for(const State&s:cur){
    ...
    if(m<n){
      ...
      if(shell_controls<20000){
        if(iterate(n,K)!=m){std::cerr<<"DIRECT_SHELL_FORWARD_REPLAY_FAIL\n";return 30;}
        ++shell_controls;
      }
      continue;
    }
    ...
    bool closed=false;
    const int r0max=v3_u128(m+1,64);
    u128 den=1,pow2=1;

    // r0=0: start macro grammar directly from m.
    {
      int blocks=0,steps=0;u128 witness=0;
      if(shell_macro_depth>0 &&
         shell_macro_close(m,n,shell_macro_depth,blocks,steps,witness)){
        ++shell_reverse;++shell_depth_hist[blocks];++initial_o_hist[0];
        if(shell_controls<20000){
          if(iterate(witness,steps)!=m){
            std::cerr<<"DIRECT_SHELL_MACRO_REPLAY_FAIL b="<<b<<" r0=0\n";
            return 31;
          }
          ++shell_controls;
        }
    ...
    // r0>=1: consume the maximal exact initial O-run prefixes, checking each
    // prefix directly and then continuing with E O^s blocks.
    for(int r0=1;!closed && r0<=r0max;++r0){
      den*=3;pow2*=2;
      const u128 x=pow2*(m+1)/den-1;
      if(x==0)continue;

      if(x<n){
        ++shell_reverse;++initial_o_hist[r0];
        if(shell_controls<20000){
          if(iterate(x,r0)!=m){
            std::cerr<<"DIRECT_SHELL_INITIAL_O_REPLAY_FAIL b="<<b
                     <<" r0="<<r0<<"\n";
            return 32;
          }
          ++shell_controls;
      ...
      if(shell_macro_depth>0 &&
         shell_macro_close(x,n,shell_macro_depth,blocks,extra,witness)){
        ++shell_reverse;++shell_depth_hist[blocks];++initial_o_hist[r0];
        if(shell_controls<20000){
          if(iterate(witness,r0+extra)!=m){
            std::cerr<<"DIRECT_SHELL_CONT_REPLAY_FAIL b="<<b
                     <<" r0="<<r0<<" blocks="<<blocks<<"\n";
            return 33;
          }
          ++shell_controls;
  ...
  std::cout<<"DIRECT_SHELL_RESULT"
           <<" K="<<K
           <<" macro_depth="<<shell_macro_depth
  ...
  for(const auto&[d,nc]:shell_depth_hist)
    std::cout<<"DIRECT_SHELL_MACRO_DEPTH blocks="<<d<<" closed="<<nc<<"\n";
  for(const auto&[r,nc]:initial_o_hist)
    std::cout<<"DIRECT_SHELL_INITIAL_O r="<<r<<" closed="<<nc<<"\n";
  ...
  std::cout<<"VERIFIED_DIRECT_SHELL_REVERSE_TREE_SCOUT\n";
  return 0;
}
