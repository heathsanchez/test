...
int main(int argc,char**argv){
  ...
  for(int depth=1;depth<=DEPTH;++depth){
    ...
    std::map<int,uint64_t> residualDC;
    std::map<int,uint64_t> residualMaxA;
    uint64_t fullHigh=0,lowStopped=0;
    for(const auto&z:next){
      const DualProfile dp=dual_profile(z);
      const bool isFull=(dp.L==z.j);
      const bool isLow=(z.max_a<=2);
      if(isFull && !isLow){
        ++fullHigh;
        ++residualDC[dp.C-z.A];
        ++residualMaxA[z.max_a];
      }
      if(!isFull && isLow)++lowStopped;
    }
    if(depth>=17){
      std::cout<<"DUALITY_RESIDUAL_CENSUS depth="<<depth
               <<" full_high="<<fullHigh
               <<" low_stopped="<<lowStopped
               <<" bad_balance="<<badBalance;
      for(const auto&kv:residualDC)
        std::cout<<" dC"<<kv.first<<"="<<kv.second;
      for(const auto&kv:residualMaxA)
        std::cout<<" maxA"<<kv.first<<"="<<kv.second;
      std::cout<<"\n";
    }
  ...
  std::cout<<"VERIFIED_DUALITY_RESIDUAL_CENSUS_D"<<DEPTH<<"\n";
  std::cout<<"FINAL_GATE=DUALITY_RESIDUAL_CENSUS_D"<<DEPTH<<"\n";
  return 0;
}
