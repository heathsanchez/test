...
static State extend_forward(const State&s,int a) {
  ...
  z.B=3*s.B+pow2A(s.A);
  z.word=s.word + (s.word.empty()?"":",") + std::to_string(a);
  verify_B(z);
  return z;
}
...
int main(int argc,char**argv){
  ...
  if(DEPTH<1||DEPTH>17||R!=0)return 2;
  ...
  for(int depth=1;depth<=DEPTH;++depth){
    ...
    for(const auto&z:next){
      ...
      if(isFull!=isLow){
        ++badFullLow;
        std::cout<<"DUALITY_COUNTEREXAMPLE kind=full_low"
                 <<" depth="<<depth
                 <<" word="<<z.word
                 <<" A="<<z.A
                 <<" B="<<s128(z.B)
      ...
      if(isFull && dp.C!=z.A){
        ++badBalance;
        std::cout<<"DUALITY_COUNTEREXAMPLE kind=balance"
                 <<" depth="<<depth
                 <<" word="<<z.word
                 <<" A="<<z.A
                 <<" B="<<s128(z.B)
    ...
    if(badFullLow||badBalance){
      std::cerr<<"DUALITY_INVARIANT_FAIL depth="<<depth
  ...
  std::cout<<"VERIFIED_LOW12_FULL_REVERSE_DUALITY\n";
  std::cout<<"FINAL_GATE=FORWARD_REVERSE_DUALITY_D"<<DEPTH<<"_R"<<R<<"\n";
  return 0;
}
