...
int main(int argc,char**argv){
  if(argc!=4){
    std::cerr<<"usage: minimal_reverse DEPTH CHAIN_DEPTH EMAX01\n";return 2;
  }
  ...
  std::cout<<"GLOBAL_MINREVERSE_START depth="<<DEPTH
  ...
  for(int depth=1;depth<=DEPTH;++depth){
    ...
    std::cout<<"GLOBAL_MINREVERSE_LEVEL depth="<<depth
             <<" parents="<<live.size()
             <<" children="<<children
             <<" tails="<<tails
  ...
  std::cout<<"GLOBAL_MINREVERSE_RESULT depth="<<DEPTH
           <<" macro_depth="<<MD
           <<" emax="<<EM
           <<" survivors="<<live.size()
  ...
  std::cout<<"FINAL_GATE=GLOBAL_MINREVERSE_D"<<DEPTH
           <<"_M"<<MD<<"_E"<<EM<<"\n";
  return 0;
}
