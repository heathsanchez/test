...
struct RevAffine{u128 A,D;};

static bool reverse_macro_closes(const State&s,int maxDepth,int& usedDepth){
  const u128 Ncoef=u128(1)<<(s.A+1);
  const u128 Nconst=s.r;
  std::vector<RevAffine> cur,next;
  cur.push_back({2*p3(s.j),s.x});
  ...
  for(int depth=1;depth<=maxDepth;++depth){
    ...
    for(const auto&t:cur){
      ...
      u128 z=2*t.D+1;
      int rd=v3(z,64);
      int rmax=std::min(ra,rd);
      for(int r=0;r<=rmax;++r){
        u128 A,D;
        if(r==0){
          A=2*t.A;D=2*t.D;
        }else{
          u128 den=p3(r),pow2=u128(1)<<r;
          A=(2*pow2*t.A)/den;
          D=pow2*(z/den)-1;
        }
        if(D>0 && A<Ncoef && D<Nconst){
          usedDepth=depth;return true;
        }
        // Retain exact positive affine predecessor states.  A bounded macro
        // search is a sound under-approximation: failure means UNKNOWN.
        if(A>0)next.push_back({A,D});
      }
    }
    std::sort(next.begin(),next.end(),[](const RevAffine&a,const RevAffine&b){
      if(a.A!=b.A)return a.A<b.A;return a.D<b.D;
    });
    next.erase(std::unique(next.begin(),next.end(),[](const RevAffine&a,const RevAffine&b){
...
int main(int argc,char**argv){
  ...
  uint64_t totalClosed=0,totalPartial=0,totalTails=0,totalMacro=0;
  std::cout<<"GLOBAL_REVMACRO_START depth="<<DEPTH<<"\n";
  for(int depth=1;depth<=DEPTH;++depth){
    std::vector<State> next;
    uint64_t closed=0,partial=0,tails=0,children=0,macroClosed=0;
    uint64_t o1=0,o2p=0;
    ...
    for(const State&parent:live){
      ...
      for(int a=1;a<tail;++a){
        ...
        int md=0;
        if(reverse_macro_closes(z,3,md)){
          ++closed;++totalClosed;++macroClosed;++totalMacro;continue;
        }
    ...
    std::cout<<"GLOBAL_REVMACRO_LEVEL depth="<<depth
             <<" parents="<<live.size()
             <<" explicit_children="<<children
             <<" infinite_direct_tails="<<tails
             <<" closed="<<closed
             <<" macro_closed="<<macroClosed
             <<" partial="<<partial
             <<" survivors="<<next.size()
  ...
  std::cout<<"GLOBAL_REVMACRO_RESULT depth="<<DEPTH
           <<" survivors="<<live.size()
           <<" density="<<(double)density(live)
           <<" total_closed="<<totalClosed
           <<" total_macro_closed="<<totalMacro
           <<" total_partial="<<totalPartial
           <<" infinite_direct_tails="<<totalTails<<"\n";
  if(totalPartial==0)std::cout<<"REVMACRO_NO_PARTIAL_CYLINDERS\n";
  else std::cout<<"REVMACRO_PARTIAL_CYLINDERS_RETAINED\n";
  std::cout<<"FINAL_GATE=GLOBAL_REVERSE_MACRO_CYLINDER_DEPTH_"<<DEPTH<<"\n";
}
