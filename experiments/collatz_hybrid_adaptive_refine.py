#!/usr/bin/env python3
"""
Adaptive exact Collatz residue refiner.

Build the theorem-strengthened H-step CNF once for one lawful low-bit parent.
Probe finer lawful residue classes with a MiniSat-like incremental solver under
assumptions. Each probe is conflict-budgeted:

  SAT   -> exact surviving witness
  UNSAT -> exact closed subregion
  None  -> no verdict; refine that residue to finer lawful children

Thus resource exhaustion changes grain instead of becoming an answer. Learned
clauses are retained across sibling probes.

The live-domain equivalence is inherited from collatz_hybrid_adaptive:
below FIRST_DANGEROUS, no-descent implies coefficient persistence, so prefixes
that fail the density condition are impossible and may be omitted exactly.
"""
import argparse
import json
import time
...
def main():
    ...
    ap.add_argument("--max-bits", type=int, default=20)
    ...
    ap.add_argument("--out", default="adaptive-refine.json")
    A = ap.parse_args()
    ...
    with Glucose4() as solver:
        ...
        print(
            f"ADAPTIVE_REFINE_EXACT parent={A.parent_value}/2^{A.parent_bits} "
        ...
        # Stack items are (residue value, residue bits). Seed it with the
        # lawful children at the first requested refinement depth.
        stack = [
            (v, A.start_bits)
    ...
    summary = {
        "kind": "exact_budgeted_adaptive_residue_refinement",
        "scope": scope,
        "bits": K,
        "lower": str(A.lower),
        "upper": str(A.upper),
