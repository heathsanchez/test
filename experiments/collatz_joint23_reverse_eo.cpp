...
static void consequence_refine(const State& s, int budget,
  ...
  if (budget>0 && c.uniform_o==c.x_v3) {
    ++st.splits;
    for (int e=0;e<3;++e)
      consequence_refine(refine3(s,e),budget-1,macroDepth,emax,out,st);
    return;
  }
  out.push_back(s);
  ++st.leaves;
...
int main(int argc,char**argv) {
  if (argc!=5) {
    std::cerr<<"usage: joint_reverse DEPTH TERNARY_BUDGET MACRO_DEPTH EMAX\n";
    return 2;
  }
  ...
  std::cout<<"JOINT23_REVERSE_START depth="<<DEPTH<<" ternary_budget="<<RB
  ...
  for (int depth=1; depth<=DEPTH; ++depth) {
    ...
    std::cout<<"JOINT23_REVERSE_LEVEL depth="<<depth
             <<" parents="<<live.size()
  ...
  std::cout<<"JOINT23_REVERSE_RESULT depth="<<DEPTH
           <<" ternary_budget="<<RB
           <<" macro_depth="<<MD
           <<" emax="<<EM
           <<" survivors="<<live.size()
  ...
  std::cout<<"FINAL_GATE=JOINT23_REVERSE_D"<<DEPTH<<"_R"<<RB
           <<"_M"<<MD<<"_E"<<EM<<"\n";
  return 0;
}
