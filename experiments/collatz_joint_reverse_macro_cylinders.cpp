...
int main(int argc,char**argv){
  ...
  std::cout<<"JOINT_REVERSE_START depth="<<DEPTH<<" rb="<<RB
  ...
  for(int depth=1;depth<=DEPTH;++depth){
    ...
    std::cout<<"JOINT_REVERSE_LEVEL depth="<<depth
  ...
  std::cout<<"JOINT_REVERSE_RESULT depth="<<DEPTH<<" rb="<<RB
           <<" macro_depth="<<MD<<" emax="<<EM
  ...
  std::cout<<"FINAL_GATE=JOINT_REVERSE_D"<<DEPTH<<"_R"<<RB
           <<"_M"<<MD<<"_E"<<EM<<"\n";
}
