...
static bool macro_fixpoint_close(
    uint64_t b,int k,
    ...
    i128& outA,i128& outD){
  struct Node{i128 A; i128 D; int blocks; int reverse_steps;};
  const i128 M=i128(1)<<k;

  std::vector<Node> stack;
  stack.push_back({startA,startD,0,0});
  std::set<std::pair<i128,i128>> seen;

  while(!stack.empty()){
    const Node s=stack.back();
    stack.pop_back();

    if(!seen.emplace(s.A,s.D).second)continue;
    ++explored_states;

    // Exact impossibility bound.  Write A=2^p 3^q where q=v3(A).
    // Any nonempty future macro sequence with total R O-steps and m blocks
    // has coefficient A*2^(R+m)/3^R, with 0<=R<=q and m>=1.
    // This is minimized by R=q,m=1:
    //      A_min=(A/3^q)*2^(q+1).
    // If A_min>=2^k, no descendant can ever be coefficient-subcritical.
    const int q=v3_i128(s.A,64);
    i128 p3q=1,p2q1=2;
    for(int j=0;j<q;++j){p3q*=3;p2q1*=2;}
    const i128 amin=(s.A/p3q)*p2q1;
    if(s.A>=M && amin>=M)continue;

    const int vd=v3_i128(2*s.D+1,64);
    const int rmax=std::min(q,vd);
    ...
    for(int r=0;r<=rmax;++r){
      ...
      if(A>0 && A+D>0 && L<0 && L<R){
        used_blocks=s.blocks+1;
        used_reverse_steps=s.reverse_steps+add_steps;
        outA=A;outD=D;
        return true;
      }

      // Push only if a rigorous coefficient lower bound leaves some chance.
      const int cq=v3_i128(A,64);
      i128 cp3=1,cp2=2;
      for(int j=0;j<cq;++j){cp3*=3;cp2*=2;}
      const i128 child_min=(A/cp3)*cp2;
      if(A<M || child_min<M)
        stack.push_back({A,D,s.blocks+1,s.reverse_steps+add_steps});
    }
  }
  return false;
}
...
int main(int argc,char**argv){
  if(argc!=3){std::cerr<<"usage: macro_fixpoint K BANK\n";return 2;}
  const int K=std::stoi(argv[1]);
  const std::string bankpath=argv[2];
  if(K<1||K>24){
    std::cerr<<"K_OUT_OF_FIXPOINT_SCOUT_RANGE\n";return 2;
  }
  ...
  uint64_t total_coalesce=0,total_descent=0,total_bridge=0,total_deep_o=0,total_macro=0;
  uint64_t bridge_controls=0,deep_o_controls=0,macro_controls=0;
  std::map<int,uint64_t> macro_depth_hist;
  uint64_t total_macro_search_states=0;
  uint64_t max_macro_search_states=0;
  ...
  for(int k=1;k<=K;++k){
    ...
    // Generate every child of the currently unresolved prefixes first.
    // Coalescence is goal-relative within this exact live frontier: children
    // with identical (c,d) have identical T^k affine futures, so after sorting
    // by b the later/larger residue is closed by the earlier/smaller one.
    raw.clear();
    raw.reserve(cur.size()*2);
    for(const State&s:cur){
      ...
      for(int e=0;e<2;++e){
        ...
        int cc=c0;uint64_t dd;
        if(y&1){
          ++cc;
          if(y>(UINT64_MAX-1)/3){std::cerr<<"CHILD_D_OVERFLOW\n";return 9;}
          dd=(3*y+1)/2;
        }else dd=y/2;
        raw.push_back(make_state(b,dd,cc));
      }
    }
    std::sort(raw.begin(),raw.end(),[](const State&a,const State&b){
      return a.b<b.b;
    });

    next.clear();
    next.reserve(raw.size());
    std::unordered_set<uint64_t> seen_affine;
    seen_affine.reserve(raw.size()*2+1);

    uint64_t coalesce=0,descent=0,bridge=0,deep_o=0,macro=0;

    for(const State&cand:raw){
      const uint64_t b=cand.b,d=state_d(cand);
      const int cc=state_c(cand);
      const uint64_t key=cand.dc;

      if(!seen_affine.insert(key).second){
        ++coalesce;++total_coalesce;
        continue;
      }

      const i128 L=i128(p3[cc])-i128(M);
      const i128 R=i128(b)-i128(d);
      if(L<0&&L<R){++descent;++total_descent;continue;}

      RevCert z{};i128 pc=0,pk=0;
      if(bridge_closes(b,cc,d,k,bank,max_o,p3,z,pc,pk)){
        ++bridge;++total_bridge;
        if(bridge_controls<20000){
          for(uint64_t a:{1ULL,3ULL}){
            const u128 n=u128(a)*M+b;
            const u128 m=u128(a)*p3[cc]+d;
      ...
      if(deep_o_closes(b,cc,d,k,p3,oj,opc,opk)){
        ++deep_o;++total_deep_o;
      ...
      bool have_base=false;
      {
        ...
        for(int o=1;o<=top;++o){
          ...
          mD=nc/i128(mod);
          mA=(i128(1)<<mb.steps)*i128(p3[cc-mb.o]);
          have_base=true;
          break;
        }
      }
      ...
      uint64_t explored=0;
      if(have_base && macro_fixpoint_close(
            b,k,mA,mD,md,msteps,explored,qA,qD)){
        total_macro_search_states+=explored;
        max_macro_search_states=std::max(max_macro_search_states,explored);
    ...
    std::cout<<"COALESCENT_FIXPOINT_LEVEL"
             <<" k="<<k
             <<" parents="<<cur.size()
             <<" coalesce="<<coalesce
             <<" descent="<<descent
  ...
  std::cout<<"MACRO_FIXPOINT_PROFILE"
           <<" K="<<K
  ...
  std::cout<<"COALESCENT_FIXPOINT_DONE K="<<K
           <<" final_live="<<cur.size()
           <<" total_coalesce="<<total_coalesce
  ...
  for(const auto& [d,n]:macro_depth_hist)
    std::cout<<"MACRO_FIXPOINT_DEPTH blocks="<<d
             <<" closed="<<n<<"\n";
  std::cout<<"MACRO_FIXPOINT_SEARCH"
           <<" total_states="<<total_macro_search_states
           <<" max_states_per_family="<<max_macro_search_states<<"\n";
  std::cout<<"VERIFIED_COALESCENT_EXHAUSTIVE_MACRO_FIXPOINT\n";
  return 0;
}
