...
static bool macro_fixpoint_close(
    ...
    u128& best_finite_cutoff,
    i128& outA,i128& outD){
  struct Node{i128 A; i128 D; int blocks; int reverse_steps;};
  const i128 M=i128(1)<<k;
  best_finite_cutoff=~u128(0);

  auto record_tail=[&](i128 A,i128 D){
    if(A<=0 || A>=M)return;
    const i128 gap=M-A;

    u128 need_lower=1;
    const i128 excess=D-i128(b);
    if(excess>=0) need_lower=u128(excess/gap)+1;

    u128 need_positive=1;
    if(D<0) need_positive=u128((-D)/A)+1;

    const u128 need=std::max(need_lower,need_positive);
    if(need>1) best_finite_cutoff=std::min(best_finite_cutoff,need-1);
  };

  record_tail(startA,startD);
...
int main(int argc,char**argv){
  ...
  uint64_t macro_tail_families=0,macro_hard_families=0;
  u128 total_macro_tail_exceptions=0,max_macro_tail_cutoff=0;
  ...
  for(int k=1;k<=K;++k){
    ...
    for(const State&s:cur){
      ...
      for(int e=0;e<2;++e){
        ...
        uint64_t explored=0;
        u128 finite_cutoff=~u128(0);
        if(have_base && macro_fixpoint_close(
              b,k,mA,mD,md,msteps,explored,finite_cutoff,qA,qD)){
        ...
        if(have_base){
          ...
          max_macro_search_states=std::max(max_macro_search_states,explored);
          if(k==K){
            if(finite_cutoff!=~u128(0)){
              ++macro_tail_families;
              total_macro_tail_exceptions+=finite_cutoff;
              max_macro_tail_cutoff=std::max(max_macro_tail_cutoff,finite_cutoff);
            }else ++macro_hard_families;
          }
        }else if(k==K){
          ++macro_hard_families;
        }
  ...
  std::cout<<"MACRO_FIXPOINT_TAILS"
           <<" K="<<K
           <<" live="<<cur.size()
           <<" finite_tail_families="<<macro_tail_families
           <<" hard_families="<<macro_hard_families
           <<" total_finite_exceptions="<<u128s(total_macro_tail_exceptions)
           <<" max_exception_a="<<u128s(max_macro_tail_cutoff)<<"\n";
  ...
  std::cout<<"VERIFIED_MACRO_FIXPOINT_TAIL_CLASSIFICATION\n";
  return 0;
}
