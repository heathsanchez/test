...
int main(int argc,char**argv){
  if(argc!=5){std::cerr<<"usage: macro_persistence K BANK MACRO_DEPTH CHECKPOINT\n";return 2;}
  const int K=std::stoi(argv[1]);
  const std::string bankpath=argv[2];
  const int macro_depth=std::stoi(argv[3]);
  const int checkpoint=std::stoi(argv[4]);
  if(K<1||K>34||macro_depth<1||macro_depth>5||checkpoint<1||checkpoint>K){
  ...
  for(int k=1;k<=K;++k){
    ...
    if(k>=checkpoint){
      const uint64_t mask=(UINT64_C(1)<<checkpoint)-1;
      std::unordered_map<uint64_t,uint32_t> desc;
      desc.reserve(std::min<size_t>(cur.size(),size_t(1)<<20));
      for(const State& z:cur)++desc[z.b&mask];

      if(k==checkpoint)checkpoint_live=cur.size();
      uint64_t singletons=0,max_desc=0,min_desc=UINT64_MAX;
      for(const auto& kv:desc){
        const uint64_t n=kv.second;
        if(n==1)++singletons;
        max_desc=std::max(max_desc,n);
        min_desc=std::min(min_desc,n);
      }
      if(desc.empty())min_desc=0;
      const uint64_t surviving=desc.size();
      const uint64_t extinct=checkpoint_live-surviving;
      std::cout<<"PERSISTENCE_LEVEL"
               <<" checkpoint="<<checkpoint
               <<" k="<<k
               <<" live="<<cur.size()
               <<" surviving_ancestors="<<surviving
               <<" extinct_ancestors="<<extinct
               <<" singleton_ancestors="<<singletons
               <<" min_live_descendants="<<min_desc
               <<" max_live_descendants="<<max_desc<<"\n";
    }
  }

  std::cout<<"PERSISTENCE_DONE checkpoint="<<checkpoint
           <<" checkpoint_live="<<checkpoint_live
           <<" final_k="<<K<<"\n";
  ...
  std::cout<<"VERIFIED_MACRO_RESIDUAL_PERSISTENCE_TREE\n";
  return 0;
}
