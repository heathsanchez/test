...
int main(int argc,char**argv) {
  if (argc!=5) {
    std::cerr<<"usage: multidigit_reverse DEPTH TERNARY_BUDGET MACRO_DEPTH EMAX\n";
    return 2;
  }
  ...
  std::cout<<"MULTIDIGIT23_REVERSE_START depth="<<DEPTH<<" ternary_budget="<<RB
  ...
  for (int depth=1; depth<=DEPTH; ++depth) {
    ...
    std::cout<<"MULTIDIGIT23_REVERSE_LEVEL depth="<<depth
             <<" parents="<<live.size()
  ...
  std::cout<<"MULTIDIGIT23_REVERSE_RESULT depth="<<DEPTH
           <<" ternary_budget="<<RB
           <<" macro_depth="<<MD
           <<" emax="<<EM
           <<" survivors="<<live.size()
  ...
  std::cout<<"FINAL_GATE=MULTIDIGIT23_REVERSE_D"<<DEPTH<<"_R"<<RB
           <<"_M"<<MD<<"_E"<<EM<<"\n";
  return 0;
}
