...
int main(int argc,char**argv){
  ...
  // Deterministic shell cone reduction.
  //
  // For n=2^K+b and y=T^(K+f)(n), define r=v_3(y+1).
  // If r>0 then
  //
  //   P(y) = 2^r * (y+1)/3^r - 1
  //
  // satisfies T^r(P(y))=y.  Among all valid pure-O predecessors
  // corresponding to 1<=j<=r, this maximal-r predecessor is smallest,
  // because each extra inverse odd step multiplies y+1 by 2/3.
  //
  // Hence no reverse branching is needed: at each forward context f it is
  // sufficient to test y<n or P(y)<n.
  const Big shellM=Big(1)<<K;
  uint64_t shell_forward=0,shell_cone=0,shell_hard=0;
  uint64_t shell_controls=0;
  std::map<int,uint64_t> forward_hit_hist;
  std::map<int,uint64_t> v3_hit_hist;
  ...
  for(const State&s:cur){
    ...
    for(int f=0;f<=forward_h && !closed;++f){
      ...
      if(y<n){
        ...
        if(shell_controls<20000){
          if(iterate_big(n,K+f)!=y){
            std::cerr<<"CONE_FORWARD_REPLAY_FAIL b="<<b<<" f="<<f<<"\n";
            return 30;
          }
          ++shell_controls;
        }
      ...
      const int rfull=v3_big(y+1);
      const int r=(rfull>0)?1:0;
      if(r>0){
        Big den=1,pow2=1;
        for(int j=0;j<r;++j){den*=3;pow2*=2;}
        const Big p=pow2*(y+1)/den-1;
        if(p>0 && p<n){
          ++shell_cone;++forward_hit_hist[f];++v3_hit_hist[r];
          if(shell_controls<20000){
            if(iterate_big(p,r)!=y || iterate_big(n,K+f)!=y){
              std::cerr<<"CONE_REPLAY_FAIL b="<<b<<" f="<<f<<" r="<<r<<"\n";
              return 31;
            }
            ++shell_controls;
          }
  ...
  std::cout<<"ONE_STEP_CONE_RESULT"
           <<" K="<<K
           <<" forward_h="<<forward_h
           <<" universal_residual="<<cur.size()
           <<" shell_forward="<<shell_forward
           <<" shell_cone="<<shell_cone
           <<" shell_hard="<<shell_hard
           <<" controls="<<shell_controls<<"\n";
  for(const auto&[f,nc]:forward_hit_hist)
    std::cout<<"CONE_FORWARD_HIT f="<<f<<" closed="<<nc<<"\n";
  for(const auto&[r,nc]:v3_hit_hist)
    std::cout<<"CONE_V3_HIT r="<<r<<" closed="<<nc<<"\n";
  ...
  std::cout<<"VERIFIED_DETERMINISTIC_ONE_STEP_O_CONE\n";
  return 0;
}
