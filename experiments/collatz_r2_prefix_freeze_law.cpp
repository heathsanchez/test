...
static State refine3(const State&s,int e) {
  ...
  State z{s.r+N*u128(e),s.x+X*u128(e),s.j,s.A,s.k+1,
          s.last_a,s.max_a,s.B,s.lowPrefix,s.lowOnly};
  verify_B(z);
  return z;
}
...
static State extend_forward(const State&s,int a) {
  ...
  z.B=3*s.B+pow2A(s.A);
  z.lowOnly=s.lowOnly && a<=2;
  z.lowPrefix=z.lowOnly ? z.j : s.lowPrefix;
  verify_B(z);
  return z;
}
...
static uint32_t ternary_shape_signature(
  ...
  std::function<uint32_t(int)> sig=[&](int u)->uint32_t{
    ...
    // Forget ternary digit labels. This quotients by the full rooted-tree
    // child-permutation group, a STRICTLY coarser relation than affine
    // 3-adic equivalence. Therefore lack of compression here would rule out
    // affine relabeling as an explanation; compression is only a scout.
...
struct ReverseProfile {
  int L=0;
  int C=0;
};

static ReverseProfile reverse_profile_root(const State& st) {
  const u128 N0=u128(1)<<(st.A+1);
  const u128 r0=st.r%N0;
  if(r0==0){std::cerr<<"PROFILE_ROOT_ZERO\n";std::exit(50);}
  const u128 num=P3.at(st.j)*r0+st.B;
  const u128 den=u128(1)<<st.A;
  if(num%den){std::cerr<<"PROFILE_X_DIV_FAIL\n";std::exit(51);}
  State root{r0,num/den,st.j,st.A,0,st.last_a,st.max_a,st.B,
             st.lowPrefix,st.lowOnly};
  verify_B(root);
  RevAff z{Xcoef(root),root.x};
  ReverseProfile p;
  for(int depth=1;depth<=root.j;++depth){
    if(z.a%3!=0)break;
    int e=-1;
    if(z.d%3==2)e=0;
    else if(z.d%3==1)e=1;
...
int main(int argc,char**argv){
  ...
  if(DEPTH<1||DEPTH>16||R!=2)return 2;

  P3.resize(128);P3[0]=1;
  for(int i=1;i<(int)P3.size();++i)P3[i]=P3[i-1]*3;

  // At depth zero B = 1*3 - 1*3 = 0.
  std::vector<State>live{{3,3,0,0,0,0,0,0,0,true}};
  verify_B(live[0]);

  std::map<std::array<uint32_t,3>,uint32_t> globalShapeIntern;
  uint32_t globalNextShape=2;
  std::map<int,uint32_t> frozenPrefixShape;
  uint64_t totalPredictionErrors=0,totalPrefixConflicts=0,totalShapeConflicts=0;
  ...
  for(int depth=1;depth<=DEPTH;++depth){
    ...
    auto& shapeIntern=globalShapeIntern;
    auto& nextShape=globalNextShape; // global IDs make cross-depth equality exact
    std::map<uint32_t,uint64_t> rootFreq;
    std::map<uint32_t,std::map<std::pair<int,int>,uint64_t>> shapeProfiles;
    std::map<std::pair<int,int>,uint64_t> profileFreq;
    uint64_t trieNodes=0;
    int trieMaxDepth=0;
    uint64_t fullBalanced=0, fullUnbalanced=0;
    std::map<int,uint32_t> levelPrefixShape;
    std::map<uint32_t,int> levelShapePrefix;
    uint64_t predictionErrors=0,prefixConflicts=0,shapeConflicts=0;
    for(auto&kv:groups){
      const uint32_t root=ternary_shape_signature(
          kv.second,shapeIntern,nextShape,trieNodes,trieMaxDepth);
      ++rootFreq[root];

      const int prefix=kv.second.front()->lowPrefix;
      const bool lowOnly=kv.second.front()->lowOnly;
      for(const State* sp:kv.second)
        if(sp->lowPrefix!=prefix || sp->lowOnly!=lowOnly){
          std::cerr<<"AB_GROUP_HISTORY_MISMATCH\n";return 71;
        }

      auto pi=levelPrefixShape.find(prefix);
      if(pi==levelPrefixShape.end())levelPrefixShape[prefix]=root;
      else if(pi->second!=root)++prefixConflicts;
      auto si=levelShapePrefix.find(root);
      if(si==levelShapePrefix.end())levelShapePrefix[root]=prefix;
      else if(si->second!=prefix)++shapeConflicts;

      // At depth p the all-low p-prefix develops its final open-core shape.
      // Once the first a>=3 occurs, lowPrefix stays p; the freeze law predicts
      // that exact unlabeled consequence shape will never change thereafter.
      if(prefix==depth && lowOnly){
        auto fi=frozenPrefixShape.find(prefix);
        if(fi==frozenPrefixShape.end())frozenPrefixShape[prefix]=root;
        else if(fi->second!=root)++predictionErrors;
      } else if(prefix<depth){
        auto fi=frozenPrefixShape.find(prefix);
        if(fi==frozenPrefixShape.end() || fi->second!=root)++predictionErrors;
      }

      const ReverseProfile rp=reverse_profile_root(*kv.second.front());
      ++shapeProfiles[root][{rp.L,rp.C-kv.first.A}];
      ++profileFreq[{rp.L,rp.C-kv.first.A}];
      if(rp.L==depth){
        if(rp.C==kv.first.A)++fullBalanced;
        else ++fullUnbalanced;
      }
    }
    totalPredictionErrors+=predictionErrors;
    totalPrefixConflicts+=prefixConflicts;
    totalShapeConflicts+=shapeConflicts;
    uint64_t largestRootClass=0;
    for(const auto&kv:rootFreq)largestRootClass=std::max(largestRootClass,kv.second);
    std::cout<<"R2_FREEZE_LAW_LEVEL depth="<<depth
             <<" R="<<R
    ...
    if(depth==DEPTH){
      for(const auto&kv:rootFreq){
        std::cout<<"ROOT_SHAPE_PROFILE shape="<<kv.first
                 <<" size="<<kv.second;
        for(const auto&pv:shapeProfiles[kv.first])
          std::cout<<" L"<<pv.first.first<<"_dC"<<pv.first.second<<"="<<pv.second;
        std::cout<<"\n";
      }
      std::cout<<"REVERSE_PROFILE_CENSUS";
      for(const auto&pv:profileFreq)
        std::cout<<" L"<<pv.first.first<<"_dC"<<pv.first.second<<"="<<pv.second;
      std::cout<<"\n";
    }
