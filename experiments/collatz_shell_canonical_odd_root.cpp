...
int main(int argc,char**argv){
  ...
  // Canonical odd-root contraction.
  //
  // For any positive y, let r=v3(y+1) and
  //
  //   C(y) = 2^r * ((y+1)/3^r) - 1.
  //
  // Then T^r(C(y))=y.  Among the pure reverse-O chain predecessors of y,
  // C(y) is the smallest because each additional valid O multiplies p+1
  // by 2/3.  The r=0 case is C(y)=y, so direct descent and reverse
  // odd-chain descent are one deterministic test:
  //
  //   C(T^(K+f)(n)) < n.
  //
  // No reverse search tree is required.
  const Big shellM=Big(1)<<K;
  uint64_t shell_closed=0,shell_hard=0;
  uint64_t shell_controls=0;
  std::map<int,uint64_t> forward_hit_hist;
  std::map<int,uint64_t> odd_root_r_hist;
  int max_hit_f=-1,max_hit_r=0;
  ...
  for(const State&s:cur){
    ...
    for(int f=0;f<=forward_h && !closed;++f){
      ...
      const Big yp1=y+1;
      const int r=v3_big(yp1);
      Big q=yp1;
      for(int j=0;j<r;++j)q/=3;
      Big p=q;
      p <<= r;
      p-=1;
      ...
      if(p>0 && p<n){
        ...
        ++forward_hit_hist[f];
        ++odd_root_r_hist[r];
        max_hit_f=std::max(max_hit_f,f);
        max_hit_r=std::max(max_hit_r,r);

        if(shell_controls<20000){
          if(iterate_big(p,r)!=y || iterate_big(n,K+f)!=y){
            std::cerr<<"CANONICAL_ODD_ROOT_REPLAY_FAIL"
                     <<" K="<<K<<" b="<<b<<" f="<<f<<" r="<<r<<"\n";
            return 30;
          }
          ++shell_controls;
  ...
  std::cout<<"CANONICAL_ODD_ROOT_RESULT"
           <<" K="<<K
           <<" forward_h="<<forward_h
  ...
  for(const auto&[f,nc]:forward_hit_hist)
    std::cout<<"CANONICAL_ODD_ROOT_F f="<<f<<" closed="<<nc<<"\n";
  for(const auto&[r,nc]:odd_root_r_hist)
    std::cout<<"CANONICAL_ODD_ROOT_R r="<<r<<" closed="<<nc<<"\n";
  ...
  std::cout<<"VERIFIED_CANONICAL_ODD_ROOT_CONTRACTION\n";
  return 0;
}
