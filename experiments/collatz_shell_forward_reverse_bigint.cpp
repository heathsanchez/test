...
static int v3_big(Big x,int cap=4096){
  if(x<0)x=-x;
  int v=0;
  while(v<cap && x%3==0){x/=3;++v;}
  return v;
...
// Exact shell-specific reverse macro search with unbounded integers.
static bool shell_macro_close_big(
    const Big& start,const Big& n,int max_blocks,
    int& used_blocks,int& used_reverse_steps,
    Big& witness){
  struct Node{Big x;int blocks;int steps;};
  ...
  for(int depth=1;depth<=max_blocks;++depth){
    ...
    for(const Node&s:cur){
      ...
      const Big z=2*s.x+1;
      const int rmax=v3_big(z);
      Big den=1,pow2=1;
      for(int r=1;r<=rmax;++r){
        den*=3;pow2*=2;
        const Big x2=pow2*z/den-1;
        if(x2==0)continue;
        if(x2<n){
          used_blocks=depth;
...
int main(int argc,char**argv){
  ...
  if(K<1||K>30||shell_macro_depth<0||shell_macro_depth>20||forward_h<0||forward_h>2048){
  ...
  // Shell-specific forward/reverse coalescence with arbitrary precision.
  //
  // This second-generation implementation removes any fixed-width overflow
  // ambiguity while allowing large forward horizons.  The universal residual
  // remains the exact B24 affine frontier; only the concrete shell witnesses
  // are promoted to cpp_int.
  ...
  for(const State&s:cur){
    ...
    const Big n=shellM+b;
    const Big m=Big(p3[cc])+dd;
    ...
    for(int f=0;f<=forward_h && !closed;++f){
      ...
      if(y<n){
        ...
        if(shell_controls<20000){
          if(iterate_big(n,K+f)!=y){
            std::cerr<<"BIG_SHELL_FORWARD_REPLAY_FAIL b="<<b<<" f="<<f<<"\n";
            return 30;
          }
          ++shell_controls;
        }
      ...
      const int r0max=v3_big(y+1);
      Big den=1,pow2=1;

      if(shell_macro_depth>0){
        int blocks=0,steps=0;Big witness=0;
        if(shell_macro_close_big(y,n,shell_macro_depth,blocks,steps,witness)){
          ++shell_reverse;++forward_hit_hist[f];++reverse_depth_hist[blocks];
          ++initial_o_hist[0];
          if(shell_controls<20000){
            if(iterate_big(witness,steps)!=y || iterate_big(n,K+f)!=y){
              std::cerr<<"BIG_SHELL_FORWARD_REVERSE_REPLAY_FAIL b="<<b
                       <<" f="<<f<<" r0=0\n";
              return 31;
            }
            ++shell_controls;
      ...
      for(int r0=1;!closed && r0<=r0max;++r0){
        den*=3;pow2*=2;
        const Big x=pow2*(y+1)/den-1;
        if(x==0)continue;

        if(x<n){
          ++shell_reverse;++forward_hit_hist[f];++initial_o_hist[r0];
          if(shell_controls<20000){
            if(iterate_big(x,r0)!=y || iterate_big(n,K+f)!=y){
              std::cerr<<"BIG_SHELL_INITIAL_O_REPLAY_FAIL b="<<b
                       <<" f="<<f<<" r0="<<r0<<"\n";
              return 32;
            }
            ++shell_controls;
        ...
        if(shell_macro_depth>0){
          int blocks=0,extra=0;Big witness=0;
          if(shell_macro_close_big(x,n,shell_macro_depth,blocks,extra,witness)){
            ++shell_reverse;++forward_hit_hist[f];++reverse_depth_hist[blocks];
            ++initial_o_hist[r0];
            if(shell_controls<20000){
              if(iterate_big(witness,r0+extra)!=y || iterate_big(n,K+f)!=y){
                std::cerr<<"BIG_SHELL_LOOKAHEAD_CONT_REPLAY_FAIL b="<<b
  ...
  std::cout<<"BIG_SHELL_FORWARD_REVERSE_RESULT"
           <<" K="<<K
  ...
  for(const auto&[f,nc]:forward_hit_hist)
    std::cout<<"BIG_SHELL_FORWARD_HIT f="<<f<<" closed="<<nc<<"\n";
  for(const auto&[d,nc]:reverse_depth_hist)
    std::cout<<"BIG_SHELL_REVERSE_DEPTH blocks="<<d<<" closed="<<nc<<"\n";
  for(const auto&[r,nc]:initial_o_hist)
    std::cout<<"BIG_SHELL_INITIAL_O r="<<r<<" closed="<<nc<<"\n";
  ...
  std::cout<<"VERIFIED_BIGINT_SHELL_FORWARD_REVERSE_COALESCENCE\n";
  return 0;
}
