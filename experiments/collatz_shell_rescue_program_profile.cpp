...
int main(int argc,char**argv){
  ...
  for(const State&s:cur){
    ...
    for(int f=0;f<=forward_h && !closed;++f){
      ...
      if(y<n){
        ...
        ++program_hist["F:"+std::to_string(f)];
      ...
      if(shell_macro_depth>0){
        ...
        if(shell_macro_close(y,n,shell_macro_depth,blocks,steps,witness)){
          ...
          ++initial_o_hist[0];
          ++program_hist["R:"+std::to_string(f)+":0:"+shell_program_text(g_shell_program)];
          if(shell_controls<20000){
      ...
      for(int r0=1;!closed && r0<=r0max;++r0){
        ...
        if(x<n){
          ...
          ++program_hist["R:"+std::to_string(f)+":"+std::to_string(r0)+":"];
          if(shell_controls<20000){
        ...
        if(shell_macro_depth>0){
          ...
          if(shell_macro_close(x,n,shell_macro_depth,blocks,extra,witness)){
            ++shell_reverse;++forward_hit_hist[f];++reverse_depth_hist[blocks];
            ++initial_o_hist[r0];
            ++program_hist["R:"+std::to_string(f)+":"+std::to_string(r0)+":"+shell_program_text(g_shell_program)];
            if(shell_controls<20000){
  ...
  std::cout<<"VERIFIED_SHELL_RESCUE_PROGRAM_VOCABULARY\n";
  return 0;
}
