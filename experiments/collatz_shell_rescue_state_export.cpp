...
static std::string g_shell_program;

static std::string shell_program_text(const std::string& p){
  std::string out;
  for(size_t i=0;i<p.size();++i){
    if(i)out.push_back(',');
    out+=std::to_string((unsigned char)p[i]);
  }
  return out;
}
...
static bool shell_macro_close(
    ...
    u128& witness){
  struct Node{u128 x;int blocks;int steps;std::string path;};
  std::vector<Node> cur,next;
  g_shell_program.clear();
  ...
  for(int depth=1;depth<=max_blocks;++depth){
    ...
    for(const Node&s:cur){
      ...
      // R_0 = reverse E.
      {
        ...
        if(x2<n){
          ...
          witness=x2;
          g_shell_program=s.path;
          g_shell_program.push_back(char(0));
          return true;
        }
        {
          std::string p=s.path;p.push_back(char(0));
          next.push_back({x2,depth,s.steps+1,std::move(p)});
        }
      }
      ...
      for(int r=1;r<=rmax;++r){
        ...
        if(x2<n){
          ...
          witness=x2;
          g_shell_program=s.path;
          g_shell_program.push_back(char(r));
          return true;
        }
        {
          std::string p=s.path;p.push_back(char(r));
          next.push_back({x2,depth,s.steps+r+1,std::move(p)});
        }
      }
    }
    ...
    for(const Node&z:next){
      ...
      else if(z.steps<uniq.back().steps)uniq.back()=z;
    }
    cur.swap(uniq);
  }
  return false;
...
int main(int argc,char**argv){
  if(argc!=5){std::cerr<<"usage: shell_forward_reverse K BANK REVERSE_DEPTH FORWARD_H\n";return 2;}
  ...
  if(K<1||K>30||shell_macro_depth<0||shell_macro_depth>20||forward_h<0||forward_h>512){
  ...
  // Shell-specific forward/reverse coalescence.
  //
  // For n=2^K+b, let y_f=T^f(T^K(n)).  At every 0<=f<=forward_h:
  //   * if y_f<n, direct strong-induction descent closes n;
  //   * otherwise search the exact reverse tree of y_f for p<n.
  //
  // A reverse witness satisfies T^s(p)=y_f, hence n and p coalesce after
  // K+f and s shortcut steps respectively.  Since p<n, strong induction
  // closes n.  This strictly generalizes the f=0 donor-free shell search.
  const u128 shellM=u128(1)<<K;
  uint64_t shell_forward=0,shell_reverse=0,shell_hard=0;
  uint64_t shell_controls=0;
  std::map<int,uint64_t> forward_hit_hist;
  std::map<int,uint64_t> reverse_depth_hist;
  std::map<int,uint64_t> initial_o_hist;
  std::map<std::string,uint64_t> program_hist;
  ...
  for(const State&s:cur){
    ...
    for(int f=0;f<=forward_h && !closed;++f){
      if(f>0)y=T(y);

      if(y<n){
        ++shell_forward;++forward_hit_hist[f];
        {
          const std::string key="F:"+std::to_string(f);
          ++program_hist[key];
          if(f>128)
            std::cout<<"SHELL_RESCUE_STATE K="<<K<<" b="<<b
                     <<" c="<<cc<<" d="<<dd<<" key="<<key<<"\n";
        }
        if(shell_controls<20000){
          if(iterate(n,K+f)!=y){
            std::cerr<<"SHELL_FORWARD_REPLAY_FAIL b="<<b<<" f="<<f<<"\n";
            return 30;
          }
          ++shell_controls;
        }
      ...
      const int r0max=v3_u128(y+1,64);
      u128 den=1,pow2=1;

      // Optional initial O-run length zero.
      if(shell_macro_depth>0){
        int blocks=0,steps=0;u128 witness=0;
        if(shell_macro_close(y,n,shell_macro_depth,blocks,steps,witness)){
          ++shell_reverse;++forward_hit_hist[f];++reverse_depth_hist[blocks];
          ++initial_o_hist[0];
          {
            const std::string key="R:"+std::to_string(f)+":0:"+shell_program_text(g_shell_program);
            ++program_hist[key];
            if(f>128)
              std::cout<<"SHELL_RESCUE_STATE K="<<K<<" b="<<b
                       <<" c="<<cc<<" d="<<dd<<" key="<<key<<"\n";
          ...
          if(shell_controls<20000){
            if(iterate(witness,steps)!=y || iterate(n,K+f)!=y){
              std::cerr<<"SHELL_FORWARD_REVERSE_REPLAY_FAIL b="<<b
                       <<" f="<<f<<" r0=0\n";
              return 31;
            }
            ++shell_controls;
      ...
      for(int r0=1;!closed && r0<=r0max;++r0){
        den*=3;pow2*=2;
        const u128 x=pow2*(y+1)/den-1;
        if(x==0)continue;

        if(x<n){
          ++shell_reverse;++forward_hit_hist[f];++initial_o_hist[r0];
          {
            const std::string key="R:"+std::to_string(f)+":"+std::to_string(r0)+":";
            ++program_hist[key];
            if(f>128)
              std::cout<<"SHELL_RESCUE_STATE K="<<K<<" b="<<b
                       <<" c="<<cc<<" d="<<dd<<" key="<<key<<"\n";
          ...
          if(shell_controls<20000){
            if(iterate(x,r0)!=y || iterate(n,K+f)!=y){
              std::cerr<<"SHELL_INITIAL_O_LOOKAHEAD_REPLAY_FAIL b="<<b
                       <<" f="<<f<<" r0="<<r0<<"\n";
              return 32;
            }
            ++shell_controls;
        ...
        if(shell_macro_depth>0){
          int blocks=0,extra=0;u128 witness=0;
          if(shell_macro_close(x,n,shell_macro_depth,blocks,extra,witness)){
            ++shell_reverse;++forward_hit_hist[f];++reverse_depth_hist[blocks];
            ++initial_o_hist[r0];
            {
              const std::string key="R:"+std::to_string(f)+":"+std::to_string(r0)+":"+shell_program_text(g_shell_program);
              ++program_hist[key];
              if(f>128)
                std::cout<<"SHELL_RESCUE_STATE K="<<K<<" b="<<b
                         <<" c="<<cc<<" d="<<dd<<" key="<<key<<"\n";
            ...
            if(shell_controls<20000){
              if(iterate(witness,r0+extra)!=y || iterate(n,K+f)!=y){
                std::cerr<<"SHELL_LOOKAHEAD_CONT_REPLAY_FAIL b="<<b
                         <<" f="<<f<<" r0="<<r0<<" blocks="<<blocks<<"\n";
                return 33;
              }
              ++shell_controls;
            }
    ...
    if(!closed)++shell_hard;
  }

  std::cout<<"SHELL_FORWARD_REVERSE_RESULT"
           <<" K="<<K
           <<" reverse_depth="<<shell_macro_depth
           <<" forward_h="<<forward_h
           <<" universal_residual="<<cur.size()
           <<" shell_forward="<<shell_forward
           <<" shell_reverse="<<shell_reverse
           <<" shell_hard="<<shell_hard
           <<" controls="<<shell_controls<<"\n";
  for(const auto&[f,nc]:forward_hit_hist)
    std::cout<<"SHELL_FORWARD_HIT f="<<f<<" closed="<<nc<<"\n";
  for(const auto&[d,nc]:reverse_depth_hist)
    std::cout<<"SHELL_REVERSE_DEPTH blocks="<<d<<" closed="<<nc<<"\n";
  for(const auto&[r,nc]:initial_o_hist)
    std::cout<<"SHELL_FORWARD_INITIAL_O r="<<r<<" closed="<<nc<<"\n";

  std::vector<std::pair<std::string,uint64_t>> programs(program_hist.begin(),program_hist.end());
  std::sort(programs.begin(),programs.end(),[](const auto&a,const auto&b){
    if(a.second!=b.second)return a.second>b.second;
    return a.first<b.first;
  });
  uint64_t top10=0,top50=0,top100=0;
  for(size_t i=0;i<programs.size();++i){
    if(i<10)top10+=programs[i].second;
    if(i<50)top50+=programs[i].second;
    if(i<100)top100+=programs[i].second;
    if(i<200)
      std::cout<<"SHELL_PROGRAM rank="<<(i+1)
               <<" count="<<programs[i].second
               <<" key="<<programs[i].first<<"\n";
  }
  std::cout<<"SHELL_PROGRAM_VOCAB"
           <<" unique="<<programs.size()
  ...
  std::cout<<"VERIFIED_SHELL_RESCUE_STATE_EXPORT\n";
  return 0;
}
