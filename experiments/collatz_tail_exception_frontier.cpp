...
int main(int argc,char**argv){
  ...
  uint64_t total_descent=0,total_bridge=0,total_deep_o=0;
  uint64_t bridge_controls=0,deep_o_controls=0;
  ...
  for(int k=1;k<=K;++k){
    ...
    uint64_t descent=0,bridge=0,deep_o=0;
  ...
  // Final-frontier tail analysis.
  //
  // Universal family closure above required the lower witness to beat n at
  // a=1.  If the witness coefficient is nevertheless smaller than 2^K, then
  // only finitely many small affine parameters a can fail.  Count those
  // finite-exception families exactly instead of misclassifying their entire
  // infinite tails as unresolved.
  const i128 FM=i128(1)<<K;
  uint64_t tail_direct=0,tail_bridge=0,tail_deep_o=0,hard=0;
  u128 total_exceptions=0;
  u128 max_cutoff=0;

  auto cutoff_for=[&](i128 pcoef,i128 pconst,uint64_t b)->u128{
    if(pcoef<=0||pcoef>=FM)return ~u128(0);
    const i128 gap=FM-pcoef;
    const i128 excess=pconst-i128(b);

    u128 nonlower=0;
    if(excess>=gap)nonlower=u128(excess/gap);

    u128 nonpositive=0;
    if(pconst<0)nonpositive=u128((-pconst)/pcoef);

    return std::max(nonlower,nonpositive);
  };

  for(const State&s:cur){
    const uint64_t b=s.b,d=state_d(s);
    const int cc=state_c(s);

    u128 best=~u128(0);
    int source=0; // 1 direct, 2 retained reverse, 3 deep O

    // Direct future family m(a)=a*3^c+d.
    {
      const i128 pc=i128(p3[cc]);
      const u128 cut=cutoff_for(pc,i128(d),b);
      if(cut<best){best=cut;source=1;}
    }

    // First matching retained reverse cylinder, exactly as in bridge_closes.
    {
      ...
      for(int o=1;o<=top;++o){
        ...
        const RevCert& z=it->second;
        const i128 nc=(i128(1)<<z.steps)*i128(d)-i128(z.C);
        if(nc%i128(mod)!=0){std::cerr<<"TAIL_BRIDGE_DIV_FAIL\n";return 12;}
        const i128 pk=nc/i128(mod);
        const i128 pc=(i128(1)<<z.steps)*i128(p3[cc-z.o]);
        const u128 cut=cutoff_for(pc,pk,b);
        if(cut<best){best=cut;source=2;}
        break;
      }
    }

    // Nested O^j continuations, including constructors discarded as
    // redundant by the original first-contraction quotient.
    if(cc>=2 && d!=UINT64_MAX){
      const uint64_t dp1=d+1;
      for(int j=2;j<=cc;++j){
        if(dp1%p3[j])break;
        const uint64_t q=dp1/p3[j];
        const i128 pk=(i128(1)<<j)*i128(q)-1;
        const i128 pc=(i128(1)<<j)*i128(p3[cc-j]);
        const u128 cut=cutoff_for(pc,pk,b);
        if(cut<best){best=cut;source=3;}
      }
    }
    ...
    // A cutoff of zero would mean the family should already have been
    // universally removed by the corresponding constructor.
    if(best==0){
      std::cerr<<"TAIL_ZERO_CUTOFF_SURVIVOR b="<<b<<" source="<<source<<"\n";
      return 13;
    }

    if(source==1)++tail_direct;
    else if(source==2)++tail_bridge;
    else if(source==3)++tail_deep_o;
    else{std::cerr<<"TAIL_SOURCE_INVALID\n";return 14;}

    total_exceptions+=best;
    if(best>max_cutoff)max_cutoff=best;
  }
  ...
  std::cout<<"TAIL_FRONTIER"
           <<" K="<<K
           <<" live_families="<<cur.size()
           <<" tail_direct="<<tail_direct
           <<" tail_bridge="<<tail_bridge
           <<" tail_deep_o="<<tail_deep_o
           <<" finite_tail_families="<<(tail_direct+tail_bridge+tail_deep_o)
           <<" hard_families="<<hard
           <<" total_finite_exceptions="<<s128(total_exceptions)
           <<" max_exception_a="<<s128(max_cutoff)<<"\n";
  ...
  std::cout<<"VERIFIED_TAIL_EXCEPTION_FRONTIER\n";
  return 0;
}
