// Exact deep-prefix + coalescence + predecessor + block Collatz scanner.
//
// Mid-k production candidate:
//   predecessor half merge
//   + exact transfer-prefix contraction through DEEP_BITS (sparse table)
//   + C-bit lower-family coalescence
//   + q7 reverse-predecessor induction
//   + exact B-bit block scan.
//
// The deep prefix is generated from BASE_BITS classes and extended only across
// the extra DEEP_BITS-BASE_BITS suffix bits.  No dense 2^DEEP_BITS table exists.
// This is the executable consequence-language compression discovered by the
// p28 BDD experiment.
...
static std::vector<uint32_t>LIVE_DEEP;
...
static void init_coalescence(int k){
  ...
  if(C==16&&k>=1&&k<=40&&killed!=17917){
    std::cerr<<"COALESCENCE_CENSUS_MISMATCH got="<<killed<<"\n";std::exit(9);
  }
  std::cout<<"COALESCENCE_TABLE K="<<k<<" C="<<C<<" killed_classes="<<killed
           <<" odd_classes="<<(CMOD/2)<<" fraction="<<(double(killed)/double(CMOD/2))<<"\n";
}
...
static void init_pred_q7(int k){
  ...
  uint32_t step=1;
  for(int i=0;i<k+D;++i)step=(step*2u)%PRED_MOD;
  ...
  std::cout<<"PRED_Q7_JUMP_SCHEDULE step="<<PRED_STEP<<"\n";
}

static void init_deep_prefix(int k){
  const int W0=k+S,W1=k+D;
  if(!(k>=8&&W1<127&&uint64_t(W1)<FIRST_DANGEROUS)){std::cerr<<"DEEP_PREFIX_PRECONDITION\n";std::exit(4);}
  std::array<u128,128>P3{};
  P3[0]=1;for(int i=1;i<128;++i){if(P3[i-1]>UMAX/3)break;P3[i]=P3[i-1]*3;}
  LIVE_DEEP.clear();
  const int killed_mod4=(k&1)?3:1;
  const uint32_t suffixes=uint32_t(1)<<(D-S);
  ...
  for(uint32_t r=1;r<SMOD;r+=2){
    ...
    for(uint32_t suf=0;suf<suffixes;++suf){
      ...
      if(ok)LIVE_DEEP.push_back(r|(suf<<S));
    }
  }
  if(D==28){
    uint64_t expect=0;
    ...
    if(expect&&LIVE_DEEP.size()!=expect){
      std::cerr<<"DEEP_PREFIX_CENSUS_MISMATCH got="<<LIVE_DEEP.size()<<" expected="<<expect<<"\n";std::exit(13);
    }
  }
  std::cout<<"DEEP_PREFIX_TABLE K="<<k<<" base_bits="<<S<<" deep_bits="<<D
           <<" live_classes="<<LIVE_DEEP.size()
           <<" odd_classes="<<(uint64_t(1)<<(D-1))
           <<" fraction="<<(double(LIVE_DEEP.size())/double(uint64_t(1)<<(D-1)))<<"\n";
}
...
int main(int argc,char**argv){
  ...
  init_blocks();init_coalescence(k);init_pred_q7(k);init_deep_prefix(k);
  ...
  if(mhi<mlo){std::cout<<"STATUS DEEP_PREFIX_COAL_PREDQ7_SIEVE_AND_SCAN_CLOSED\n";return 0;}
  ...
  std::cout<<"EXACT_V2_DEEP_FUSED_SCAN B="<<B<<" S="<<S<<" D="<<D<<" C="<<C<<" k="<<k
           <<" full_count="<<full_total<<" live_residue_classes="<<LIVE_DEEP.size()
...
#pragma omp for schedule(dynamic,64)
    for(size_t ri=0;ri<LIVE_DEEP.size();++ri){
      uint32_t r=LIVE_DEEP[ri];
      uint64_t delta=(uint64_t(r)-(smlo&DMASK))&DMASK;
      uint64_t m=smlo+delta;
      if(m<smlo||m>smhi)continue;
      uint64_t positions=(smhi-m)/DMOD+1;
      ldeep+=positions;

      if(COAL_KILLED[r&CMASK]){lcoal+=positions;continue;}
      ...
      lpred+=d0;positions-=d0;m+=uint64_t(d0)*DMOD;rp=PRED_LIVE0[rp];
...
#pragma omp critical
        ...
        m+=uint64_t(d)*DMOD;rp=PRED_NEXTLIVE[rp];
      }
    }
#pragma omp critical
    {
  ...
  std::cout<<"DEEP_PREFIX_LIVE "<<deep_live
  ...
  if(any_survivor){
    ...
    std::cout<<"STATUS DEEP_PREFIX_COAL_PREDQ7_SIEVE_AND_SCAN_CLOSED\n";
  }
  return 0;
}
