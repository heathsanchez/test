// Exact cache-tuned full valuation-slice scanner.
//
// Difference from the early-exit scanner: it always completes the assigned
// shard and performs no per-seed atomic load. This is the right mode when we
// want an exact slice maximum, not merely the first survivor. Each thread keeps
// local evidence and merges once. BLOCK_BITS is tunable; B=10 is current best.
#include <omp.h>
#include <algorithm>
#include <array>
#include <cstdint>
...
int main(int argc,char**argv){
  ...
  if(mhi<mlo){
    std::cout<<"EXACT_V2_FULLSCAN B="<<B<<" k="<<k<<" count=0 horizon="<<H<<" shard="<<I<<"/"<<S<<"\n";
  ...
  std::cout<<"EXACT_V2_FULLSCAN B="<<B<<" k="<<k<<" count="<<((smhi-smlo)/2+1)
           <<" horizon="<<H<<" shard="<<I<<"/"<<S
           <<" threads="<<omp_get_max_threads()
           <<" table_bytes="<<(TAB.size()*sizeof(Block))
