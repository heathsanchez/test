...
static std::array<uint16_t,PRED_MOD> PRED_JUMP0{};
static std::array<uint16_t,PRED_MOD> PRED_JUMP1{};
static std::array<uint16_t,PRED_MOD> PRED_LIVE0{};
static std::array<uint16_t,PRED_MOD> PRED_NEXTLIVE{};
...
static void init_pred_q7(int k){
  ...
  if(n!=1013){std::cerr<<"PRED_Q7_CENSUS_MISMATCH got="<<n<<"\n";std::exit(10);}
  uint32_t step=1;
  ...
  for(uint32_t r=0;r<PRED_MOD;++r){
    ...
    while(PRED_KILLED[x]){
      ...
      if(d>PRED_MOD){std::cerr<<"PRED_JUMP0_CYCLE\n";std::exit(11);}
    }
    PRED_JUMP0[r]=(uint16_t)d;
    PRED_LIVE0[r]=(uint16_t)x;

    x=r+step; if(x>=PRED_MOD)x-=PRED_MOD; d=1;
    while(PRED_KILLED[x]){
      x+=step; if(x>=PRED_MOD)x-=PRED_MOD; ++d;
      if(d>PRED_MOD){std::cerr<<"PRED_JUMP1_CYCLE\n";std::exit(12);}
    }
    PRED_JUMP1[r]=(uint16_t)d;
    PRED_NEXTLIVE[r]=(uint16_t)x;
  }
...
int main(int argc,char**argv){
  ...
  if(mhi<mlo){std::cout<<"STATUS PREFIX_COAL_PREDQ7_SIEVE_AND_SCAN_CLOSED\n";return 0;}
  ...
  unsigned long long scanned=0,coal_killed=0,predq7_killed=0,fastblocks=0,slowblocks=0,overflows=0;
  int global_best=0;u128 global_best_seed=0;

  std::cout<<"EXACT_V2_FUSED_PREFIX_COAL_PREDQ7_SCAN B="<<B<<" S="<<S<<" k="<<k
...
#pragma omp parallel
  {
    unsigned long long lscan=0,lcoal=0,lpred=0,lfb=0,lsb=0,lov=0;
...
#pragma omp for schedule(dynamic,64)
    for(uint32_t r=1;r<RMOD;r+=2){
      ...
      const uint64_t class_count=(smhi-m)/RMOD+1;
      // C<=S, so every concrete m in this p24 class has the same low C bits.
      // Compile the entire coalescence decision once at class granularity.
      if(COAL_KILLED[r&CMASK]){
        lcoal+=class_count;
        continue;
      }
      ...
      // Jump directly over an initial run of q7-killed phases.
      uint32_t d0=PRED_JUMP0[rp];
      if(uint64_t(d0)>=positions){
        lpred+=positions;
        continue;
      }
      lpred+=d0;
...
#pragma omp critical
        ...
        // PRED_JUMP1 is the distance from this live q7 phase to the next live
        // phase.  Skip all intervening killed seeds without touching them.
        const uint32_t d=PRED_JUMP1[rp];
        const uint64_t killed_between=uint64_t(d)-1;
        if(killed_between>=positions){
          lpred+=positions;
          break;
        }
        lpred+=killed_between;
        positions-=killed_between;
        m+=uint64_t(d)*RMOD;
        rp=PRED_NEXTLIVE[rp];
      }
    }
#pragma omp critical
    {
      scanned+=lscan;coal_killed+=lcoal;predq7_killed+=lpred;fastblocks+=lfb;slowblocks+=lsb;overflows+=lov;
  ...
  std::cout<<"COALESCENCE_EXTRA_KILLED "<<coal_killed
           <<" PREDQ7_KILLED_AFTER_COALESCENCE "<<predq7_killed
           <<" EXPLICIT "<<scanned<<"\n";
  ...
  if(any_survivor){
    ...
    std::cout<<"STATUS PREFIX_COAL_PREDQ7_SIEVE_AND_SCAN_CLOSED\n";
  }
  return 0;
}
