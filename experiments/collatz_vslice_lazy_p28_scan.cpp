// Exact p24 production scheduler with a lazy p28 veto.
//
// Preserve the fast retained execution order:
//   p24 transfer -> whole-class coalescence -> q7 jump schedule -> trajectory.
//
// Only at a q7-live candidate do we consult the exact four-bit p28 future
// consequence mask for its p24 class.  Thus the stronger p28 theorem acts as
// one cheap bit-test immediately before expensive trajectory work, without
// replacing the efficient p24/q7 traversal.
//
// The p28 mask is computed once per live, non-coalesced p24 class.
...
static void init_residue_sieve(int k,uint64_t&merge_classes,uint64_t&contract_classes,uint64_t&live_classes){
  const int W=k+S;
  if(!(k>=8&&W<127&&uint64_t(W)<FIRST_DANGEROUS)){std::cerr<<"PREFIX_SIEVE_PRECONDITION_FAILED\n";std::exit(4);}
  RTYPE.assign(RMOD,0);
...
// Exact survival language for bits S..27, conditional on this p24 class
// surviving the retained p24 transfer theorem.
static inline uint16_t p28_future_mask(int k,uint32_t r){
  const int W0=k+S;
  u128 x=(u128(r)<<k)-1;
  uint8_t q=0;
  for(int t=1;t<=W0;++t){
...
int main(int argc,char**argv){
  ...
  if(mhi<mlo){std::cout<<"STATUS LAZY_P28_VETO_SCAN_CLOSED\n";return 0;}
  ...
  unsigned long long scanned=0,coal_killed=0,pred_killed=0,p28_vetoed=0,fastblocks=0,slowblocks=0,overflows=0;
  int global_best=0;u128 global_best_seed=0;

  std::cout<<"EXACT_V2_LAZY_P28_VETO_SCAN B="<<B<<" S="<<S<<" D="<<D<<" C="<<C<<" k="<<k
...
#pragma omp parallel
  {
    unsigned long long lscan=0,lcoal=0,lpred=0,lveto=0,lfb=0,lsb=0,lov=0;
...
#pragma omp for schedule(dynamic,64)
    for(uint32_t r=1;r<RMOD;r+=2){
      ...
      const uint16_t mask=p28_future_mask(k,r);
      if(!mask){
        // Every p24 position in this class is killed by the added four-bit
        // transfer theorem, irrespective of q7 phase.
        lveto+=class_count;
        continue;
      }
      ...
      while(positions){
        const uint32_t phase=uint32_t((m>>S)&(PHASES-1));
        if(((mask>>phase)&1u)==0){
          ++lveto;
        }else{
...
#pragma omp critical
    {
      scanned+=lscan;coal_killed+=lcoal;pred_killed+=lpred;p28_vetoed+=lveto;
  ...
  std::cout<<"LAZY_P28_COUNTS COALESCENCE_KILLED "<<coal_killed
  ...
  if(any_survivor){
    ...
    std::cout<<"STATUS LAZY_P28_VETO_SCAN_CLOSED\n";
  }
  return 0;
}
