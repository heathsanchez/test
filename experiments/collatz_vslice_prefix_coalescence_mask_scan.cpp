...
#endif
#ifndef COAL_BASE_BITS
#define COAL_BASE_BITS 16
#endif
#ifndef COAL_DEEP_BITS
#define COAL_DEEP_BITS 20
#endif
static constexpr int B=BLOCK_BITS;
static constexpr int S=PREFIX_BITS;
static constexpr int CBASE=COAL_BASE_BITS;
static constexpr int CDEEP=COAL_DEEP_BITS;
static_assert(B>=4 && B<=20);
static_assert(S>=2 && S<=24);
static_assert(CBASE>=2 && CBASE<=20);
static_assert(CDEEP>=CBASE && CDEEP<=20);
static_assert(CDEEP-CBASE<=4);
...
static constexpr uint32_t CBASE_MOD=(uint32_t(1)<<CBASE);
static constexpr uint32_t CBASE_MASK=CBASE_MOD-1;
static constexpr uint32_t CDEEP_MOD=(uint32_t(1)<<CDEEP);
static constexpr uint32_t CREFINE=(uint32_t(1)<<(CDEEP-CBASE));
...
// Exact compressed cross-valuation coalescence classifier.
// A 20-bit killed set is stored as one refinement mask for each odd 16-bit
// prefix.  With CDEEP-CBASE=4 this is 32768 uint16_t entries = 64 KiB.
static std::vector<uint16_t> COAL_MASK;
...
static inline bool coalesces(int k,uint32_t r){
  u128 nA=u128(1)<<(k+CDEEP);
  u128 nB=(u128(1)<<k)*r-1;
  u128 pA=nA>>1;
  u128 pB=(nB-1)>>1;
  while(!(nA&1)){
    affine_step(nA,nB);
    if(nA==pA && nB==pB)return true;
    if(pA&1)break;
    affine_step(pA,pB);
  }
  return false;
...
static void init_coalescence_sieve(int k){
  COAL_MASK.assign(CBASE_MOD/2,0);
  uint64_t killed=0;
  for(uint32_t r=1;r<CDEEP_MOD;r+=2){
    if(!coalesces(k,r))continue;
    const uint32_t low=r&CBASE_MASK;
    const uint32_t hi=r>>CBASE;
    COAL_MASK[low>>1] |= uint16_t(1u<<hi);
    ++killed;
  }
  if(CDEEP==20 && killed!=292321){
    std::cerr<<"COALESCENCE_CENSUS_MISMATCH got="<<killed<<"\n";std::exit(9);
  }
  std::cout<<"COALESCENCE_MASK"
           <<" K="<<k
           <<" base_bits="<<CBASE
           <<" deep_bits="<<CDEEP
           <<" killed_classes="<<killed
           <<" odd_classes="<<(CDEEP_MOD/2)
           <<" bytes="<<(COAL_MASK.size()*sizeof(uint16_t))
           <<" fraction="<<(double(killed)/double(CDEEP_MOD/2))<<"\n";
}

static inline bool coalescence_killed(uint64_t m){
  const uint32_t low=uint32_t(m)&CBASE_MASK;
  const uint32_t hi=(uint32_t(m)>>CBASE)&(CREFINE-1);
  return (COAL_MASK[low>>1]>>hi)&1u;
}
...
int main(int argc,char**argv){
  ...
  if(mhi<mlo){std::cout<<"STATUS PREFIX_COALMASK_MOD9_SIEVE_AND_SCAN_CLOSED\n";return 0;}
  ...
  std::cout<<"EXACT_V2_PREFIX_COALMASK_MOD9_SCAN B="<<B<<" S="<<S<<" k="<<k
...
#pragma omp critical
  ...
  if(any_survivor){
    ...
    std::cout<<"STATUS PREFIX_COALMASK_MOD9_SIEVE_AND_SCAN_CLOSED\n";
  }
  return 0;
}
