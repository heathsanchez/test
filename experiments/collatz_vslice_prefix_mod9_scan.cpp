...
int main(int argc,char**argv){
  ...
  if(mhi<mlo){std::cout<<"STATUS PREFIX_MOD9_SIEVE_AND_SCAN_CLOSED\n";return 0;}
  ...
  unsigned long long scanned=0,mod9_killed=0,fastblocks=0,slowblocks=0,overflows=0;
  int global_best=0;u128 global_best_seed=0;

  std::cout<<"EXACT_V2_PREFIX_MOD9_SCAN B="<<B<<" S="<<S<<" k="<<k
...
#pragma omp parallel
  {
    unsigned long long lscan=0,lmod9=0,lfb=0,lsb=0,lov=0;
...
#pragma omp critical
    {
      scanned+=lscan;mod9_killed+=lmod9;fastblocks+=lfb;slowblocks+=lsb;overflows+=lov;
  ...
  std::cout<<"MOD9_KILLED "<<mod9_killed<<" EXPLICIT "<<scanned<<"\n";
  ...
  if(any_survivor){
    ...
    std::cout<<"STATUS PREFIX_MOD9_SIEVE_AND_SCAN_CLOSED\n";
  }
  return 0;
}
