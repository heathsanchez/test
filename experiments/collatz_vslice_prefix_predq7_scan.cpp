...
int main(int argc,char**argv){
  ...
  if(mhi<mlo){std::cout<<"STATUS PREFIX_PREDQ7_SIEVE_AND_SCAN_CLOSED\n";return 0;}
  ...
  unsigned long long scanned=0,pred_killed=0,fastblocks=0,slowblocks=0,overflows=0;
  int global_best=0;u128 global_best_seed=0;

  std::cout<<"EXACT_V2_PREFIX_PREDQ7_SCAN B="<<B<<" S="<<S<<" k="<<k
...
#pragma omp parallel
  {
    unsigned long long lscan=0,lpred=0,lfb=0,lsb=0,lov=0;
...
#pragma omp critical
    {
      scanned+=lscan;pred_killed+=lpred;fastblocks+=lfb;slowblocks+=lsb;overflows+=lov;
  ...
  std::cout<<"PREDQ7_KILLED "<<pred_killed<<" EXPLICIT "<<scanned<<"\n";
  ...
  if(any_survivor){
    ...
    std::cout<<"STATUS PREFIX_PREDQ7_SIEVE_AND_SCAN_CLOSED\n";
  }
  return 0;
}
