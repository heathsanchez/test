// Exact early-exit valuation-slice survivor scanner with arbitrary-precision
// fallback for rare trajectories that outgrow unsigned __int128.
//
// Fast path: identical exact 16-step block compiler used for prior valuation
// slices.  If an individual shortcut step would overflow u128, replay that
// seed from scratch with boost::multiprecision::cpp_int.  Thus overflow is a
// representation change, never a scientific verdict.
#include <omp.h>
...
struct BigReplay { int descent; cpp_int peak; };

static BigReplay replay_big(u128 seed128,int H){
  cpp_int seed=0;
  // Import u128 without narrowing.
  seed += uint64_t(seed128>>64);
  seed <<= 64;
  seed += uint64_t(seed128);
  cpp_int x=seed,peak=seed;
  for(int t=1;t<=H;++t){
    if((x&1)!=0) x=(3*x+1)>>1;
    else x>>=1;
    if(x>peak) peak=x;
    if(x<seed) return {t,peak};
  }
  return {H+1,peak};
}
...
int main(int argc,char**argv){
  ...
  if(mhi<mlo){
    std::cout<<"EXACT_V2_SURVIVOR_BIG k="<<k<<" count=0 horizon="<<H<<" shard="<<I<<"/"<<S<<"\n";
  ...
  u128 witness=0;
  unsigned long long scanned=0,fastblocks=0,slowblocks=0,bigfallbacks=0;
  int global_best=0; u128 global_best_seed=0;

  std::cout<<"EXACT_V2_SURVIVOR_BIG k="<<k<<" count="<<((smhi-smlo)/2+1)
           <<" horizon="<<H<<" shard="<<I<<"/"<<S
...
#pragma omp parallel
  {
    unsigned long long lscan=0,lfb=0,lsb=0,lbig=0;
...
#pragma omp for schedule(dynamic,4096)
    for(uint64_t m=smlo;m<=smhi;m+=2){
      ...
      while(descent==H+1 && !need_big && t+B<=H){
        ...
        for(int j=0;j<B && t<H;++j){
          if(!step_u128(x)){need_big=true;break;}
          ++t;
          if(x<seed){descent=t;break;}
        }
      }
      while(descent==H+1 && !need_big && t<H){
        if(!step_u128(x)){need_big=true;break;}
        ++t;
        if(x<seed){descent=t;break;}
      }
      ...
      if(need_big){
        ++lbig;
        const BigReplay br=replay_big(seed,H);
        descent=br.descent;
#pragma omp critical
        {
          std::cout<<"BIG_FALLBACK seed="<<str128(seed)
                   <<" first_descent_or_survival="<<descent
                   <<" peak="<<br.peak<<"\n";
        }
      }
