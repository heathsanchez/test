// Exact early-exit valuation-slice scanner with external arbitrary-precision
// fallback. Rare u128-overflow seeds are emitted, not classified. The workflow
// replays only those seeds with Python big integers before assigning a final
// shard verdict. Overflow is therefore a representation switch, never a verdict.
#include <omp.h>
#include <algorithm>
#include <array>
#include <atomic>
...
int main(int argc,char**argv){
  ...
  if(mhi<mlo){
    std::cout<<"EXACT_V2_SURVIVOR_FALLBACK k="<<k<<" count=0 horizon="<<H<<" shard="<<I<<"/"<<S<<"\n";
  ...
  std::cout<<"EXACT_V2_SURVIVOR_FALLBACK k="<<k<<" count="<<((smhi-smlo)/2+1)
           <<" horizon="<<H<<" shard="<<I<<"/"<<S
