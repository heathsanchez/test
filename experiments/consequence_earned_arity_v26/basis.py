#!/usr/bin/env python3
"""Frozen V26 generic tuple-set substrate."""
...
def tuple_universe(
    ...
    return tuple(
        tuple(parts)
        for parts in itertools.product(world.cells(), repeat=int(arity))
    )
...
def all_tuple_sets(
    world: ConsequenceWorld,
    arity: int,
) -> Iterator[frozenset[TupleFact]]:
    universe = tuple_universe(world, arity)
    for mask in range(1 << len(universe)):
        yield frozenset(
            fact
            for i, fact in enumerate(universe)
            if (mask >> i) & 1
        )
...
def relation_distance(
    ...
    current = frozenset(zip(world.cells(), identity_mapping(world)))
    candidate = frozenset(zip(world.cells(), mapping))
    return len(current ^ candidate)
