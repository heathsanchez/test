#!/usr/bin/env python3
"""Frozen V24 positive relation-construction substrate."""
...
def extensional_distance(
    ...
    return len(
        mapping_relation(world, mapping)
        ^ identity_relation(world)
    )
...
def all_total_mappings(world: ConsequenceWorld) -> Iterator[Mapping]:
    cells = world.cells()
    yield from itertools.product(cells, repeat=len(cells))
...
def difference_term(
    ...
    return (
        ...
        *tuple(
            ...
            for source, target in difference_edges(world, mapping)
        ),
    )
