#!/usr/bin/env python3
"""Frozen V24 direct relation-construction and derived-difference abstraction kernel."""
...
from basis import (
    Cell, ConsequenceWorld, Mapping,
    all_total_mappings, compose_mappings,
...
class Kernel:
    ...
    @staticmethod
    def verify_mapping(world,residual,mapping,all_l0,verification_enabled=True):
        if not verification_enabled:
            return {"status":"UNKNOWN_NO_VERIFIER","accepted":False}
        cells=world.cells(); idx={c:i for i,c in enumerate(cells)}
        bij=mapping_is_bijection(world,mapping)
        discharge=mapping[idx[residual.left]]==residual.right
        consequence=mapping_preserves_consequence(world,mapping) if bij else False
        novel=mapping not in all_l0
        accepted=bij and discharge and consequence and novel
        return {
            "status":"VERIFIED" if accepted else "REJECTED",
            "accepted":accepted,"bijection":bij,
            "residual_discharge":discharge,
            "consequence_preserving":consequence,
            "novel_outside_current_language":novel,
            "distance":extensional_distance(world,mapping),
        }
    ...
    def exhaustive_repair(self,world,residual,verification_enabled=True):
        ...
        all_l0=factorized_mapping_set(world)
        tested=0; accepted=[]
        for mapping in all_total_mappings(world):
            tested+=1
            verdict=self.verify_mapping(world,residual,mapping,all_l0,True)
            if verdict["accepted"]:
                accepted.append((mapping,verdict))
        if not accepted:
            return {
                "status":"CERTIFIED_NO_REPAIR_IN_DECLARED_RELATION_LANGUAGE",
                "tested_mapping_count":tested,
                "acquisition_search_count":1,
                "_frontier_objects":tuple(),
            }
        best=min(v["distance"] for _,v in accepted)
        frontier=[(m,v) for m,v in accepted if v["distance"]==best]
        ...
        return {
            ...
            "frontier":[
                {
                    "mapping":[list(c) for c in m],
                    "difference_edges":[[list(a),list(b)] for a,b in difference_edges(world,m)],
                    "verdict":v,
                }
                for m,v in frontier
            ],
            "_frontier_objects":tuple(m for m,_ in frontier),
            "acquisition_search_count":1,
        }

    def compile_schema(self,training):
        if len(training)<2:return {"status":"INSUFFICIENT_INDEPENDENT_REPAIRS"}
        terms=[difference_term(world,mapping) for origin,world,residual,mapping in training]
        schema=terms[0]
        for term in terms[1:]:
        ...
        sid="er_"+hashlib.sha256(json.dumps(term_to_json(schema),sort_keys=True,separators=(",",":")).encode()).hexdigest()[:16]
    ...
    def develop(self,world,verification_enabled=True):
        ...
        if diag.get("status")=="NO_RESIDUAL":
            return {
                ...
                "diagnosis":diag,"acquisition_search_count":0,
            }
        ...
        return {
            ...
            "_mapping":repair.get("_frontier_objects",(None,))[0] if repair.get("_frontier_objects") else None,
