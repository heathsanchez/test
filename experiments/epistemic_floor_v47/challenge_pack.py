...
def consequence(base_kind:str,history:tuple[int,...])->int:
    ...
    if base_kind=="parity":
        ...
        return state
    if base_kind=="future":
        # Two hidden bits; token 0 flips high bit, token 1 flips low bit.
        state=0
        for token in history:
...
def make_world(kind:str,variant:int=0,*,relabel=False,packet_complete=True)->World:
    ...
    if kind=="branching_open":
        # Explicitly observe both consequences for the empty history.
        samples.append(Sample((),0 if not relabel else 1))
        samples.append(Sample((),1 if not relabel else 0))
