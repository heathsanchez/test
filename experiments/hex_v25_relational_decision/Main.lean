...
/-! # V25: executable verified relational isomorphism decision -/
...
/-- Run Hex's verified isomorphism search on the constructed incidence graphs. -/
noncomputable def findSourceIso
    (P : IncidenceNumberingPair A B n k) : Option (Hex.Perm n) :=
  Hex.GraphIso.findIso P.leftFinite.toHex P.rightFinite.toHex

/-- Boolean source-isomorphism decision inherited from Hex. -/
noncomputable def sourceIsIso
    (P : IncidenceNumberingPair A B n k) : Bool :=
  Hex.GraphIso.isIso P.leftFinite.toHex P.rightFinite.toHex

/-- The returned Hex transporter exists exactly when the source structures are isomorphic. -/
theorem findSourceIso_isSome_iff
    (P : IncidenceNumberingPair A B n k) :
    P.findSourceIso.isSome = true ↔ Nonempty (SourceIso A B) := by
  rw [findSourceIso, Hex.GraphIso.findIso_isSome_iff]
  exact P.sourceIso_iff_hexIsomorphic.symm

/-- The executable Boolean decision is sound and complete for source isomorphism. -/
theorem sourceIsIso_eq_true_iff
    (P : IncidenceNumberingPair A B n k) :
    P.sourceIsIso = true ↔ Nonempty (SourceIso A B) := by
  rw [sourceIsIso, Hex.GraphIso.isIso_eq_true_iff]
  exact P.sourceIso_iff_hexIsomorphic.symm

/-- A false Boolean result is a genuine refutation of source isomorphism. -/
theorem sourceIsIso_eq_false_iff
    (P : IncidenceNumberingPair A B n k) :
    P.sourceIsIso = false ↔ ¬ Nonempty (SourceIso A B) := by
  rw [sourceIsIso, Hex.GraphIso.isIso_eq_false_iff]
  constructor
  · intro h hs
    exact h (P.sourceIso_iff_hexIsomorphic.mp hs)
  · intro h hi
    exact h ((P.sourceIso_iff_hexIsomorphic).mpr hi)

/-- Any concrete transporter returned by Hex is therefore evidence of source isomorphism. -/
theorem sourceIso_of_findSourceIso
    (P : IncidenceNumberingPair A B n k)
    {p : Hex.Perm n}
    (h : P.findSourceIso = some p) :
    Nonempty (SourceIso A B) := by
  have hi : Hex.GraphIso.Isomorphic P.leftFinite.toHex P.rightFinite.toHex :=
    Hex.GraphIso.Isomorphic.intro p (Hex.GraphIso.findIso_sound h)
  exact P.sourceIso_iff_hexIsomorphic.mpr hi
