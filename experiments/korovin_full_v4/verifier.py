...
def law_audit(model):
    ...
    associative=False
    if closed:
        associative=all(table[table[a][b]][c]==table[a][table[b][c]] for a,b,c in product(states,repeat=3))
    inv={}
    ...
    return {"closed":closed,"identity_count":len(identities),
            "identity":identities[0] if len(identities)==1 else None,
            "associative":associative,"all_elements_invertible":all_inv,
