#!/usr/bin/env python3
"""Use the pinned benchmark with a custom v1 baseline, without editing its files.
Only the source-selection callback is overridden. Timing and proof-target
construction remain the pinned benchmark implementation. Never claims PMU rank.
"""
...
EVIDENCE = ROOT / 'mertens-v2-evidence'
...
def prove():
    ...
    for version in ['v1', *VARIANTS]:
        ...
        axioms = {x.strip() for x in axioms_match.group(1).split(',') if x.strip()} if axioms_match else set()
        passed = code == 0 and (axioms_match is not None or axiom_free)
        passed = passed and axioms <= {'propext', 'Quot.sound', 'Classical.choice'}
        gates[version] = {'proof_pass': passed, 'returncode': code,
    ...
    if not gates['v1']['proof_pass']:
        raise RuntimeError('Protected v1 failed the proof/audit control; stop.')
    if not any(gates[v]['proof_pass'] for v in VARIANTS):
        raise RuntimeError('No challenger passed. Logs name exact proof residuals.')
...
def diagnostic(version, metric, output_name):
    ...
    try:
        ...
        command = ['diagnostic', '--problem', 'mertens', '--submission',
                   str(SOURCES / f'Submission_{version}.lean'), '--metric', metric,
                   '--repetitions', '3', '--timeout', '600', '--memory-mb', '4096',
    ...
    print('Diagnostic fields:', sorted(data), flush=True)
    data['baseline'] = 'custom-protected-mertens-v1'
    data['source_selection_note'] = 'Only official_baseline callback overridden; pinned timing engine unmodified.'
    (out / 'diagnostic.json').write_text(json.dumps(data, indent=2) + '\n')
    if code != 0 or not data.get('complete') or data['inputs'] != INPUTS:
        raise RuntimeError(f'{version} has an incomplete {metric} diagnostic')
    ...
    return {'version': version, 'metric': metric, 'baseline': b, 'candidate': c,
            'reduction_pct': 100 * (1 - c / b), 'ratio': c / b,
...
def screen():
    ...
    save('selected.json', {'version': selected['version'],
                           'selection_metric': 'wall-time-only-screen',
                           'promoted': False, 'screen': selected})


def qualify():
    selected = json.loads((EVIDENCE / 'selected.json').read_text())['version']
    source = SOURCES / f'Submission_{selected}.lean'
    code, text = execute(['python3', 'evaluation/run.py', '--problem', 'mertens',
                          '--submission', str(source), '--timeout', '180'],
                         'canonical.log', cwd=UPSTREAM, timeout=1800)
    passed = code == 0 and ': accepted' in text and re.search(r'Computation replay T:.*complete', text) is not None
    passed = passed and all(re.search(rf'n={n}: OK\b', text) for n in INPUTS)
    save('canonical.json', {'version': selected, 'accepted_complete': bool(passed),
                            'local_unseeded_only': True, 'returncode': code})
    if not passed:
...
def instructions():
    ...
    result.update(local_promotion_eligible=promote, official_pmu_measured=False,
