...
def partitionDirect (n : Nat) : Nat :=
  (buildRowsDirect n n).getD n 0

def impl : Nat → Nat := partitionDirect

theorem impl_correct : ∀ n, impl n = partitionSpec n := by
  intro n
  unfold impl partitionDirect partitionSpec
  rw [buildRowsDirect_eq, buildRowsShift_eq]
  exact buildRows_getD n n n (Nat.le_refl n)

end Submission
'''
p=OUT/"Submission_v9.lean"
p.write_text(text)
