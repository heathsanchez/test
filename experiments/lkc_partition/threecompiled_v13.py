...
/-! V12: universally seed the complete part-size-2 row. -/

theorem partAux_two (m : Nat) : partAux 2 m = m / 2 + 1 := by
  unfold partAux
  change
...
def twosRow (n : Nat) : List Nat :=
  (List.range (n + 1)).map fun m => m / 2 + 1

theorem twosRow_getD (n m : Nat) (hm : m ≤ n) :
    (twosRow n).getD m 0 = partAux 2 m := by
  rw [twosRow, range_map_getD (h := by omega), partAux_two]

theorem twosRow_eq_buildRowsSeeded_two (n : Nat) :
    twosRow n = buildRowsSeeded 2 n := by
  apply List.ext_getElem
  · rw [buildRowsSeeded_eq, buildRowsDirect_eq, buildRowsShift_eq,
      buildRows_length]
    simp [twosRow]
  · intro i hi ht
    have him : i ≤ n := by
      simp [twosRow] at hi
      omega
    have hl := twosRow_getD n i him
    have hr :
        (buildRowsSeeded 2 n).getD i 0 = partAux 2 i := by
      rw [buildRowsSeeded_eq, buildRowsDirect_eq, buildRowsShift_eq]
      exact buildRows_getD 2 n i him
    have hd :
        (twosRow n).getD i 0 =
          (buildRowsSeeded 2 n).getD i 0 :=
      hl.trans hr.symm
    have hleft :
        (twosRow n).getD i 0 = (twosRow n)[i] := by
      simp [List.getD_eq_getElem?_getD, hi]
    have hright :
        (buildRowsSeeded 2 n).getD i 0 =
          (buildRowsSeeded 2 n)[i] := by
      simp [List.getD_eq_getElem?_getD, ht]
    exact hleft.symm.trans (hd.trans hright)

def buildAboveTwo : Nat → Nat → List Nat
  | 0, n => twosRow n
  | r + 1, n => nextRowSeeded (r + 2) n (buildAboveTwo r n)

theorem buildAboveTwo_eq :
    ∀ r n, buildAboveTwo r n = buildRowsSeeded (r + 2) n
  | 0, n => twosRow_eq_buildRowsSeeded_two n
  | r + 1, n => by
      simp only [buildAboveTwo]
      rw [buildAboveTwo_eq r n]
      rfl

/-! V13: compile the proved part-size-2 law into the full part-size-3 row. -/

def threeValue (m : Nat) : Nat :=
  sumFold (m / 3 + 1) (fun j => (m - j * 3) / 2 + 1)

theorem partAux_three (m : Nat) : partAux 3 m = threeValue m := by
  unfold threeValue
  unfold partAux
  change
    ((List.range (m / 3 + 1)).map
      (fun j => partAux 2 (m - j * 3))).foldl (· + ·) 0 =
      sumFold (m / 3 + 1) (fun j => (m - j * 3) / 2 + 1)
  rw [sumFold_eq_list]
...
def threesRow (n : Nat) : List Nat :=
  (List.range (n + 1)).map threeValue

theorem threesRow_getD (n m : Nat) (hm : m ≤ n) :
    (threesRow n).getD m 0 = partAux 3 m := by
  rw [threesRow, range_map_getD (h := by omega), partAux_three]

theorem threesRow_eq_buildRowsSeeded_three (n : Nat) :
    threesRow n = buildRowsSeeded 3 n := by
  apply List.ext_getElem
  · rw [buildRowsSeeded_eq, buildRowsDirect_eq, buildRowsShift_eq,
      buildRows_length]
    simp [threesRow]
  · intro i hi ht
    have him : i ≤ n := by
      simp [threesRow] at hi
      omega
    have hl := threesRow_getD n i him
    have hr :
        (buildRowsSeeded 3 n).getD i 0 = partAux 3 i := by
      rw [buildRowsSeeded_eq, buildRowsDirect_eq, buildRowsShift_eq]
      exact buildRows_getD 3 n i him
    have hd :
        (threesRow n).getD i 0 =
          (buildRowsSeeded 3 n).getD i 0 :=
      hl.trans hr.symm
    have hleft :
        (threesRow n).getD i 0 = (threesRow n)[i] := by
      simp [List.getD_eq_getElem?_getD, hi]
    have hright :
        (buildRowsSeeded 3 n).getD i 0 =
...
def buildAboveThree : Nat → Nat → List Nat
  | 0, n => threesRow n
  | r + 1, n => nextRowSeeded (r + 3) n (buildAboveThree r n)

theorem buildAboveThree_eq :
    ∀ r n, buildAboveThree r n = buildRowsSeeded (r + 3) n
  | 0, n => threesRow_eq_buildRowsSeeded_three n
  | r + 1, n => by
      simp only [buildAboveThree]
      rw [buildAboveThree_eq r n]
      rfl

def partitionThree : Nat → Nat
  | 0 => 1
  | 1 => 1
  | 2 => 2
  | n + 3 => (buildAboveThree n (n + 3)).getD (n + 3) 0

def partitionTwo : Nat → Nat
  | Nat.zero => 1
  | Nat.succ Nat.zero => 1
  | Nat.succ (Nat.succ n) =>
      (buildAboveTwo n (n + 2)).getD (n + 2) 0
...
def impl : Nat → Nat := partitionThree
...
theorem impl_correct : ∀ n, impl n = partitionSpec n := by
  ...
  | succ n =>
      cases n with
      | zero =>
          change 1 = partAux 1 1
          exact (partAux_one 1).symm
      | succ n =>
          cases n with
          | zero =>
              change 2 = partAux 2 2
              rw [partAux_two]
          | succ m =>
              change (buildAboveThree m (m + 3)).getD (m + 3) 0 =
                partAux (m + 3) (m + 3)
              rw [buildAboveThree_eq]
              rw [buildRowsSeeded_eq, buildRowsDirect_eq, buildRowsShift_eq]
              exact buildRows_getD (m + 3) (m + 3) (m + 3) (Nat.le_refl _)

end Submission
'''
p=OUT/"Submission_v13.lean"
p.write_text(text)
