#!/usr/bin/env python3
"""Protected Primecount V1 versus composed fast-primality V2."""
from __future__ import annotations
import argparse, hashlib, json, re, subprocess, sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
SOURCES = ROOT / "experiments/lkc_primecount/generated"
BENCH = ROOT / "bench"
UPSTREAM = BENCH / ".cache/upstream"
PACKAGE = UPSTREAM / "evaluation/problems/primecount"
EVIDENCE = ROOT / "primecount-v2-evidence"
...
def prove():
    source = SOURCES / "Submission_v2.lean"
    ...
    code, text = execute(
        ...
        "proof.log", cwd=PACKAGE, timeout=1200
    )
    ...
    if passed:
        ...
        bad_code, bad_text = execute(
            ...
            "negative.log", cwd=PACKAGE, timeout=1200
        )
    ...
    verdict = {
        ...
        "protected_v1_sha256": hashlib.sha256((SOURCES / "Submission_v1.lean").read_bytes()).hexdigest(),
    }
    save("proof-gate.json", verdict)
    if not passed:
        raise RuntimeError("Primecount V2 proof gate failed.")
...
def diagnostic(metric, label):
    ...
    if code != 0 or not data.get("complete"):
        raise RuntimeError(f"Primecount V2 incomplete {metric} diagnostic")
    return {
        "metric": metric,
        "baseline": "protected-primecount-v1",
        "totals": data["totals"],
...
def qualify():
    ...
    if not passed:
        raise RuntimeError("Primecount V2 canonical qualification failed.")
...
def instructions():
    ...
    if proof["source_sha256"] != result["candidate_sha256"]:
        raise RuntimeError("Measured candidate bytes differ from proof-checked bytes")
    if proof["protected_v1_sha256"] != result["baseline_sha256"]:
        raise RuntimeError("Protected V1 bytes drifted")
    result.update(
