...
/-! V10: 16-cell grain, using V8's verified 8-cell blocks compositionally. -/

def advance16 (x : Nat) : Nat :=
  advance8 (advance8 x)

def pack14 (x : Nat) : Nat :=
  pack8 x + 256 * pack6 (advance8 x)

def pack16 (x : Nat) : Nat :=
  pack8 x + 256 * pack8 (advance8 x)

def pack16Tail : Nat → Nat → Nat
  | x, 0 => pack14 x
  | x, n + 1 => pack16 x + 65536 * pack16Tail (advance16 x) n

theorem packMixBit_fourteen (x : Nat) :
    packMixBit x 14 = pack14 x := by
  unfold pack14
  rw [show 14 = 6 + 8 by omega, packMixBit_eight x 6, packMixBit_six]

theorem packMixBit_sixteen (x k : Nat) :
    packMixBit x (k + 16) =
      pack16 x + 65536 * packMixBit (advance16 x) k := by
  unfold pack16 advance16
  rw [show k + 16 = (k + 8) + 8 by omega]
  rw [packMixBit_eight x (k + 8)]
  rw [packMixBit_eight (advance8 x) k]
  omega

theorem pack16Tail_eq (x n : Nat) :
    pack16Tail x n = packMixBit x (16 * n + 14) := by
  induction n generalizing x with
  | zero =>
      simp only [pack16Tail, Nat.mul_zero, Nat.zero_add]
      exact (packMixBit_fourteen x).symm
  | succ n ih =>
      unfold pack16Tail
      rw [ih (advance16 x)]
      have hn : 16 * (n + 1) + 14 = (16 * n + 14) + 16 := by omega
      rw [hn]
      exact (packMixBit_sixteen x (16 * n + 14)).symm

def initPacked16 (seed : Nat) : Nat :=
  1 + 4 * pack16Tail (seed + 3 * stepConst) 15

theorem initPacked16_eq (seed : Nat) :
    initPacked16 seed = initPackedFastBit seed := by
  unfold initPacked16 initPackedFastBit
  rw [pack16Tail_eq]

def impl : Nat → Nat := fun n =>
  biterFast (caSteps n) (initPacked16 (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPacked16_eq, initPackedFastBit_eq, initPackedFast_eq]
