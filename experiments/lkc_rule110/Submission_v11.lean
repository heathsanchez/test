...
/-! V11: 4-cell grain search point. -/

def advance4 (x : Nat) : Nat :=
  let x1 := x + stepConst
  let x2 := x1 + stepConst
  let x3 := x2 + stepConst
  x3 + stepConst
...
def pack4Tail : Nat → Nat → Nat
  | x, 0 => pack2 x
  | x, n + 1 => pack4 x + 16 * pack4Tail (advance4 x) n

theorem packMixBit_two (x : Nat) :
    packMixBit x 2 = pack2 x := by
  unfold pack2
  simp only [packMixBit]
  omega

theorem packMixBit_four (x k : Nat) :
    packMixBit x (k + 4) =
      pack4 x + 16 * packMixBit (advance4 x) k := by
  unfold pack4 advance4
  simp only [packMixBit]
  omega

theorem pack4Tail_eq (x n : Nat) :
    pack4Tail x n = packMixBit x (4 * n + 2) := by
  induction n generalizing x with
  | zero =>
      simp only [pack4Tail, Nat.mul_zero, Nat.zero_add]
      exact (packMixBit_two x).symm
  | succ n ih =>
      unfold pack4Tail
      rw [ih (advance4 x)]
      have hn : 4 * (n + 1) + 2 = (4 * n + 2) + 4 := by omega
      rw [hn]
      exact (packMixBit_four x (4 * n + 2)).symm

def initPacked4 (seed : Nat) : Nat :=
  1 + 4 * pack4Tail (seed + 3 * stepConst) 63

theorem initPacked4_eq (seed : Nat) :
    initPacked4 seed = initPackedFastBit seed := by
  unfold initPacked4 initPackedFastBit
  rw [pack4Tail_eq]

def impl : Nat → Nat := fun n =>
  biterFast (caSteps n) (initPacked4 (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPacked4_eq, initPackedFastBit_eq, initPackedFast_eq]
