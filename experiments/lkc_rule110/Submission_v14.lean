...
/-! V14: replace the mixer's 32-bit AND truncation with exact mod 2^32. -/

def mixBit31Mod (x : Nat) : Bool :=
  let y := ((x ^^^ (x >>> 16)) * 0x7feb352d) % 0x100000000
  ((y ^^^ (y >>> 15)) * 0x846ca68b).testBit 31

theorem mixBit31Mod_eq (x : Nat) :
    mixBit31Mod x = mixBit31 x := by
  unfold mixBit31Mod mixBit31
  have hm : (0xffffffff : Nat) = 2 ^ 32 - 1 := by decide
  have hp : (0x100000000 : Nat) = 2 ^ 32 := by decide
  rw [hm, Nat.and_two_pow_sub_one_eq_mod, hp]

def pack8Mod (x : Nat) : Nat :=
  let x1 := x + stepConst
  let x2 := x1 + stepConst
  let x3 := x2 + stepConst
  let x4 := x3 + stepConst
  ...
  let x6 := x5 + stepConst
  let x7 := x6 + stepConst
  (if mixBit31Mod x  then 1 else 0) +
  2   * (if mixBit31Mod x1 then 1 else 0) +
  4   * (if mixBit31Mod x2 then 1 else 0) +
  8   * (if mixBit31Mod x3 then 1 else 0) +
  16  * (if mixBit31Mod x4 then 1 else 0) +
  32  * (if mixBit31Mod x5 then 1 else 0) +
  64  * (if mixBit31Mod x6 then 1 else 0) +
  128 * (if mixBit31Mod x7 then 1 else 0)

def pack6Mod (x : Nat) : Nat :=
  let x1 := x + stepConst
  let x2 := x1 + stepConst
  let x3 := x2 + stepConst
  let x4 := x3 + stepConst
...
theorem pack8Mod_eq (x : Nat) :
    pack8Mod x = pack8 x := by
  unfold pack8Mod pack8
  simp only [mixBit31Mod_eq]

theorem pack6Mod_eq (x : Nat) :
    pack6Mod x = pack6 x := by
  unfold pack6Mod pack6
  simp only [mixBit31Mod_eq]

def packByteTailMod : Nat → Nat → Nat
  | x, 0 => pack6Mod x
  | x, n + 1 => pack8Mod x + 256 * packByteTailMod (advance8 x) n

theorem packByteTailMod_eq (x n : Nat) :
    packByteTailMod x n = packByteTail x n := by
  induction n generalizing x with
  | zero =>
      unfold packByteTailMod packByteTail
      exact pack6Mod_eq x
  | succ n ih =>
      unfold packByteTailMod packByteTail
      rw [pack8Mod_eq, ih]

def initPackedByteMod (seed : Nat) : Nat :=
  1 + 4 * packByteTailMod (seed + 3 * stepConst) 31

theorem initPackedByteMod_eq (seed : Nat) :
    initPackedByteMod seed = initPackedByte seed := by
  unfold initPackedByteMod initPackedByte
  rw [packByteTailMod_eq]

def impl : Nat → Nat := fun n =>
  biterFast (caSteps n) (initPackedByteMod (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPackedByteMod_eq, initPackedByte_eq, initPackedFastBit_eq, initPackedFast_eq]
