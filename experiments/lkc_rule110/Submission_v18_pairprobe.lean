...
def impl : Nat → Nat := fun n =>
  biterFast (caSteps n) (initPackedByteNat (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPackedByteNat_eq, initPackedByte_eq, initPackedFastBit_eq, initPackedFast_eq]
...
/-!
V18 proof probe: two-lane packed constant multiplication.

This does not yet alter the measured implementation; it establishes the reusable exact SIMD lemma first.
-/
