...
def packByteTailSWARShared : Nat → Nat → Nat
  | x, 0 => pack6SWAR x
  | x, n + 1 =>
      let x1 := x + stepConst
      let x2 := x1 + stepConst
      ...
      16 * Vec2.mixPairSWAR x4 x5 +
      64 * Vec2.mixPairSWAR x6 x7 +
      256 * packByteTailSWARShared x8 n
...
def initPackedSWARShared (seed : Nat) : Nat :=
  1 + 4 * packByteTailSWARShared (seed + 3 * stepConst) 31

theorem initPackedSWARShared_eq (seed : Nat) :
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPackedSWARShared_eq, initPackedSWAR_eq, initPackedByteNat_eq, initPackedByte_eq, initPackedFastBit_eq, initPackedFast_eq]
