...
/-! V23: compile the arithmetic progression into the packed pair state itself. -/

def mixPairPacked (p0 : Nat) : Nat :=
  let u := (p0 ^^^ (p0 >>> 16)) &&& Vec2.mask2
  let yv := (u * Vec2.c1) &&& Vec2.mask2
  let v := (yv ^^^ (yv >>> 15)) &&& Vec2.mask2
  let p := v * Vec2.c2
  (p >>> 31) % 2 + 2 * ((p >>> 95) % 2)

theorem mixPairPacked_eq (x y : Nat) :
    mixPairPacked (Vec2.pack2 x y) = Vec2.mixPairSWAR x y := by
  rfl

def pairState (x : Nat) : Nat :=
  Vec2.pack2 x (x + stepConst)

def pairDelta : Nat :=
  2 * stepConst + ((2 * stepConst) <<< 64)

def advancePair (p : Nat) : Nat :=
  p + pairDelta

theorem advancePair_state (x : Nat) :
    advancePair (pairState x) = pairState (x + 2 * stepConst) := by
  unfold advancePair pairState pairDelta Vec2.pack2
...
def pack8PackedPair (p : Nat) : Nat :=
  ...
  let p3 := advancePair p2
  mixPairPacked p +
  4  * mixPairPacked p1 +
  16 * mixPairPacked p2 +
  64 * mixPairPacked p3

def pack6PackedPair (p : Nat) : Nat :=
  let p1 := advancePair p
  let p2 := advancePair p1
  mixPairPacked p +
  4  * mixPairPacked p1 +
...
def packPackedPairTail : Nat → Nat → Nat
  ...
  | p, n + 1 =>
      pack8PackedPair p + 256 * packPackedPairTail (advance4Pairs p) n

theorem packPackedPairTail_eq (x n : Nat) :
    packPackedPairTail (pairState x) n = packByteTailSWAR x n := by
  induction n generalizing x with
  | zero =>
      simp only [packPackedPairTail, packByteTailSWAR]
      exact pack6PackedPair_eq x
  | succ n ih =>
      simp only [packPackedPairTail, packByteTailSWAR]
      rw [pack8PackedPair_eq, advance4Pairs_state, ih]

def initPackedPairProgression (seed : Nat) : Nat :=
  1 + 4 * packPackedPairTail (pairState (seed + 3 * stepConst)) 31

theorem initPackedPairProgression_eq (seed : Nat) :
    initPackedPairProgression seed = initPackedSWAR seed := by
  unfold initPackedPairProgression initPackedSWAR
  rw [packPackedPairTail_eq]

def impl : Nat → Nat := fun n =>
  biterFast (caSteps n) (initPackedPairProgression (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPackedPairProgression_eq, initPackedSWAR_eq, initPackedByteNat_eq, initPackedByte_eq, initPackedFastBit_eq, initPackedFast_eq]
