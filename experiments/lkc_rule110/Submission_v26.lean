...
def mask16 : Nat :=
  pack512 Vec8.mask8 Vec8.mask8

def pack16v
    (a b c d e f g h i j k l m n o p : Nat) : Nat :=
  pack512
    (Vec8.pack8v a b c d e f g h)
    (Vec8.pack8v i j k l m n o p)

theorem mask8_lt512 : Vec8.mask8 < 2 ^ 512 := by decide

theorem testBit_pack512 (p q i : Nat) (hp : p < 2 ^ 512) :
    (pack512 p q).testBit i =
      if i < 512 then p.testBit i else q.testBit (i - 512) := by
  unfold pack512
  rw [Nat.add_comm, Nat.shiftLeft_eq, Nat.mul_comm]
  exact Nat.testBit_two_pow_mul_add q hp i

theorem testBit_mask16 (i : Nat) :
    mask16.testBit i =
      if i < 512 then Vec8.mask8.testBit i else Vec8.mask8.testBit (i - 512) := by
  unfold mask16
  exact testBit_pack512 Vec8.mask8 Vec8.mask8 i mask8_lt512

def octStage (s p : Nat) : Nat :=
  (p ^^^ (p >>> s)) &&& Vec8.mask8

theorem octStage_lt512 (s p : Nat) :
    octStage s p < 2 ^ 512 := by
  unfold octStage
  have hle : ((p ^^^ (p >>> s)) &&& Vec8.mask8) ≤ Vec8.mask8 := Nat.and_le_right
  exact Nat.lt_of_le_of_lt hle mask8_lt512

theorem mask8_true_lt480 {i : Nat} (hm : Vec8.mask8.testBit i = true) :
    i < 480 := by
  rw [Vec8.testBit_mask8] at hm
  by_cases h256 : i < 256
  · rw [if_pos h256] at hm
    have h := Vec8.mask4_true_lt224 hm
    omega
  · rw [if_neg h256] at hm
    have h := Vec8.mask4_true_lt224 hm
    omega

theorem lift_oct_stage (p q s : Nat)
    (hp : p < 2 ^ 512) (hs0 : 0 < s) (hs : s ≤ 32) :
    ((pack512 p q ^^^ (pack512 p q >>> s)) &&& mask16) =
      pack512 (octStage s p) (octStage s q) := by
  apply Nat.eq_of_testBit_eq
  intro i
  rw [Nat.testBit_and, testBit_mask16]
  rw [testBit_pack512 (octStage s p) (octStage s q) i (octStage_lt512 s p)]
  simp only [octStage, Nat.testBit_and, Nat.testBit_xor, Nat.testBit_shiftRight]
  by_cases h512 : i < 512
  · rw [if_pos h512, if_pos h512]
    by_cases hm : Vec8.mask8.testBit i = true
    · have hi480 : i < 480 := mask8_true_lt480 hm
      have his : s + i < 512 := by omega
      rw [testBit_pack512 p q i hp, testBit_pack512 p q (s + i) hp]
      simp [h512, his, hm]
    · have hmf : Vec8.mask8.testBit i = false := by
        cases h : Vec8.mask8.testBit i <;> simp_all
      simp [hmf]
  · rw [if_neg h512, if_neg h512]
    have hi512 : 512 ≤ i := by omega
    let j := i - 512
    have hij : i = 512 + j := by dsimp [j]; omega
    by_cases hm : Vec8.mask8.testBit j = true
    · have hj480 : j < 480 := mask8_true_lt480 hm
      have hsj : s + j < 512 := by omega
      have hsihi : ¬s + i < 512 := by omega
      have hsub : (s + i) - 512 = s + j := by rw [hij]; omega
      rw [testBit_pack512 p q i hp, testBit_pack512 p q (s + i) hp]
      simp [h512, hi512, hsihi, j, hsub, hm]
    · have hmf : Vec8.mask8.testBit j = false := by
        cases h : Vec8.mask8.testBit j <;> simp_all
      simp [j, hmf]

theorem pack512_mul (p q k : Nat) :
    pack512 p q * k = pack512 (p * k) (q * k) := by
  unfold pack512
  rw [Nat.add_mul, Nat.shiftLeft_eq, Nat.shiftLeft_eq]
  congr 1
  calc
    (q * 2 ^ 512) * k = q * (2 ^ 512 * k) := Nat.mul_assoc _ _ _
    _ = q * (k * 2 ^ 512) := by rw [Nat.mul_comm (2 ^ 512) k]
    _ = (q * k) * 2 ^ 512 := (Nat.mul_assoc _ _ _).symm

theorem pack256_lt512 {p q : Nat} (hp : p < 2 ^ 256) (hq : q < 2 ^ 256) :
    Vec8.pack256 p q < 2 ^ 512 := by
  unfold Vec8.pack256
  rw [Nat.shiftLeft_eq]
  have hp' : p ≤ 2 ^ 256 - 1 := by omega
  have hq' : q ≤ 2 ^ 256 - 1 := by omega
  have hm := Nat.mul_le_mul_right (2 ^ 256) hq'
  calc
    p + q * 2 ^ 256 ≤ (2 ^ 256 - 1) + (2 ^ 256 - 1) * 2 ^ 256 :=
      Nat.add_le_add hp' hm
    _ < 2 ^ 512 := by decide

theorem pack8v_lt512
    {a b c d e f g h : Nat}
    (ha : a < 2 ^ 64) (hb : b < 2 ^ 64)
    (hc : c < 2 ^ 64) (hd : d < 2 ^ 64)
    (he : e < 2 ^ 64) (hf : f < 2 ^ 64)
    (hg : g < 2 ^ 64) (hh : h < 2 ^ 64) :
    Vec8.pack8v a b c d e f g h < 2 ^ 512 := by
  unfold Vec8.pack8v
  exact pack256_lt512
    (Vec8.pack4_lt256 ha hb hc hd)
    (Vec8.pack4_lt256 he hf hg hh)

theorem and_mask8_lt512 (x : Nat) :
    (x &&& Vec8.mask8) < 2 ^ 512 := by
  have hle : (x &&& Vec8.mask8) ≤ Vec8.mask8 := Nat.and_le_right
  exact Nat.lt_of_le_of_lt hle mask8_lt512

theorem mask_pack512 (p q : Nat) (hp : p < 2 ^ 512) :
    (pack512 p q &&& mask16) =
      pack512 (p &&& Vec8.mask8) (q &&& Vec8.mask8) := by
  apply Nat.eq_of_testBit_eq
  intro i
  rw [Nat.testBit_and, testBit_mask16]
  rw [testBit_pack512 (p &&& Vec8.mask8) (q &&& Vec8.mask8) i (and_mask8_lt512 p)]
  rw [testBit_pack512 p q i hp]
  simp only [Nat.testBit_and]
  by_cases h : i < 512
  · simp [h]
  · simp [h]

theorem pack8v_mul (a b c d e f g h k : Nat) :
    Vec8.pack8v a b c d e f g h * k =
      Vec8.pack8v (a * k) (b * k) (c * k) (d * k)
        (e * k) (f * k) (g * k) (h * k) := by
  unfold Vec8.pack8v
  rw [Vec8.pack256_mul, Vec8.pack4_mul, Vec8.pack4_mul]

theorem octU_eq (a b c d e f g h : Nat)
    (ha : a < 2 ^ 64) (hb : b < 2 ^ 64)
    (hc : c < 2 ^ 64) (hd : d < 2 ^ 64)
    (he : e < 2 ^ 64) (hf : f < 2 ^ 64)
    (hg : g < 2 ^ 64) (hh : h < 2 ^ 64) :
    octStage 16 (Vec8.pack8v a b c d e f g h) =
      Vec8.pack8v (Vec8.u32 a) (Vec8.u32 b) (Vec8.u32 c) (Vec8.u32 d)
        (Vec8.u32 e) (Vec8.u32 f) (Vec8.u32 g) (Vec8.u32 h) := by
  unfold octStage Vec8.pack8v
  rw [Vec8.lift_quad_stage
        (Vec8.pack4 a b c d) (Vec8.pack4 e f g h) 16
        (Vec8.pack4_lt256 ha hb hc hd) (by decide) (by decide)]
  rw [Vec8.quadU_eq a b c d ha hb hc hd, Vec8.quadU_eq e f g h he hf hg hh]

theorem octU_mul_c1_lt512 (a b c d e f g h : Nat)
    (ha : a < 2 ^ 64) (hb : b < 2 ^ 64)
    (hc : c < 2 ^ 64) (hd : d < 2 ^ 64)
    (he : e < 2 ^ 64) (hf : f < 2 ^ 64)
    (hg : g < 2 ^ 64) (hh : h < 2 ^ 64) :
    octStage 16 (Vec8.pack8v a b c d e f g h) * Vec8.c1 < 2 ^ 512 := by
  rw [octU_eq a b c d e f g h ha hb hc hd he hf hg hh, pack8v_mul]
  exact pack8v_lt512
    (Vec8.u32_mul_c1_lt64 a) (Vec8.u32_mul_c1_lt64 b)
    (Vec8.u32_mul_c1_lt64 c) (Vec8.u32_mul_c1_lt64 d)
    (Vec8.u32_mul_c1_lt64 e) (Vec8.u32_mul_c1_lt64 f)
    (Vec8.u32_mul_c1_lt64 g) (Vec8.u32_mul_c1_lt64 h)

theorem octY_eq (a b c d e f g h : Nat)
    (ha : a < 2 ^ 64) (hb : b < 2 ^ 64)
    (hc : c < 2 ^ 64) (hd : d < 2 ^ 64)
    (he : e < 2 ^ 64) (hf : f < 2 ^ 64)
    (hg : g < 2 ^ 64) (hh : h < 2 ^ 64) :
    (octStage 16 (Vec8.pack8v a b c d e f g h) * Vec8.c1) &&& Vec8.mask8 =
      Vec8.pack8v (Vec8.y32 a) (Vec8.y32 b) (Vec8.y32 c) (Vec8.y32 d)
        (Vec8.y32 e) (Vec8.y32 f) (Vec8.y32 g) (Vec8.y32 h) := by
  simpa [octStage] using Vec8.packed_y8 a b c d e f g h ha hb hc hd he hf hg hh
...
theorem packed_y16
    ...
    (hm : m < 2 ^ 64) (hn : n < 2 ^ 64)
    (ho : o < 2 ^ 64) (hp : p < 2 ^ 64) :
    ((((pack16v a b c d e f g h i j k l m n o p ^^^
        (pack16v a b c d e f g h i j k l m n o p >>> 16)) &&& mask16) * Vec8.c1) &&& mask16) =
      pack16v
        (Vec8.y32 a) (Vec8.y32 b) (Vec8.y32 c) (Vec8.y32 d)
        (Vec8.y32 e) (Vec8.y32 f) (Vec8.y32 g) (Vec8.y32 h)
        (Vec8.y32 i) (Vec8.y32 j) (Vec8.y32 k) (Vec8.y32 l)
        (Vec8.y32 m) (Vec8.y32 n) (Vec8.y32 o) (Vec8.y32 p) := by
  unfold pack16v
  rw [lift_oct_stage
        (Vec8.pack8v a b c d e f g h) (Vec8.pack8v i j k l m n o p) 16
        (pack8v_lt512 ha hb hc hd he hf hg hh) (by decide) (by decide)]
  rw [pack512_mul]
  rw [mask_pack512 _ _
        (octU_mul_c1_lt512 a b c d e f g h ha hb hc hd he hf hg hh)]
  rw [octY_eq a b c d e f g h ha hb hc hd he hf hg hh,
      octY_eq i j k l m n o p hi hj hk hl hm hn ho hp]

theorem octV_eq (a b c d e f g h : Nat)
    (ha : a < 2 ^ 64) (hb : b < 2 ^ 64)
    (hc : c < 2 ^ 64) (hd : d < 2 ^ 64)
    (he : e < 2 ^ 64) (hf : f < 2 ^ 64)
    (hg : g < 2 ^ 64) (hh : h < 2 ^ 64) :
    octStage 15 (Vec8.pack8v a b c d e f g h) =
      Vec8.pack8v (Vec8.v32 a) (Vec8.v32 b) (Vec8.v32 c) (Vec8.v32 d)
        (Vec8.v32 e) (Vec8.v32 f) (Vec8.v32 g) (Vec8.v32 h) := by
  simpa [octStage] using Vec8.packed_v8 a b c d e f g h ha hb hc hd he hf hg hh
...
theorem packed_v16
    ...
    (ho : o < 2 ^ 64) (hp : p < 2 ^ 64) :
    ((pack16v a b c d e f g h i j k l m n o p ^^^
        (pack16v a b c d e f g h i j k l m n o p >>> 15)) &&& mask16) =
      pack16v
        (Vec8.v32 a) (Vec8.v32 b) (Vec8.v32 c) (Vec8.v32 d)
        (Vec8.v32 e) (Vec8.v32 f) (Vec8.v32 g) (Vec8.v32 h)
        (Vec8.v32 i) (Vec8.v32 j) (Vec8.v32 k) (Vec8.v32 l)
        (Vec8.v32 m) (Vec8.v32 n) (Vec8.v32 o) (Vec8.v32 p) := by
  unfold pack16v
  rw [lift_oct_stage
        (Vec8.pack8v a b c d e f g h) (Vec8.pack8v i j k l m n o p) 15
        (pack8v_lt512 ha hb hc hd he hf hg hh) (by decide) (by decide)]
  rw [octV_eq a b c d e f g h ha hb hc hd he hf hg hh,
      octV_eq i j k l m n o p hi hj hk hl hm hn ho hp]

theorem packed_p16 (a b c d e f g h i j k l m n o p : Nat) :
    pack16v
        (Vec8.v32 a) (Vec8.v32 b) (Vec8.v32 c) (Vec8.v32 d)
        (Vec8.v32 e) (Vec8.v32 f) (Vec8.v32 g) (Vec8.v32 h)
        (Vec8.v32 i) (Vec8.v32 j) (Vec8.v32 k) (Vec8.v32 l)
        (Vec8.v32 m) (Vec8.v32 n) (Vec8.v32 o) (Vec8.v32 p) * Vec8.c2 =
      pack16v
        (Vec8.v32 a * Vec8.c2) (Vec8.v32 b * Vec8.c2)
        (Vec8.v32 c * Vec8.c2) (Vec8.v32 d * Vec8.c2)
        (Vec8.v32 e * Vec8.c2) (Vec8.v32 f * Vec8.c2)
        (Vec8.v32 g * Vec8.c2) (Vec8.v32 h * Vec8.c2)
        (Vec8.v32 i * Vec8.c2) (Vec8.v32 j * Vec8.c2)
        (Vec8.v32 k * Vec8.c2) (Vec8.v32 l * Vec8.c2)
        (Vec8.v32 m * Vec8.c2) (Vec8.v32 n * Vec8.c2)
        (Vec8.v32 o * Vec8.c2) (Vec8.v32 p * Vec8.c2) := by
  unfold pack16v
  rw [pack512_mul, pack8v_mul, pack8v_mul]

def extract8 (p : Nat) : Nat :=
  ((p >>> 31) % 2) +
  2 * ((p >>> 95) % 2) +
  4 * ((p >>> 159) % 2) +
  8 * ((p >>> 223) % 2) +
...
def scalar8 (a b c d e f g h : Nat) : Nat :=
  Vec8.mixScalarNat a + 2 * Vec8.mixScalarNat b +
  4 * Vec8.mixScalarNat c + 8 * Vec8.mixScalarNat d +
  16 * Vec8.mixScalarNat e + 32 * Vec8.mixScalarNat f +
  64 * Vec8.mixScalarNat g + 128 * Vec8.mixScalarNat h

theorem extract8_final_eq (a b c d e f g h : Nat)
    (ha : a < 2 ^ 64) (hb : b < 2 ^ 64)
    (hc : c < 2 ^ 64) (hd : d < 2 ^ 64)
    (he : e < 2 ^ 64) (hf : f < 2 ^ 64)
    (hg : g < 2 ^ 64) (hh : h < 2 ^ 64) :
    extract8
      (Vec8.pack8v
        (Vec8.v32 (Vec8.y32 a) * Vec8.c2) (Vec8.v32 (Vec8.y32 b) * Vec8.c2)
        (Vec8.v32 (Vec8.y32 c) * Vec8.c2) (Vec8.v32 (Vec8.y32 d) * Vec8.c2)
        (Vec8.v32 (Vec8.y32 e) * Vec8.c2) (Vec8.v32 (Vec8.y32 f) * Vec8.c2)
        (Vec8.v32 (Vec8.y32 g) * Vec8.c2) (Vec8.v32 (Vec8.y32 h) * Vec8.c2)) =
      scalar8 a b c d e f g h := by
  have hx := Vec8.mix8SWAR_eq a b c d e f g h ha hb hc hd he hf hg hh
  simp only [Vec8.mix8SWAR] at hx
  rw [Vec8.packed_y8 a b c d e f g h ha hb hc hd he hf hg hh] at hx
  rw [Vec8.packed_v8
        (Vec8.y32 a) (Vec8.y32 b) (Vec8.y32 c) (Vec8.y32 d)
        (Vec8.y32 e) (Vec8.y32 f) (Vec8.y32 g) (Vec8.y32 h)
        (Vec8.y32_lt64 a) (Vec8.y32_lt64 b) (Vec8.y32_lt64 c) (Vec8.y32_lt64 d)
        (Vec8.y32_lt64 e) (Vec8.y32_lt64 f) (Vec8.y32_lt64 g) (Vec8.y32_lt64 h)] at hx
  rw [Vec8.packed_p8] at hx
  simpa [extract8, scalar8] using hx

theorem bitAt_pack512_low (p q i : Nat) (hp : p < 2 ^ 512) (hi : i < 512) :
    ((pack512 p q >>> i) % 2) = ((p >>> i) % 2) := by
  have h := congrArg Bool.toNat (testBit_pack512 p q i hp)
  rw [if_pos hi] at h
  simpa [Nat.toNat_testBit, Nat.shiftRight_eq_div_pow] using h

theorem extract8_pack512_low (p q : Nat) (hp : p < 2 ^ 512) :
    extract8 (pack512 p q) = extract8 p := by
  unfold extract8
  rw [bitAt_pack512_low p q 31 hp (by decide)]
  rw [bitAt_pack512_low p q 95 hp (by decide)]
  rw [bitAt_pack512_low p q 159 hp (by decide)]
  rw [bitAt_pack512_low p q 223 hp (by decide)]
  rw [bitAt_pack512_low p q 287 hp (by decide)]
  rw [bitAt_pack512_low p q 351 hp (by decide)]
  rw [bitAt_pack512_low p q 415 hp (by decide)]
  rw [bitAt_pack512_low p q 479 hp (by decide)]

theorem shift512_pack512 (p q : Nat) (hp : p < 2 ^ 512) :
    pack512 p q >>> 512 = q := by
...
def mix16SWAR
    (a b c d e f g h i j k l m n o p : Nat) : Nat :=
  let p0 := pack16v a b c d e f g h i j k l m n o p
  let u := (p0 ^^^ (p0 >>> 16)) &&& mask16
  let y := (u * Vec8.c1) &&& mask16
  let v := (y ^^^ (y >>> 15)) &&& mask16
  let z := v * Vec8.c2
  extract8 z + 256 * extract8 (z >>> 512)

theorem mix16SWAR_eq
    (a b c d e f g h i j k l m n o p : Nat)
    (ha : a < 2 ^ 64) (hb : b < 2 ^ 64)
    (hc : c < 2 ^ 64) (hd : d < 2 ^ 64)
    (he : e < 2 ^ 64) (hf : f < 2 ^ 64)
    ...
    (hm : m < 2 ^ 64) (hn : n < 2 ^ 64)
    (ho : o < 2 ^ 64) (hp : p < 2 ^ 64) :
    mix16SWAR a b c d e f g h i j k l m n o p =
      scalar8 a b c d e f g h + 256 * scalar8 i j k l m n o p := by
  simp only [mix16SWAR]
  rw [packed_y16 a b c d e f g h i j k l m n o p
        ha hb hc hd he hf hg hh hi hj hk hl hm hn ho hp]
  rw [packed_v16
        (Vec8.y32 a) (Vec8.y32 b) (Vec8.y32 c) (Vec8.y32 d)
        (Vec8.y32 e) (Vec8.y32 f) (Vec8.y32 g) (Vec8.y32 h)
        (Vec8.y32 i) (Vec8.y32 j) (Vec8.y32 k) (Vec8.y32 l)
        (Vec8.y32 m) (Vec8.y32 n) (Vec8.y32 o) (Vec8.y32 p)
        (Vec8.y32_lt64 a) (Vec8.y32_lt64 b) (Vec8.y32_lt64 c) (Vec8.y32_lt64 d)
        (Vec8.y32_lt64 e) (Vec8.y32_lt64 f) (Vec8.y32_lt64 g) (Vec8.y32_lt64 h)
        (Vec8.y32_lt64 i) (Vec8.y32_lt64 j) (Vec8.y32_lt64 k) (Vec8.y32_lt64 l)
        (Vec8.y32_lt64 m) (Vec8.y32_lt64 n) (Vec8.y32_lt64 o) (Vec8.y32_lt64 p)]
  rw [packed_p16]
  let lo := Vec8.pack8v
      (Vec8.v32 (Vec8.y32 a) * Vec8.c2) (Vec8.v32 (Vec8.y32 b) * Vec8.c2)
      (Vec8.v32 (Vec8.y32 c) * Vec8.c2) (Vec8.v32 (Vec8.y32 d) * Vec8.c2)
      ...
      (Vec8.v32 (Vec8.y32 g) * Vec8.c2) (Vec8.v32 (Vec8.y32 h) * Vec8.c2)
  let hiw := Vec8.pack8v
      (Vec8.v32 (Vec8.y32 i) * Vec8.c2) (Vec8.v32 (Vec8.y32 j) * Vec8.c2)
      (Vec8.v32 (Vec8.y32 k) * Vec8.c2) (Vec8.v32 (Vec8.y32 l) * Vec8.c2)
      (Vec8.v32 (Vec8.y32 m) * Vec8.c2) (Vec8.v32 (Vec8.y32 n) * Vec8.c2)
      (Vec8.v32 (Vec8.y32 o) * Vec8.c2) (Vec8.v32 (Vec8.y32 p) * Vec8.c2)
  have hlo : lo < 2 ^ 512 := by
    dsimp [lo]
    exact pack8v_lt512
      (Vec8.v32_mul_c2_lt64 (Vec8.y32 a)) (Vec8.v32_mul_c2_lt64 (Vec8.y32 b))
      (Vec8.v32_mul_c2_lt64 (Vec8.y32 c)) (Vec8.v32_mul_c2_lt64 (Vec8.y32 d))
      (Vec8.v32_mul_c2_lt64 (Vec8.y32 e)) (Vec8.v32_mul_c2_lt64 (Vec8.y32 f))
      (Vec8.v32_mul_c2_lt64 (Vec8.y32 g)) (Vec8.v32_mul_c2_lt64 (Vec8.y32 h))
  change
    extract8 (pack512 lo hiw) + 256 * extract8 (pack512 lo hiw >>> 512) =
      scalar8 a b c d e f g h + 256 * scalar8 i j k l m n o p
  rw [extract8_pack512_low lo hiw hlo, shift512_pack512 lo hiw hlo]
  rw [extract8_final_eq a b c d e f g h ha hb hc hd he hf hg hh]
  rw [extract8_final_eq i j k l m n o p hi hj hk hl hm hn ho hp]
...
/-! V26: persistent sixteen-lane progression state. -/
...
theorem mix16_eq_two8
    ...
    (hm : m < 2 ^ 64) (hn : n < 2 ^ 64)
    (ho : o < 2 ^ 64) (hp : p < 2 ^ 64) :
    Vec16.mix16SWAR a b c d e f g h i j k l m n o p =
      Vec8.mix8SWAR a b c d e f g h +
        256 * Vec8.mix8SWAR i j k l m n o p := by
  rw [Vec16.mix16SWAR_eq
        a b c d e f g h i j k l m n o p
        ha hb hc hd he hf hg hh hi hj hk hl hm hn ho hp]
  rw [Vec8.mix8SWAR_eq a b c d e f g h ha hb hc hd he hf hg hh]
  rw [Vec8.mix8SWAR_eq i j k l m n o p hi hj hk hl hm hn ho hp]
  rfl

def pack16SWAR16 (x : Nat) : Nat :=
  let x1 := x + stepConst
  let x2 := x1 + stepConst
  let x3 := x2 + stepConst
  let x4 := x3 + stepConst
  ...
  Vec16.mix16SWAR x x1 x2 x3 x4 x5 x6 x7 x8 x9 x10 x11 x12 x13 x14 x15
...
theorem pack16SWAR16_eq (x : Nat) (h : x + 15 * stepConst < 2 ^ 64) :
  ...
  have h0 : x < 2 ^ 64 := by omega
  have h1 : x + stepConst < 2 ^ 64 := by omega
  have h2 : x + 2 * stepConst < 2 ^ 64 := by omega
  have h3 : x + 3 * stepConst < 2 ^ 64 := by omega
  have h4 : x + 4 * stepConst < 2 ^ 64 := by omega
  have h5 : x + 5 * stepConst < 2 ^ 64 := by omega
  have h6 : x + 6 * stepConst < 2 ^ 64 := by omega
  have h7 : x + 7 * stepConst < 2 ^ 64 := by omega
  have h8 : x + 8 * stepConst < 2 ^ 64 := by omega
  have h9 : x + 9 * stepConst < 2 ^ 64 := by omega
  have h10 : x + 10 * stepConst < 2 ^ 64 := by omega
  have h11 : x + 11 * stepConst < 2 ^ 64 := by omega
  have h12 : x + 12 * stepConst < 2 ^ 64 := by omega
  have h13 : x + 13 * stepConst < 2 ^ 64 := by omega
  have h14 : x + 14 * stepConst < 2 ^ 64 := by omega
  have h15 : x + 15 * stepConst < 2 ^ 64 := h
  unfold pack16SWAR16 pack8SWAR8
  rw [mix16_eq_two8
        x
        (x + stepConst)
        (x + stepConst + stepConst)
        ...
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
        (x + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst + stepConst)
...
def hexState (x : Nat) : Nat :=
  ...
  let x14 := x13 + stepConst
  let x15 := x14 + stepConst
  Vec16.pack16v x x1 x2 x3 x4 x5 x6 x7 x8 x9 x10 x11 x12 x13 x14 x15

def mix16Packed (p0 : Nat) : Nat :=
  let u := (p0 ^^^ (p0 >>> 16)) &&& Vec16.mask16
  let y := (u * Vec8.c1) &&& Vec16.mask16
  let v := (y ^^^ (y >>> 15)) &&& Vec16.mask16
  let z := v * Vec8.c2
  Vec16.extract8 z + 256 * Vec16.extract8 (z >>> 512)

theorem mix16Packed_state_eq (x : Nat) :
    mix16Packed (hexState x) = pack16SWAR16 x := by
  rfl

def hexDelta : Nat :=
  Vec16.pack16v
    (16 * stepConst) (16 * stepConst) (16 * stepConst) (16 * stepConst)
    (16 * stepConst) (16 * stepConst) (16 * stepConst) (16 * stepConst)
    (16 * stepConst) (16 * stepConst) (16 * stepConst) (16 * stepConst)
    (16 * stepConst) (16 * stepConst) (16 * stepConst) (16 * stepConst)

def advanceHex (p : Nat) : Nat :=
  p + hexDelta
...
theorem advanceHex_state (x : Nat) :
    advanceHex (hexState x) = hexState (advance16 x) := by
  unfold advanceHex hexState hexDelta advance16 advance8
  unfold Vec16.pack16v Vec16.pack512
...
theorem pack14SWAR16_eq (x : Nat) :
    pack14SWAR16 x = packByteTailSWAR8 x 1 := by
  rfl

def packHexTail : Nat → Nat → Nat → Nat
  | _, x, 0 => pack14SWAR16 x
  | p, x, n + 1 =>
      mix16Packed p + 65536 * packHexTail (advanceHex p) (advance16 x) n

set_option maxRecDepth 65536 in
theorem packHexTail_eq (x n : Nat)
    (h : x + (16 * n + 13) * stepConst < 2 ^ 64) :
    packHexTail (hexState x) x n = packByteTailSWAR8 x (2 * n + 1) := by
  induction n generalizing x with
  | zero =>
      simp only [packHexTail, Nat.mul_zero, Nat.zero_add]
      exact pack14SWAR16_eq x
  | succ n ih =>
      simp only [packHexTail]
      have h15 : x + 15 * stepConst < 2 ^ 64 := by
        have hk : 15 ≤ 16 * (n + 1) + 13 := by omega
        have hm : 15 * stepConst ≤ (16 * (n + 1) + 13) * stepConst :=
          Nat.mul_le_mul_right stepConst hk
        exact Nat.lt_of_le_of_lt (Nat.add_le_add_left hm x) h
      rw [mix16Packed_state_eq, pack16SWAR16_eq x h15, advanceHex_state]
      have hadv : advance16 x = advance8 (advance8 x) := rfl
      have hr :
      ...
      rw [hidx]
      simp only [packByteTailSWAR8]
      omega

def initPackedHexProgression (seed : Nat) : Nat :=
  let x := seed + 3 * stepConst
  1 + 4 * packHexTail (hexState x) x 15

theorem initPackedHexProgression_eq (n : Nat) :
    initPackedHexProgression (caSeed n) = initPackedSWAR8 (caSeed n) := by
  simp only [initPackedHexProgression, initPackedSWAR8]
  rw [packHexTail_eq]
  · rfl
  · have hs : caSeed n < 2 ^ 32 := by
      unfold caSeed
...
def impl : Nat → Nat := fun n =>
  biterFast (caSteps n) (initPackedHexProgression (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPackedHexProgression_eq, initPackedSWAR8_eq, initPackedByteNat_eq, initPackedByte_eq, initPackedFastBit_eq, initPackedFast_eq]
