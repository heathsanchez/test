...
/-! V28: contract the Rule 110 bitwise kernel itself.

The official Boolean form duplicates the wrapped-right neighbor and realizes
negation through several XOR-with-M layers.  This equivalent form names each
rotation once and applies the pointwise Boolean identity
  ¬((¬m ∧ ¬r) ∨ (l ∧ m ∧ r)) = (m ∨ r) ∧ ¬(l ∧ m ∧ r).
-/

def rotL256 (m : Nat) : Nat :=
  ((m <<< 1) ||| (m >>> 255)) &&& M

def rotR256 (m : Nat) : Nat :=
  ((m >>> 1) ||| ((m &&& 1) <<< 255)) &&& M

def bstepContract (m : Nat) : Nat :=
  let l := rotL256 m
  let r := rotR256 m
  (m ||| r) &&& (M ^^^ (l &&& m &&& r))

theorem bstepContract_eq (m : Nat) :
    bstepContract m = bstep m := by
  unfold bstepContract
  change
    (m ||| rotR256 m) &&& (M ^^^ (rotL256 m &&& m &&& rotR256 m)) =
      (M ^^^ (((M ^^^ m) &&& (M ^^^ rotR256 m)) |||
        (rotL256 m &&& m &&& rotR256 m))) &&& M
  apply Nat.eq_of_testBit_eq
  intro i
  simp only [Nat.testBit_and, Nat.testBit_or, Nat.testBit_xor, testBit_M]
  by_cases hi : i < 256
  · simp only [hi, decide_true, Bool.and_true]
    generalize m.testBit i = mb
    generalize (rotR256 m).testBit i = rb
    generalize (rotL256 m).testBit i = lb
    cases mb <;> cases rb <;> cases lb <;> decide
  · have hr : (rotR256 m).testBit i = false := by
      unfold rotR256
      rw [Nat.testBit_and, testBit_M]
      simp [hi]
    have hl : (rotL256 m).testBit i = false := by
      unfold rotL256
      rw [Nat.testBit_and, testBit_M]
      simp [hi]
    simp [hi, hr, hl]
...
def biterContract (t m : Nat) : Nat :=
  if t = 2 then
    bstepContract (bstepContract m)
  else if t = 4 then
    bstepContract (bstepContract (bstepContract (bstepContract m)))
  else if t = 8 then
    bstepContract
      (bstepContract
        (bstepContract
...
theorem biterContract_eq (t m : Nat) : biterContract t m = biter t m := by
  unfold biterContract
  by_cases h2 : t = 2
  · rw [if_pos h2]
    subst t
    simp only [bstepContract_eq]
    rfl
  · rw [if_neg h2]
    by_cases h4 : t = 4
    · rw [if_pos h4]
...
def impl : Nat → Nat := fun n =>
  biterContract (caSteps n) (initPackedOctProgression (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterContract_eq, initPackedOctProgression_eq, initPackedSWAR8_eq, initPackedByteNat_eq, initPackedByte_eq, initPackedFastBit_eq, initPackedFast_eq]
