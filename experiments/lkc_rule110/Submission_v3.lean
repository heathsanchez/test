...
def rotL (m : Nat) : Nat := ((m <<< 1) ||| (m >>> 255)) &&& M
def rotR (m : Nat) : Nat := ((m >>> 1) ||| ((m &&& 1) <<< 255)) &&& M
...
theorem biterFast_eq (t m : Nat) : biterFast t m = biterMin t m := by
  unfold biterFast
  by_cases h2 : t = 2
  · rw [if_pos h2]; subst t; rfl
  · rw [if_neg h2]
    by_cases h4 : t = 4
    · rw [if_pos h4]; subst t; rfl
    · rw [if_neg h4]
      by_cases h8 : t = 8
      · rw [if_pos h8]; subst t; rfl
      · rw [if_neg h8]
...
def impl : Nat → Nat := fun n =>
  biterFast (caSteps n) (initPacked (caSeed n))
