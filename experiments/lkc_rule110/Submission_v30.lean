...
/-! V30: compile power-of-two output scaling into shifts.

V25 assembles the byte stream with multiplication by 256 and applies the final
two-bit offset with multiplication by 4.  These are exact powers of two, so the
runtime representation can use shifts while the proof reduces them back to the
same arithmetic form. -/

def packOctTailShift : Nat → Nat → Nat → Nat
  | _, x, 0 => pack6SWAR8 x
  | p, x, n + 1 =>
      mix8Packed p + (packOctTailShift (advanceOct p) (advance8 x) n <<< 8)

theorem packOctTailShift_eq (p x n : Nat) :
    packOctTailShift p x n = packOctTail p x n := by
  induction n generalizing p x with
  | zero =>
      rfl
  | succ n ih =>
      simp only [packOctTailShift, packOctTail]
      rw [ih]
      simp [Nat.shiftLeft_eq, Nat.mul_comm]

def initPackedOctShift (seed : Nat) : Nat :=
  let x := seed + 3 * stepConst
  1 + (packOctTailShift (octState x) x 31 <<< 2)

theorem initPackedOctShift_eq (seed : Nat) :
    initPackedOctShift seed = initPackedOctProgression seed := by
  unfold initPackedOctShift initPackedOctProgression
  dsimp
  rw [packOctTailShift_eq]
  simp [Nat.shiftLeft_eq, Nat.mul_comm]

def impl : Nat → Nat := fun n =>
  biterFast (caSteps n) (initPackedOctShift (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPackedOctShift_eq, initPackedOctProgression_eq, initPackedSWAR8_eq, initPackedByteNat_eq, initPackedByte_eq, initPackedFastBit_eq, initPackedFast_eq]
