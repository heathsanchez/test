...
/-! V31: isolated sixteen-lane width test.

This deliberately tests width only.  It reuses the independently green Vec16
lane-independence proof, emits two bytes per mixer invocation, and keeps the
...
theorem pack16SWAR16_eq (x : Nat) (h : x + 15 * stepConst < 2 ^ 64) :
  ...
  rw [mix16_eq_two8
        ...
        h0 h1 (by omega) (by omega) (by omega) (by omega) (by omega) (by omega)
...
theorem pack16Tail_eq (x n : Nat)
  ...
  induction n generalizing x with
  | zero =>
      simp only [pack16Tail, Nat.mul_zero, Nat.zero_add]
      exact pack14SWAR16_eq x
  | succ n ih =>
      simp only [pack16Tail]
...
def initPackedSWAR16 (seed : Nat) : Nat :=
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPackedSWAR16_eq, initPackedSWAR8_eq, initPackedByteNat_eq, initPackedByte_eq, initPackedFastBit_eq, initPackedFast_eq]
