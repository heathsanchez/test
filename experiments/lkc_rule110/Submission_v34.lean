...
/-! V34 composition: retain V27 state contraction and add the independently
verified V28 Rule 110 transition contraction. -/

def rotL256C (m : Nat) : Nat :=
  ((m <<< 1) ||| (m >>> 255)) &&& M

def rotR256C (m : Nat) : Nat :=
  ((m >>> 1) ||| ((m &&& 1) <<< 255)) &&& M
...
theorem bstepContract_eq (m : Nat) :
  ...
  change
    (m ||| rotR256C m) &&& (M ^^^ (rotL256C m &&& m &&& rotR256C m)) =
      (M ^^^ (((M ^^^ m) &&& (M ^^^ rotR256C m)) |||
        (rotL256C m &&& m &&& rotR256C m))) &&& M
  apply Nat.eq_of_testBit_eq
  intro i
  ...
  · simp only [hi, decide_true, Bool.and_true]
    ...
    generalize (rotR256C m).testBit i = rb
    generalize (rotL256C m).testBit i = lb
    cases mb <;> cases rb <;> cases lb <;> decide
  · have hr : (rotR256C m).testBit i = false := by
      unfold rotR256C
      rw [Nat.testBit_and, testBit_M]
      simp [hi]
    have hl : (rotL256C m).testBit i = false := by
      unfold rotL256C
      rw [Nat.testBit_and, testBit_M]
      simp [hi]
    simp [hi, hr, hl]
...
def impl : Nat → Nat := fun n =>
  biterContract (caSteps n) (initPackedOctContracted (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterContract_eq, initPackedOctContracted_eq, initPackedSWAR8_eq, initPackedByteNat_eq, initPackedByte_eq, initPackedFastBit_eq, initPackedFast_eq]
