...
/-! Project the public mixer directly to the only bit the initializer consumes. -/
...
def initCellBit (seed i : Nat) : Bool :=
  if i = 0 then true
  else if i = 1 then false
  else mixBit31 (seed + (i + 1) * 0x9e3779b9)

theorem initCellBit_eq (seed i : Nat) :
    initCellBit seed i = initCell seed i := by
  unfold initCellBit initCell
  by_cases h0 : i = 0
  · simp [h0]
...
def packFromBit (seed : Nat) : Nat → Nat → Nat
  | _, 0 => 0
  | start, k + 1 =>
      (if initCellBit seed start then 1 else 0) +
      2 * packFromBit seed (start + 1) k

theorem packFromBit_eq (seed start k : Nat) :
    packFromBit seed start k = packFrom seed start k := by
  induction k generalizing start with
  | zero => rfl
  | succ k ih =>
      unfold packFromBit packFrom
      rw [initCellBit_eq, ih]

def initPackedBit (seed : Nat) : Nat :=
  packFromBit seed 0 256

theorem initPackedBit_eq_initPacked (seed : Nat) :
    initPackedBit seed = initPacked seed := by
  unfold initPackedBit initPacked
  exact packFromBit_eq seed 0 256

theorem initPackedBit_eq (seed : Nat) :
    initPackedBit seed = encodeRow (initRowFor seed) := by
  rw [initPackedBit_eq_initPacked, initPacked_eq]

def impl : Nat → Nat := fun n =>
  biterFast (caSteps n) (initPackedBit (caSeed n))
...
theorem impl_correct : ∀ n, impl n = caSpecN n := by
  intro n
  unfold impl
  rw [biterFast_eq, initPackedBit_eq]
