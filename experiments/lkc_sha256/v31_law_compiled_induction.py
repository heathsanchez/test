...
/--
Generic certificate compiler.  The induction sees only the two public
transition laws of F; it never unfolds the concrete implementation.
-/
theorem runner_of_laws
    (F : List Nat → Window → Digest → Digest)
    (hNil : ∀ w s, F [] w s = s)
    (hCons : ∀ k ks w s,
      F (k :: ks) w s =
        F ks (w.push w.nextFast) (roundFast s k w.x0)) :
    ∀ ks w s, F ks w s = roundsShadow ks w s := by
  intro ks
  induction ks with
  | nil =>
      intro w s
  ...
  | cons k ks ih =>
      ...
      calc
        F (k :: ks) w s =
            F ks (w.push w.nextFast) (roundFast s k w.x0) :=
          hCons k ks w s
        _ = roundsShadow ks (w.push w.nextFast) (roundFast s k w.x0) :=
          ih (w.push w.nextFast) (roundFast s k w.x0)
        _ = roundsShadow (k :: ks) w s := by rfl

theorem scalar_eq_shadow :
    ∀ ks w s, roundsScalarMulW ks w s = roundsShadow ks w s :=
  runner_of_laws roundsScalarMulW scalar_nil_law scalar_cons_law

end Submission
'''
p=OUT/"V31LawCompiledInduction.lean"
