...
proof=f'''
/-! Fixed-K scalar bridge: no recursive proof object. -/
...
theorem roundsScalarMulW_nil_K (w : Window) (s : Digest) :
    roundsScalarMulW [] w s = s := by
  unfold roundsScalarMulW
  rw [roundsScalarMul]

theorem roundsScalarMulW_cons_K (k : Nat) (ks : List Nat)
    (w : Window) (s : Digest) :
...
theorem roundsFast_nil_K (w : Window) (s : Digest) :
    roundsFast [] w s = s := by
  rw [roundsFast]

theorem roundsFast_cons_K (k : Nat) (ks : List Nat)
    (w : Window) (s : Digest) :
    roundsFast (k :: ks) w s =
      roundsFast ks (w.push w.nextFast) (roundFast s k w.x0) := by
  rw [roundsFast]

theorem roundsScalarMulW_K_eq (w : Window) (s : Digest) :
    roundsScalarMulW K w s = roundsFast K w s := by
  change roundsScalarMulW {klit} w s =
    roundsFast {klit} w s
  repeat rw [roundsScalarMulW_cons_K, roundsFast_cons_K]
  rw [roundsScalarMulW_nil_K, roundsFast_nil_K]

end Submission
'''
p=OUT/"V31ScalarFixedKProofDev.lean"
