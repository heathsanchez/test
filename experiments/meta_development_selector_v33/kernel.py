...
from basis import World, Const, Atom, Bin, signature, sufficient, mapping_for, predict_with, expr_cost, canonical_skeleton, skeleton_var_count
...
class Kernel:
    ...
    def closure(self, w: World, *, max_atoms=3, max_depth=2):
        ...
        rows=[]
        for sig,(_sc,variants) in best.items():
    ...
    @staticmethod
    def _abstract_schema(sk):
        if sk[0]=="K": return sk
        if sk[0]=="A": return sk
        return ("B","*",Kernel._abstract_schema(sk[2]),Kernel._abstract_schema(sk[3]))

    @staticmethod
    def _instantiate_schema(sk, channel_count, max_offset):
        n=skeleton_var_count(sk)
        op_slots=[]
        def count_ops(x):
            if x[0]=="B":
                op_slots.append(len(op_slots))
                count_ops(x[2]); count_ops(x[3])
        count_ops(sk)
        for binding in product(range(channel_count),repeat=n):
            for ops in product(range(16),repeat=len(op_slots)):
                oi=[0]
                def go(x):
                    if x[0]=="K": return Const(int(x[1]))
                    if x[0]=="A":
                        off=int(x[1])
    ...
    def solve(self,w:World,*,allow_acquisition=True,compiled_key=None):
        ...
        if compiled_key and compiled_key in self.compiled:
            ...
            for sk in cm.schemas:
                for e in self._instantiate_schema(sk,w.channel_count,w.max_offset):
                    key=repr(e.data())
    ...
    def compile_from(self,key:str,named_results):
        ...
        for name,res in named_results:
            ...
            sk={self._abstract_schema(tupleize(row["skeleton"])) for row in res.get("frontier",[])}
        ...
        return {"status":"VERIFIED","compiled_key":key,"schema_count":len(cm.schemas),"schemas":list(cm.schemas),"provenance":prov}

    def select_probe(self, solve_result, pool_records):
    ...
    def contraction(self,w:World,solve_result):
        ...
        return {"status":"VERIFIED","current_blocks":current_blocks,"selected_blocks":selected_blocks,"contracted":selected_blocks<current_blocks}
