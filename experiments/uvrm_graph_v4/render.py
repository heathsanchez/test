...
def render(case, arm):
    ...
    if arm=='TRANSCRIPT':
        ...
        if arm=='GRAPH_RULES':
            body += '\n\nController scaffold: prefer MAP when the mechanism is not localized; prefer existing closure/composition before new representation; use TRANSFER when identity/generalization is the unresolved claim; use DISCRIMINATE when a phase split yields a matched separator.'
    else:
