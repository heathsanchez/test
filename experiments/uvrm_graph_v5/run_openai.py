...
def main():
    ...
    random.Random(2026082402).shuffle(jobs)
    answers={}; meta={'model':MODEL,'provider':'openai','temperature':0,'max_tokens':220,'seed_order':2026082402,'n_expected':len(jobs),'calls':[]}
    for i,(case,arm) in enumerate(jobs,1):
        ...
        answers[key]=text; meta['calls'].append({'key':key,'seconds':round(dt,3),'prompt_chars':len(prompt),'answer_chars':len(text)})
