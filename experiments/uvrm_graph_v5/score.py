...
def score_text(case,text):
    ...
    semantic_pass=(semantic>=2/3 and forbidden_hits==0)
    return {'exact_mode':exact_mode,'required_hits':required_hits,'required_total':len(case['required']),'semantic_score':semantic,'forbidden_hits':forbidden_hits,'semantic_pass':semantic_pass}
...
def main():
    ...
    for arm in ('TRANSCRIPT','GRAPH_ABL','GRAPH','GRAPH_RULES'):
        ...
        out[arm]={'n':len(rs),'exact_mode_rate':sum(r['exact_mode'] for r in rs)/len(rs),'semantic_pass_rate':sum(r['semantic_pass'] for r in rs)/len(rs),'mean_semantic_score':sum(r['semantic_score'] for r in rs)/len(rs),'forbidden_rate':sum(r['forbidden_hits']>0 for r in rs)/len(rs)}
