...
fn check_inductive(
    ...
    // G17-001 is the first independently earned indexed family. It is routed
    // by exact root name before the generic unearned-index boundary; all other
    // indexed families still decline there.
    ...
    {
        return check_exact_punit(export, environment, block, limits, delta_policy);
    }
...
/// G15-001's closed internal quotient, extended only by G16-001's independently
/// earned PUnit law. External recognition remains name-sealed, so the internal
/// skeleton itself does not authorize arbitrary declaration families.
#[derive(Clone, Copy)]
        ...
        match self {
            Self::PUnit { .. } => {
                recursor.level_params.len() == 2
                    && recursor.level_params.get(1) == inductive_level_params.first()
            }
...
/// G17-001's exact Eq frontier. This is a name-sealed static indexed law:
/// two parameters (α,a), one dependent index, refl, and the derived recursor
/// signature/rule with K metadata. It installs only opaque signatures; Rule-K
/// reduction is deliberately not implemented here.
fn check_exact_eq(
    export: &ResolvedExport,
    environment: &Environment,
    block: &InductiveBlock,
...
/// G16-001's exact PUnit frontier. The family is name-sealed and contributes
/// only its independently earned nullary/singleton cardinality and Sort-u law.
/// Promotion remains the retained opaque-signature transaction.
fn check_exact_punit(
    export: &ResolvedExport,
    environment: &Environment,
    block: &InductiveBlock,
    limits: Limits,
