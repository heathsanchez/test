...
pub fn check_export(export: ResolvedExport, limits: Limits) -> Verdict {
    ...
    if has_unqualified_string_literal && matches!(verdict, Verdict::Accept | Verdict::Reject) {
        trace_missing_interface("literal.string@1", "unqualified-string-literal");
        Verdict::Unknown
    } else {
        verdict
    }
...
fn check_inductive(
    ...
    if std::env::var_os("NUCLEUS_TRACE_INDUCTIVE_CENSUS").is_some() {
        ...
        eprintln!(
            ...
            block.recursors.len(),
            type_shapes.join(","),
            constructor_shapes.join(","),
            recursor_shapes.join(","),
        );
    }
