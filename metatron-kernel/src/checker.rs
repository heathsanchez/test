...
fn check_exact_fin(
    ...
    // The Fin parameter and first field are both Nat. The second field may
    // depend on the first field, so it is validated through the exact
    // constructor/recursor telescope rather than approximated as a projection.
    let Some((type_domains, type_result)) = pi_spine(export, inductive.ty, 1) else {
        return Err(Verdict::Reject);
    };
    let [parameter] = type_domains.as_slice() else {
        return Err(Verdict::Reject);
    };
    let Some(nat) = environment.nat_primitives() else {
        return Err(Verdict::Unknown);
    };
    if !is_empty_constant(export, *parameter, nat.type_name)
        || !matches!(
            export.exprs.get(type_result),
            Some(Expr::Sort(level))
                if matches!(
                    export.levels.get(*level),
                    Some(Level::Succ(inner))
    ...
    let [ctor_parameter, value_field, proof_field] = constructor_domains.as_slice() else {
        return Err(Verdict::Reject);
    };
    if !is_empty_constant(export, *ctor_parameter, nat.type_name)
        || !is_empty_constant(export, *value_field, nat.type_name)
    {
        return Err(Verdict::Reject);
    }
