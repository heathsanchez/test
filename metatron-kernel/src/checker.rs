...
fn generic_nonrecursive_recursor_shape(
    ...
    let Some((domains, result)) = pi_spine(export, recursor.ty, p + c + 2) else {
        return false;
    };
    if domains[..p] != ind_params[..] {
        return false;
    }
    ...
    if ma.len() != p
        || !is_declared_level_constant(export, mh, inductive.name, &inductive.level_params)
        || !ma
            .iter()
            .enumerate()
            .all(|(i, a)| is_bvar(export, *a, (p - 1 - i) as u64))
    {
        return false;
    }
    let motive_level = match export.exprs.get(motive_sort) {
        Some(Expr::Sort(l)) => match export.levels.get(*l) {
            Some(Level::Param(n)) => *n,
            _ => return false,
        },
        _ => return false,
    };
    ...
    for (j, ctor) in constructors.iter().enumerate() {
        let Ok(f) = usize::try_from(ctor.num_fields) else {
            return false;
        };
        let Some((ctor_domains, _)) = pi_spine(export, ctor.ty, p + f) else {
            return false;
        };
        let Some((minor_fields, minor_result)) = pi_spine(export, domains[p + 1 + j], f) else {
            return false;
        };
        ...
        if !is_bvar(export, *mm, (f + j) as u64) {
            return false;
        }
        let (ch, ca) = application_spine(export, *constructed);
        if ca.len() != p + f
            || !is_declared_level_constant(export, ch, ctor.name, &ctor.level_params)
        {
            return false;
        }
        ...
        for k in 0..f {
            if !is_bvar(export, ca[p + k], (f - 1 - k) as u64) {
                return false;
            }
        }
    }
    ...
    if ta.len() != p
        || !is_declared_level_constant(export, th, inductive.name, &inductive.level_params)
        || !ta
            .iter()
            .enumerate()
            .all(|(i, a)| is_bvar(export, *a, (c + 1 + (p - 1 - i)) as u64))
        || !is_bvar_application(export, result, (c + 1) as u64, 0)
    {
        return false;
    }

    if recursor.rules.len() != c {
        return false;
    }
    for (j, (ctor, rule)) in constructors.iter().zip(&recursor.rules).enumerate() {
        ...
        let Some((_, rr)) = lam_spine(export, rule.rhs, p + 1 + c + f) else {
            return false;
        };
        let (h, args) = application_spine(export, rr);
        if !is_bvar(export, h, (f + (c - 1 - j)) as u64)
            || args.len() != f
            || !args
                .iter()
                .enumerate()
                .all(|(k, a)| is_bvar(export, *a, (f - 1 - k) as u64))
        {
            return false;
        }
    }
...
fn check_generic_nonrecursive_type(
    ...
    let ty = if inductive.level_params.is_empty() {
