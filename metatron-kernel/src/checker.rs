    ...
    for declaration in declarations {
        ...
        let (name, established) = match declaration {
                ...
                match check_inductive(&export, &environment, &block, limits, delta_policy) {
                    ...
                    Err(verdict) => {
                        if verdict == Verdict::Unknown
                            && std::env::var_os("NUCLEUS_TRACE_POST_ATOMIC").is_some()
                        {
                            let types = block
                                .types
                                .iter()
                            ...
                            let constructors = block
                                ...
                                .collect::<Vec<_>>();
                            eprintln!(
                                "NUCLEUS_POST_ATOMIC:inductive:types=[{}]:ctors=[{}]",
                                types.join(","),
                                constructors.join(","),
                            );
                        }
            ...
            Declaration::Unsupported { .. } => {
                if std::env::var_os("NUCLEUS_TRACE_POST_ATOMIC").is_some() {
                    eprintln!("NUCLEUS_POST_ATOMIC:unsupported-declaration");
                }
...
fn verdict_boundary(judgment: Judgment<()>) -> Result<(), Verdict> {
    match judgment {
        ...
        Judgment::Unknown { residual } => {
            if std::env::var_os("NUCLEUS_TRACE_POST_ATOMIC").is_some() {
                eprintln!("NUCLEUS_POST_ATOMIC:judgment:{}", residual.0);
            }
            Err(Verdict::Unknown)
        }
    }
