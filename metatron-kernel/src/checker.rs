...
fn check_single_constructor_inductive(
    ...
    } else if exact_fin_candidate(export, block) {
        check_exact_fin(export, environment, block, limits, delta_policy)
...
fn exact_fin_recursor_shape(
    ...
    if !ctor_fields.iter().zip(&minor_domains).enumerate().all(
        |(field, (ctor_domain, minor_domain))| {
            expression_matches_binder_lift(export, *ctor_domain, *minor_domain, field as u64, 1)
        },
    ) {
        return false;
    }
    ...
    for field in 0..fields {
        if !expression_matches_binder_lift(
            export,
            minor_domains[field],
...
fn exact_fin_candidate(export: &ResolvedExport, block: &InductiveBlock) -> bool {
    ...
    if !name_is_root_str(export, inductive.name, "Fin")
    ...
    *constructor_parameter == *parameter_type
        ...
        && exact_fin_recursor_shape(export, inductive, constructor, recursor)
}
...
fn check_exact_fin(
    ...
    if !exact_fin_candidate(export, block) {
        return Err(Verdict::Unknown);
    }
    if has_duplicate_parameter(&inductive.level_params)
        || constructor_result_is_definitely_malformed(export, inductive, constructor)
