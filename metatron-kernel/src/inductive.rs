    ...
    pub(crate) fn promote(
        ...
        if std::env::var_os("NUCLEUS_TRACE_SIGNATURE").is_some() {
            match &signature_judgment {
                Judgment::Proven { .. } => eprintln!("NUCLEUS_SIGNATURE:{:?}:{}:proven", signature.kind, signature.name.0),
                Judgment::Refuted { obstruction } => eprintln!("NUCLEUS_SIGNATURE:{:?}:{}:refuted:{}", signature.kind, signature.name.0, obstruction.0),
                Judgment::Unknown { residual } => eprintln!("NUCLEUS_SIGNATURE:{:?}:{}:unknown:{}", signature.kind, signature.name.0, residual.0),
            }
        }
