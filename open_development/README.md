...
`ProofCompositionAdapter` adds the stronger capability-growth control. The first obligation justifies and retains a binary product constructor assembled over primitive certificate nodes. After restart, a held-out interval obligation is unreachable in the cold crystal but becomes solvable at zero acquisition budget by constructing a different product program. Removing the retained constructor restores the failure. This is bounded constructor-enabled reachability, not invention of the primitive grammar.
