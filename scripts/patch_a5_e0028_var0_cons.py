...
# E0028 only: direct fast path for the most local de Bruijn lookup in a Cons environment.
p = Path('e0028/src/eval.rs')
s = p.read_text()
...
new = """            Expr::Var { dbj_idx, .. } => {
                let v = if dbj_idx == 0 {
                    match env {
                        value::Env::Cons { v, .. } => *v,
                        _ => env.lookup(dbj_idx).expect("eval: loose bvar"),
                    }
                } else {
