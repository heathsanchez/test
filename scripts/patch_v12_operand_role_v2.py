...
# Instrument the two computations feeding the final declaration defeq using robust atomic anchors.
val_anchor="        let val_ty = self.infer_value(Check, 0, empty_env, empty_ctx, val);\\n"
decl_anchor="        let declared = self.eval(0, empty_env, d.info().ty);\\n"
eval_anchor='        eprintln!("[MGTRACE] kind=eval site=infer.declared_type depth=0 value={:p}", declared);\\n'
if val_anchor not in s: raise SystemExit('V12 val_ty anchor missing')
if decl_anchor not in s: raise SystemExit('V12 declared anchor missing')
if eval_anchor not in s: raise SystemExit('V12 declared eval trace anchor missing')
s=s.replace(val_anchor,
    '        eprintln!("[MGTRACE] kind=role_begin role=subject site=infer.decl");\\n'+val_anchor,1)
...
s=s.replace(eval_anchor,eval_anchor+
...
for c in sorted((ARENA/'good').rglob('*.ndjson')):
    ...
    nearest=route_nearest_mechanism(ev,fi); span=route_producer_span(ev,fi); pointer=route_pointer_only(ev,fi)
    subject=route_role(ev,fi,'subject'); reference=route_role(ev,fi,'reference'); shared=route_shared_inputs(ev,fi)
...
p.write_text(s)
print('patched V12 runner for failed-comparison operand causal quotient v2')
