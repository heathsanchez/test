...
root=Path.cwd()
out=root/'results/a3-profile'
out.mkdir(parents=True,exist_ok=True)
bin=root/'a3/target/release/sokonanoda'
cfg=root/'a3/config.json'
...
for kind,expected in [('good','accept'),('bad','reject')]:
    for p in sorted((root/'arena-tests'/kind).rglob('*.ndjson')):
        ...
        rows.append({'file':str(p.relative_to(root/'arena-tests')),'expected':expected,'statuses':statuses,'median_wall_s':statistics.median(samples),'samples_wall_s':samples,'size_bytes':p.stat().st_size})
...
if bad: raise SystemExit('semantic failure in A3 profile')
