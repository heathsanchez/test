...
root = Path.cwd()
out = root / "results/a2-e0012-public"
...
for v in variants:
    ...
    for p, expected in cases:
        with p.open("rb") as f:
            rc = subprocess.run(
                [str(bins[v]), str(cfgs[v])],
                stdin=f,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            ).returncode
        got = status(rc)
        declines += int(got == "decline")
        if got != expected:
            failures.append({
...
workload = sorted(
    workload,
    key=lambda p: hashlib.sha256(str(p.relative_to(root / "arena-tests")).encode()).hexdigest(),
)
...
for seed in range(16):
    ...
    for v in order:
        ...
        samples[v].append(time.perf_counter() - start)

med = {v: statistics.median(samples[v]) for v in variants}
paired = [(c - b) / b for b, c in zip(samples["a2"], samples["e0012"])]
summary = {
    "substrate": {
    ...
    "median_seconds": med,
    "speedup_e0012_vs_a2": med["a2"] / med["e0012"],
    "paired_fractional_change_e0012_minus_a2": paired,
