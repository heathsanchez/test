...
root=Path.cwd(); out=root/'results/a2-e0017-public'; out.mkdir(parents=True,exist_ok=True)
vs=['a2','e0017']; bins={v:root/v/'target/release/sokonanoda' for v in vs}; cfgs={v:root/v/'config.json' for v in vs}
...
med={v:statistics.median(samples[v]) for v in vs}; paired=[(b-a)/a for a,b in zip(samples['a2'],samples['e0017'])]
summary={'substrate':{'sokonanoda':'9b4ea12f4cd437d00b6bcd0e34743065c58dea08','threads':4,'session_budget':2621440},'correctness':correctness,'median_seconds':med,'speedup_e0017_vs_a2':med['a2']/med['e0017'],'paired_median_fractional_change':statistics.median(paired),'paired_fractional_change_e0017_minus_a2':paired,'samples_seconds':samples,'orders':orders,'workload':[str(p.relative_to(root/'arena-tests')) for p in workload]}
