...
root = Path.cwd()
out = root / 'results/a2-e0018-protected'
...
# Same deterministic 24-case proxy, but resource-measured under Arena-style PGO builds.
...
# Run one whole arm under GNU time so max RSS is scoped to that arm.
...
def paired(k): return [(b-a)/a for a,b in zip(vals('a2',k),vals('e0018',k))]
summary={
  'substrate':{
  ...
  'wins':{k:sum(b<a for a,b in zip(vals('a2',k),vals('e0018',k))) for k in ('wall','cpu','rss_kb')}
}
