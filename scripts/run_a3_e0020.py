...
root=Path.cwd(); out=root/'results/a3-e0020-public'; out.mkdir(parents=True,exist_ok=True)
vs=['a3','e0020']; bins={v:root/v/'target/release/sokonanoda' for v in vs}; cfgs={v:root/v/'config.json' for v in vs}
...
med={v:statistics.median(samples[v]) for v in vs}; paired=[(b-a)/a for a,b in zip(samples['a3'],samples['e0020'])]
summary={'substrate':{'sokonanoda':'9b4ea12f4cd437d00b6bcd0e34743065c58dea08','threads':4,'session_budget':2621440,'a3':'E0018 apply_many transient prune removal'},'correctness':correctness,'median_seconds':med,'speedup_e0020_vs_a3':med['a3']/med['e0020'],'paired_median_fractional_change':statistics.median(paired),'paired_win_count_e0020':sum(x<0 for x in paired),'paired_fractional_change_e0020_minus_a3':paired,'samples_seconds':samples,'orders':orders,'workload':[str(p.relative_to(root/'arena-tests')) for p in workload]}
