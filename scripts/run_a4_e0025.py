...
root=Path.cwd(); out=root/'results/a4-e0025-public'; out.mkdir(parents=True,exist_ok=True)
vs=['a4','e0025']; bins={v:root/v/'target/release/sokonanoda' for v in vs}; cfgs={v:root/v/'config.json' for v in vs}
...
med={v:statistics.median(samples[v]) for v in vs}; paired=[(b-a)/a for a,b in zip(samples['a4'],samples['e0025'])]
summary={'substrate':{'sokonanoda':'9b4ea12f4cd437d00b6bcd0e34743065c58dea08','threads':4,'session_budget':2621440,'a4':'E0018 transient prune removal + E0024 App open-cache bypass'},'intervention':'from A4, also bypass open-eval canonicalization/cache for Lambda','correctness':correctness,'median_seconds':med,'speedup_e0025_vs_a4':med['a4']/med['e0025'],'paired_median_fractional_change':statistics.median(paired),'paired_win_count_e0025':sum(x<0 for x in paired),'paired_fractional_change_e0025_minus_a4':paired,'samples_seconds':samples,'orders':orders,'workload':[str(p.relative_to(root/'arena-tests')) for p in workload]}
