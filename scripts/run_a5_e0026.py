...
root=Path.cwd(); out=root/'results/a5-e0026'; out.mkdir(parents=True,exist_ok=True)
vs=['a5','e0026']; bins={v:root/v/'target/release/sokonanoda' for v in vs}; cfgs={v:root/v/'config.json' for v in vs}
...
med={v:statistics.median(samples[v]) for v in vs}; paired=[(b-a)/a for a,b in zip(samples['a5'],samples['e0026'])]
summary={'substrate':{'sokonanoda':'9b4ea12f4cd437d00b6bcd0e34743065c58dea08','threads':4,'session_budget':2621440,'a5':'E0018 + E0024 App bypass + E0025 Lambda bypass'},'intervention':'remove redundant second force_thunk pair at unify_no_cache entry','correctness':correctness,'median_seconds':med,'speedup_e0026_vs_a5':med['a5']/med['e0026'],'paired_median_fractional_change':statistics.median(paired),'paired_win_count_e0026':sum(x<0 for x in paired),'paired_fractional_change_e0026_minus_a5':paired,'samples_seconds':samples,'orders':orders,'workload':[str(p.relative_to(root/'arena-tests')) for p in workload]}
