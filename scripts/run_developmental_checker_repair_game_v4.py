...
root=Path.cwd(); out=root/'results/developmental-checker-repair-v4'; out.mkdir(parents=True,exist_ok=True)
...
def route_nearest_semantic(ev):
    # Frozen from V3 residual: ignore panic wrappers; choose nearest preceding semantic event.
    # Successful defeq is admissible only when it is the final semantic event before panic,
    # because F1 injects the fault immediately after that boundary.
    semantic=[]
    for e in ev:
...
for ep in EPISODES:
    ...
    full=route_full(ev); nearest=route_nearest_semantic(ev)
    policies={
      'BINARY':BINARY_ORDER,
    ...
    rows.append({**ep,'full_route':full,'nearest_semantic_route':nearest,'events':ev[-32:],'arms':arms})

summary={'status':'LIVE_CONTROLLED_REPAIR_GAME_V4','baseline_full_corpus_gate_pass':True,'episodes':len(rows),'rows':rows}
