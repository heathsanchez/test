...
root=Path.cwd(); out=root/'results/developmental-distinction-discovery-v14'; out.mkdir(parents=True,exist_ok=True)
...
# Frozen before outcomes. These are generic trace descriptors, not theorem/case identities.
...
def semantic_events(ev):
    return [e for e in ev if e.get('kind')!='panic']

def exercises(ev,fam): return any(e.get('site')==SITES[fam] for e in ev)

def add_common_trap(src):
    p=src/'src/tc.rs'; s=p.read_text()
    anchor='const CHUNK_SIZE: usize = 64;'
...
def inject_fault(src,fam):
    ...
    if fam=='INFER_APP':
        ...
        a='eprintln!("[MGTRACE] kind=defeq site=infer.app_arg depth={} ok={}", depth, mg_ok);'
        r=a+'\n                    crate::tc::mg_fault_trap();'
    elif fam=='PROJECTION':
        p=src/'src/infer.rs'; s=p.read_text()
        a='eprintln!("[MGTRACE] kind=projection site=infer.proj depth={}", depth);'
        r=a+'\n        crate::tc::mg_fault_trap();'
    elif fam=='IOTA':
        p=src/'src/conv.rs'; s=p.read_text()
        a='eprintln!("[MGTRACE] kind=iota site=conv.recursor depth={} heads_match={}", depth, heads_match);'
        r=a+'\n                crate::tc::mg_fault_trap();'
...
def feature_row(stderr):
    ev=semantic_events(events(stderr))
    last=ev[-1] if ev else {}
    sites=sorted({e.get('site','') for e in ev if e.get('site')})
    depth=last.get('depth')
    try: d=int(depth) if depth is not None else -1
    except: d=-1
    return {
      'event_count_bucket':bucket_count(len(ev)),
...
def fit_map(rows,features):
    ...
    mapping={k:Counter(v).most_common(1)[0][0] for k,v in groups.items()}
    global_majority=Counter(r['family'] for r in rows).most_common(1)[0][0]
    return mapping,global_majority

def predict(rows,train,features):
    mp,glob=fit_map(train,features)
    return [mp.get(key(r,features),glob) for r in rows]

def error(rows,train,features):
    ps=predict(rows,train,features)
    return sum(p!=r['family'] for p,r in zip(ps,rows))/len(rows)
...
def learn_minimal(train):
    ...
    while cur>0:
        ...
        for f in FEATURES:
            ...
            e=error(train,train,selected+[f])
            cands.append((e,f))
        if not cands: break
        best_e,best_f=min(cands,key=lambda x:(x[0],x[1]))
        history.append({'phase':'split','before_error':cur,'feature':best_f,'after_error':best_e})
        if best_e>=cur: break
        selected.append(best_f); cur=best_e
    # Coagulate: greedily quotient any retained distinction that is not required for zero train error.
    changed=True
    while changed:
        changed=False
        for f in list(selected):
            trial=[x for x in selected if x!=f]
            e=error(train,train,trial)
            history.append({'phase':'quotient_probe','feature':f,'error_without':e})
            if e==0:
                selected=trial; changed=True; break
...
# Hard semantic identity gate: instrumentation must not alter checker outcomes.
mism=[]
...
# Freeze eligible source-distinct cases using only the un-faulted trace, before seeing repair outcomes.
selected={}; eligible={}
for fam in FAMILIES:
    pool=[]
    ...
    pool=sorted(pool); eligible[fam]=[r for _,r in pool]
    need=DEV_PER_FAMILY+HOLDOUT_PER_FAMILY
    if len(pool)<need:
...
for fam in FAMILIES:
    work=root/f'fault-v14-{fam.lower()}'
    ...
    for j,rel in enumerate(selected[fam]):
        case=ARENA/rel
        frc,ferr=run(fault_bins[fam],case)
        if status(frc)!='reject': raise SystemExit(f'fault discriminator failed {fam} {rel}: {status(frc)}')
        loc=native_fault_location(ferr); trap_locations.append(loc)
        rows.append({'family':fam,'case':rel,'split':'dev' if j<DEV_PER_FAMILY else 'holdout','features':feature_row(ferr)})
...
selected_features,history,train_error=learn_minimal(dev)
hold_preds=predict(hold,dev,selected_features); hold_acc=sum(p==r['family'] for p,r in zip(hold_preds,hold))/len(hold)
coarse_error=error(dev,dev,[])

# Prospective verifier tournament on held-out source-distinct cases.
heldout_rows=[]
for r,pred in zip(hold,hold_preds):
    case=ARENA/r['case']; fam=r['family']
    learned_attempts=verifier_calls_for_prediction(fault_bins[fam],trace_bin,case,fam,pred)
    binary_attempts=verifier_calls_for_prediction(fault_bins[fam],trace_bin,case,fam,FIXED_ORDER[0])
...
# Deletion ablation: each surviving distinction must matter on train and/or held-out verifier cost.
ablations=[]
for f in selected_features:
    trial=[x for x in selected_features if x!=f]
    ps=predict(hold,dev,trial)
    calls=[]
    for r,p in zip(hold,ps):
        calls.append(len(verifier_calls_for_prediction(fault_bins[r['family']],trace_bin,ARENA/r['case'],r['family'],p)))
...
summary={
 'status':'LIVE_SELF_DISCOVERED_DISTINCTION_V14',
 'semantic_mismatches':0,
 'common_native_boundary':locs[0],
 'families':FAMILIES,
 'frozen_candidate_features':FEATURES,
 'selected_cases':selected,
 'dev_episodes':len(dev),'holdout_episodes':len(hold),
 'coarse_train_error':coarse_error,
 'selected_features':selected_features,
 'learning_history':history,
 'minimal_train_error':train_error,
 'holdout_accuracy':hold_acc,
 'learned_mean_verifier_calls':sum(learned_calls)/len(learned_calls),
 'binary_mean_verifier_calls':sum(binary_calls)/len(binary_calls),
 'call_reduction_factor':(sum(binary_calls)/len(binary_calls))/(sum(learned_calls)/len(learned_calls)),
...
if train_error!=0: raise SystemExit('split phase failed to make consequence predictable')
if hold_acc!=1.0: raise SystemExit('learned quotient failed source-distinct transfer')
...
if not selected_features: raise SystemExit('no distinction discovered')
